...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0911URL", reqReturnURL
   SetCache "0911DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0911")
...
If (reqSysUserGroup = 41) And (reqOwnerType = 4) And (reqOwnerID = 0) Then
   reqOwnerID = reqSysMemberID
End If
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("0911URL")
      reqReturnData = GetCache("0911DATA")
      SetCache "0911URL", ""
      SetCache "0911DATA", ""
...
Else
   With oCommissions
      .SysCurrentLanguage = reqSysLanguage
      reqBookmark = .FindOwner(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqOwnerType), CLng(reqOwnerID), CDate(reqFromDate), CDate(reqToDate), CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "0911 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0911 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0911 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0911 Load file (oData) failed with error code " + CStr(oData.parseError)
