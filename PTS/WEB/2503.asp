...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "2503URL", reqReturnURL
   SetCache "2503DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "2503")
   ...
   Else
      With oParty
         ...
         If (reqSysUserGroup = 99) Then
            If (CLng(reqCode) = CLng(.ApptID)) Then
               reqTxn = 0
            End If
            If (CLng(reqCode) <> CLng(.ApptID)) Then
               reqTxn = 5
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCode):
      SetCache "ACCESSCODE", reqCode
      LoadParty
   ...
   Case CLng(cActionUpdate):
      ...
      Else
         With oParty
            ...
            .IsTrained = Request.Form.Item("IsTrained")
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oParty
            .SysCurrentLanguage = reqSysLanguage
            .Delete CLng(reqPartyID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (xmlError = "") Then
               reqTxn = 3
            End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "2503 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "2503 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "2503 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "2503 Load file (oData) failed with error code " + CStr(oData.parseError)
