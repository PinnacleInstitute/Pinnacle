...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0452URL", reqReturnURL
   SetCache "0452DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0452")
...
reqBuy =  Numeric(GetInput("Buy", reqPageData))
...
Sub AddMember()
   ...
   Else
      With oMember
         ...
         If (reqBuy = 1) Then
            .Price = 49.95
            .StatusDate = DATEADD("m", 1, reqSysDate)
         End If
         If (reqBuy = 2) Then
            .Price = 44.95
            .StatusDate = DATEADD("m", 3, reqSysDate)
         End If
         If (reqBuy = 3) Then
            .Price = 39.95
            .StatusDate = DATEADD("m", 6, reqSysDate)
         End If
         If (reqBuy = 4) Then
            .Price = 34.95
            .StatusDate = DATEADD("m", 12, reqSysDate)
         End If
...
xmlParam = xmlParam + " buy=" + Chr(34) + CStr(reqBuy) + Chr(34)
...
fileLanguage = "Language" + "\0452[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\0452[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0452 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0452 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0452 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0452 Load file (oData) failed with error code " + CStr(oData.parseError)
