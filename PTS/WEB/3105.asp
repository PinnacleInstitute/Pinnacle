...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "3105URL", reqReturnURL
   SetCache "3105DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "3105")
...
Sub Register()
   ...
   Response.Redirect "3105.asp" & "?AssessmentID=" & reqAssessmentID & "&MemberAssessID=" & reqMemberAssessID & "&MemberID=" & reqMemberID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
End Sub
   ...
   Else
      With oHTMLFile
         ...
         If (Len(.Data) < 20) Then
            ...
            Response.Redirect "3212.asp" & "?AssessmentID=" & reqAssessmentID & "&MemberAssessID=" & reqMemberAssessID & "&MemberID=" & reqMemberID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
         End If
         If (xmlError = "") Then
            xmlHTMLFile = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("3105URL")
      reqReturnData = GetCache("3105DATA")
      SetCache "3105URL", ""
      SetCache "3105DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "3105 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "3105 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "3105 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "3105 Load file (oData) failed with error code " + CStr(oData.parseError)
