...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "3826URL", reqReturnURL
   SetCache "3826DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "3826")
...
End If   
   ...
   Case CLng(cActionUpdate):
      If (reqMode = 1) Or (reqMode = 2) Or (reqMode = 11) Or (reqMode = 12) Then
      ...
      If (xmlError = "") Then
         If (reqMode = 1) Then
            ...
            Else
               With oCommission
                  ...
                  reqResult = Count & " New Enrollment Bonuses Created!"
               End With
            End If
         ...
         If (reqMode = 2) Then
            ...
            Else
               With oCommission
                  ...
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  reqResult = Count & " New Tuition Bonuses Created!"
               End With
            End If
            Set oCommission = Nothing
         End If
         ...
         If (reqMode = 4) Then
            ...
            Else
               With oPayout
                  .SysCurrentLanguage = reqSysLanguage
                  Count = CLng(.CompanyStatus2(CLng(reqCompanyID), reqToDate, CDate(reqSysDate), 50))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         ...
         End If
         If (reqMode = 9) Then
            ExportACH reqCompanyID, reqToDate, 50, tmpTotal, tmpFile
            reqResult = tmpTotal & " ACHs Exported to " + tmpFile + "<br>"
            ExportCheck reqCompanyID, reqToDate, 50, tmpTotal, tmpFile
         ...
         If (reqMode = 11) Then
            ...
            Else
               With oCommission
                  ...
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  reqResult = Count & " New Fast Start Bonuses Created!"
               End With
            End If
            Set oCommission = Nothing
         End If
         If (reqMode = 12) Then
            ...
            Else
               With oCommission
                  .SysCurrentLanguage = reqSysLanguage
                  Count = CLng(.CustomCommissions(CLng(reqCompanyID), 4, reqFromDate, reqToDate))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  reqResult = Count & " New Leadership Bonuses Created!"
               End With
            End If
            Set oCommission = Nothing
         End If
...
fileLanguage = "Language" + "\3826[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\3826[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "3826 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "3826 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "3826 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "3826 Load file (oData) failed with error code " + CStr(oData.parseError)
