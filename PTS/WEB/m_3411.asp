...
Dim oMemberAssesss, xmlMemberAssesss
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_3411URL", reqReturnURL
   SetCache "m_3411DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_3411")
...
reqIsPrivate =  Numeric(GetInput("IsPrivate", reqPageData))
...
If (reqSysUserGroup = 41) And (InStr(reqSysUserOptions,"L") = 0) And (reqM = 0) Then
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If (reqSysUserGroup = 41) Then
         If ((reqMemberID <> reqSysMemberID) And (reqMemberID <> CLng(tmpMentoringID))) Or (reqCompanyID <> reqSysCompanyID) Then

            Response.Redirect "0101.asp" & "?ActionCode=" & 9
         End If
      End If

      If (reqMemberAssessID <> 0) Then
         ...
         Else
            With oMemberAssess
               .SysCurrentLanguage = reqSysLanguage
               .SetPrivate CLng(reqMemberAssessID), reqIsPrivate
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
      ...
      If (reqRemoveID <> 0) Then
         ...
         Else
            With oMemberAssess
               .SysCurrentLanguage = reqSysLanguage
               .Load reqRemoveID, CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               .IsRemoved = 1
         ...
         If (xmlError = "") Then
            Response.Redirect "m_3411.asp" & "?MemberID=" & reqMemberID & "&CompanyID=" & reqCompanyID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup & "&RemoveID=" & 0 & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
         End If
      End If

      Set oMemberAssesss = server.CreateObject("ptsMemberAssessUser.CMemberAssesss")
      If oMemberAssesss Is Nothing Then
         DoError Err.Number, Err.Source, "Unable to Create Object - ptsMemberAssessUser.CMemberAssesss"
      Else
         With oMemberAssesss
            .SysCurrentLanguage = reqSysLanguage
            If (reqSysUserGroup <= 23) Or (reqSysUserGroup = 51) Or (reqSysUserGroup = 52) Then
               .ListAssessmentAll CLng(reqMemberID), CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqSysUserGroup > 23) And (reqSysUserGroup <> 51) And (reqSysUserGroup <> 52) Then
               .ListAssessment CLng(reqMemberID), CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            reqCount = CLng(.Count(CLng(reqSysUserID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlMemberAssesss = .XML(13)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oMemberAssesss = Nothing

   Case CLng(cActionCancel):

      reqReturnURL = GetCache("m_3411URL")
      reqReturnData = GetCache("m_3411DATA")
      SetCache "m_3411URL", ""
      SetCache "m_3411DATA", ""
   ...
   Case CLng(cActionNewAssess):

      Response.Redirect "m_3112.asp" & "?MemberID=" & reqMemberID & "&CompanyID=" & reqCompanyID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup & "&ReturnURL=" & reqPageURL
End Select
...
xmlParam = xmlParam + " isprivate=" + Chr(34) + CStr(reqIsPrivate) + Chr(34)
...
xmlTransaction = xmlTransaction +  xmlMemberAssesss
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_3411 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_3411 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_3411 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_3411 Load file (oData) failed with error code " + CStr(oData.parseError)
