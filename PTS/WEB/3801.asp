...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "3801URL", reqReturnURL
   SetCache "3801DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "3801")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Set oCompanys = server.CreateObject("ptsCompanyUser.CCompanys")
      If oCompanys Is Nothing Then
         DoError Err.Number, Err.Source, "Unable to Create Object - ptsCompanyUser.CCompanys"
      Else
         With oCompanys
            .SysCurrentLanguage = reqSysLanguage
            .FindTypeID = reqFindTypeID
            xmlCompanys = .XMLFind(14)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oCompanys = Nothing
...
xmlTransaction = "<TXN>"
xmlTransaction = xmlTransaction +  xmlCompanys
...
If oLanguage.parseError <> 0 Then
   Response.Write "3801 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "3801 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "3801 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "3801 Load file (oData) failed with error code " + CStr(oData.parseError)
