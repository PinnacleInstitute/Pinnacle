...
Const cActionNew = 0
Const cActionUpdate = 3
Const cActionShowChanges = 4
Const cActionShowAll = 5
'-----page variables
Dim oData
Dim oStyle
'-----system variables
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0098URL", reqReturnURL
   SetCache "0098DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0098")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionShowChanges):
      reqChanges = 1
      LoadLabels
      SaveLabels
      LoadLabels

   Case CLng(cActionShowAll):
      reqChanges = 0
      LoadLabels
      SaveLabels
      LoadLabels
...
xmlTransaction = "<TXN>"
xmlTransaction = xmlTransaction +  xmlLabel
xmlTransaction = xmlTransaction +  xmlEN
...
fileLanguage = "Language\0098.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0098 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0098 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0098 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0098 Load file (oData) failed with error code " + CStr(oData.parseError)
