...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_9506URL", reqReturnURL
   SetCache "m_9506DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_9506")
...
reqSubmitType =  Numeric(GetInput("SubmitType", reqPageData))
reqSubmitID =  Numeric(GetInput("SubmitID", reqPageData))
...
If (reqSysUserGroup = 41) And (reqT = 0) Then
   ...
   reqI = reqSysMemberID
End If
If (reqSysUserGroup = 99) And (reqT = 0) Then
   ...
   End If
End If
If (reqSysUserGroup > 23) And (reqSysUserGroup <> 51) And (reqSysUserGroup <> 52) And (reqT = 0) Then
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Set oIssues = server.CreateObject("ptsIssueUser.CIssues")
      If oIssues Is Nothing Then
         DoError Err.Number, Err.Source, "Unable to Create Object - ptsIssueUser.CIssues"
      Else
         With oIssues
...
If (reqActionCode <> 0) Then
   ...
   Else
      With oIssues
         .SysCurrentLanguage = reqSysLanguage
         If (reqStatus = 0) Then
            reqBookmark = .FindSubmit(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqCompanyID), reqT, reqI, CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqStatus <> 0) Then
            reqBookmark = .FindSubmitStatus(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqCompanyID), reqT, reqI, CLng(reqStatus), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
...
xmlParam = xmlParam + " submittype=" + Chr(34) + CStr(reqSubmitType) + Chr(34)
xmlParam = xmlParam + " submitid=" + Chr(34) + CStr(reqSubmitID) + Chr(34)
xmlParam = xmlParam + " />"

'-----get the transaction XML
xmlTransaction = "<TXN>"
xmlTransaction = xmlTransaction +  xmlIssues
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_9506 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_9506 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_9506 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_9506 Load file (oData) failed with error code " + CStr(oData.parseError)
