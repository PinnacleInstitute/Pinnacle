...
<!--#include file="Include\NewMembers.asp"-->
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "38101URL", reqReturnURL
   SetCache "38101DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "38101")
...
If InStr( "21,22,23,", CSTR(reqMode)+"," ) = 0 Then reqMode = 0  
      ...
      If (xmlError = "") Then
         If (reqMode = 1) Then
            ...
            Else
               With oCommission
                  ...
                  reqResult = Count & " New Startup Bonuses Created!"
               End With
            End If
         ...
         If (reqMode = 4) Then
            ...
            Else
               With oPayout
                  .SysCurrentLanguage = reqSysLanguage
                  Count = CLng(.CompanyStatus2(CLng(reqCompanyID), reqToDate, CDate(reqSysDate), 20))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         ...
         End If
         If (reqMode = 9) Then
            ExportACH reqCompanyID, reqToDate, 20, tmpTotal, tmpFile
            reqResult = tmpTotal & " ACHs Exported to " + tmpFile + "<br>"
            ExportCheck reqCompanyID, reqToDate, 20, tmpTotal, tmpFile
         ...
         If (reqMode = 15) Then
            ...
            Else
               With oCompany
                  .SysCurrentLanguage = reqSysLanguage
                  Count = CLng(.Custom(CLng(reqCompanyID), 20, 0, 0, 0))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  reqResult = Count & " Introductory Offer Upgrades Done!"
               End With
            End If
            Set oCompany = Nothing
         End If
         ...
         End If
         If (reqMode = 21) Then
            tmpTotal = NewMembers( reqCompanyID, 1)
            reqResult = tmpTotal & " New Consultants"
            reqURL = "http://" + reqSysServerName + reqSysServerPath + "Sections/Company/1/NewMembers.csv"
         End If
         If (reqMode = 22) Then
            ...
            Else
               With oCompany
                  ...
                  Count = CLng(.Custom(CLng(reqCompanyID), 6, 0, 0, 0))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  reqResult = Count & " New Consultants marked In Process!"
               End With
            End If
            Set oCompany = Nothing
         End If
         If (reqMode = 23) Then
            ...
            Else
               With oCompany
                  ...
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  reqResult = Count & " New Consultants marked Mailed!"
               End With
            End If
            Set oCompany = Nothing
         End If
...
fileLanguage = "Language" + "\38101[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\38101[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "38101 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "38101 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "38101 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "38101 Load file (oData) failed with error code " + CStr(oData.parseError)
