...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "7612URL", reqReturnURL
   SetCache "7612DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "7612")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionReturn):
      ...
      reqReturnURL = GetCache("7612URL")
      reqReturnData = GetCache("7612DATA")
      SetCache "7612URL", ""
      SetCache "7612DATA", ""
...
If (reqActionCode <> 0) Then
   ...
   Else
      With oMembers
         .SysCurrentLanguage = reqSysLanguage
         reqBookmark = .FindOwner(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, reqCompanyID, CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         xmlMembers = .XML(15)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "7612 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "7612 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "7612 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "7612 Load file (oData) failed with error code " + CStr(oData.parseError)
