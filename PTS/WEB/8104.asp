...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8104URL", reqReturnURL
   SetCache "8104DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8104")
...
If (reqSysUserGroup = 41) And (InStr(reqSysUserOptions,"1") = 0) Then
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("8104URL")
      reqReturnData = GetCache("8104DATA")
      SetCache "8104URL", ""
      SetCache "8104DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "8104 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8104 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8104 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8104 Load file (oData) failed with error code " + CStr(oData.parseError)
