...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "10702URL", reqReturnURL
   SetCache "10702DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "10702")
...
reqMemberCount =  Numeric(GetInput("MemberCount", reqPageData))
...
Sub NewMachine()
   ...
   Else
      With oMachine
         ...
         reqMemberCount = CLng(.MemberCount(CLng(reqMemberID)))
...
Sub AddMachine()
   ...
   End If
               ...
               End If
               If (xmlError <> "") And (MachineID > 0) Then
                  .Delete CLng(.MachineID), CLng(reqSysUserID)
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               End If
            End If
         End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("10702URL")
         reqReturnData = GetCache("10702DATA")
         SetCache "10702URL", ""
         SetCache "10702DATA", ""
...
xmlParam = xmlParam + " membercount=" + Chr(34) + CStr(reqMemberCount) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "10702 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "10702 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "10702 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "10702 Load file (oData) failed with error code " + CStr(oData.parseError)
