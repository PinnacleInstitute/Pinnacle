...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8123URL", reqReturnURL
   SetCache "8123DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8123")
   ...
   Else
      With oProspects
         If (reqMemberID = 0) Then
            .FallbackCompany CLng(reqCompanyID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqMemberID <> 0) Then
            .FallbackMember CLng(reqMemberID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         xmlProspects = .XML(13)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
fileLanguage = "Language\8123.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "8123 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8123 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8123 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8123 Load file (oData) failed with error code " + CStr(oData.parseError)
