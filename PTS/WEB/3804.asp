...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "3804URL", reqReturnURL
   SetCache "3804DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "3804")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      End If
      tmpSecure = Numeric(GetCache("SECURITYLEVEL"))
   ...
   xmlHTMLFile = Replace( xmlHTMLFile, "{name}", reqSysUserName )
...
If oLanguage.parseError <> 0 Then
   Response.Write "3804 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "3804 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "3804 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "3804 Load file (oData) failed with error code " + CStr(oData.parseError)
