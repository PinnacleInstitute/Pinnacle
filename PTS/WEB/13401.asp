...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "13401URL", reqReturnURL
   SetCache "13401DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "13401")
...
Select Case CLng(reqActionCode)
                  ...
                  If (xmlError = "") And (reqBillingPrice <> 0) And (reqNoBill = 0) Then
                     ...
                     If (IsNumeric(Result) <> 0) Then
                        If (tmpUpdatePaidDate <> 0) Then
                           .PaidDate = DATEADD("m", 12, .PaidDate)
                           .Save CLng(reqSysUserID)
                           If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                        End If
                     End If
            ...
            Else
               With oMember
                  ...
                  tmpTo = .Email
                     ...
                     tmpSender = "support@coopincome.com"
                     tmpFrom = "support@coopincome.com"
                     Select Case newTitle
                     Case 2
                        tmpSubject = tmpName + " has upgraded to the CO-OP Silver Program"
                     End Select
                     tmpBody = tmpSubject
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("13401URL")
      reqReturnData = GetCache("13401DATA")
      SetCache "13401URL", ""
      SetCache "13401DATA", ""
...
fileLanguage = "Language" + "\13401[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\13401[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "13401 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "13401 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "13401 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "13401 Load file (oData) failed with error code " + CStr(oData.parseError)
