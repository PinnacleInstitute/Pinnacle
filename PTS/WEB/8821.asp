...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8821URL", reqReturnURL
   SetCache "8821DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8821")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("8821URL")
      reqReturnData = GetCache("8821DATA")
      SetCache "8821URL", ""
      SetCache "8821DATA", ""
...
Else
   With oEmails
      .SysCurrentLanguage = reqSysLanguage
      reqBookmark = .FindTeam(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, reqMemberID, CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      xmlEmails = .XML(15)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
...
If oLanguage.parseError <> 0 Then
   Response.Write "8821 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8821 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8821 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8821 Load file (oData) failed with error code " + CStr(oData.parseError)
