...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "ContactFormURL", reqReturnURL
   SetCache "ContactFormDATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "contactform")
...
Sub AddProspect()
   ...
   If (xmlError = "") And (reqF <> 0) Then
      ...
      Else
         ...
         oHTTP.open "GET", tmpServer + "12502.asp" & "?FolderID=" & reqF & "&MemberID=" & reqM & "&Entity=" & tmpEntity & "&ItemID=" & ProspectID
         oHTTP.send
      End If
      Set oHTTP = Nothing
   End If
...
fileLanguage = "Language" + "\ContactForm[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\ContactForm[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "ContactForm Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "ContactForm Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
xslPage = "ContactForm.xsl"
Set oStyle = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oStyle.load server.MapPath(xslPage)
If oStyle.parseError <> 0 Then
   Response.Write "ContactForm Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "ContactForm Load file (oData) failed with error code " + CStr(oData.parseError)
