...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0901URL", reqReturnURL
   SetCache "0901DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0901")
...
If (reqFindTypeID = 0) Then
   reqFindTypeID = 0910
End If
tmpToDate = Request.Form.Item("ToDate")
...
If (reqActionCode <> 0) Then
   ...
   Else
      With oCommissions
         .SysCurrentLanguage = reqSysLanguage
         reqBookmark = .Find(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqCompanyID), CDate(reqFromDate), tmpToDate, CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "0901 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0901 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0901 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0901 Load file (oData) failed with error code " + CStr(oData.parseError)
