...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "3440URL", reqReturnURL
   SetCache "3440DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "3440")
...
reqStudentName =  GetInput("StudentName", reqPageData)
reqCompanyID =  Numeric(GetInput("CompanyID", reqPageData))
reqDay =  GetInput("Day", reqPageData)
reqMonth =  Numeric(GetInput("Month", reqPageData))
reqYear =  Numeric(GetInput("Year", reqPageData))
reqHeader =  GetInput("Header", reqPageData)
reqSignature =  GetInput("Signature", reqPageData)
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oAssessment
            .SysCurrentLanguage = reqSysLanguage
            .Load tmpAssessmentID, CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            tmpTrainerID = .TrainerID
            xmlAssessment = .XML(2)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      ...
      Else
         With oMember
            ...
            If (.IsCompany <> 0) Then
               reqStudentName = .CompanyName
            End If
            If (.IsCompany = 0) Then
               reqStudentName = .NameFirst + " " + .NameLast
            End If
            xmlMember = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      ...
      Set oTrainer = server.CreateObject("ptsTrainerUser.CTrainer")
      If oTrainer Is Nothing Then
         DoError Err.Number, Err.Source, "Unable to Create Object - ptsTrainerUser.CTrainer"
      Else
         With oTrainer
            .SysCurrentLanguage = reqSysLanguage
            .Load tmpTrainerID, CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlTrainer = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
...
xmlParam = xmlParam + " studentname=" + Chr(34) + CleanXML(reqStudentName) + Chr(34)
xmlParam = xmlParam + " companyid=" + Chr(34) + CStr(reqCompanyID) + Chr(34)
xmlParam = xmlParam + " day=" + Chr(34) + CleanXML(reqDay) + Chr(34)
xmlParam = xmlParam + " month=" + Chr(34) + CStr(reqMonth) + Chr(34)
xmlParam = xmlParam + " year=" + Chr(34) + CStr(reqYear) + Chr(34)
xmlParam = xmlParam + " header=" + Chr(34) + CleanXML(reqHeader) + Chr(34)
xmlParam = xmlParam + " signature=" + Chr(34) + CleanXML(reqSignature) + Chr(34)
xmlParam = xmlParam + " />"
...
xmlTransaction = xmlTransaction +  xmlTrainer
...
fileLanguage = "Language" + "\3440[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\3440[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "3440 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "3440 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "3440 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "3440 Load file (oData) failed with error code " + CStr(oData.parseError)
