...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "15021URL", reqReturnURL
   SetCache "15021DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "15021")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      SetCache "MERCHANTID", reqMerchantID
      ...
      Else
         With oHTMLFile
            .Path = reqSysWebDirectory + "images\store\" & reqMerchantID & "\"
            If (reqDeleteFile <> "") Then
               .Filename = reqDeleteFile
               .Delete 
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            .Ext = "gif,jpg,png"
            xmlHTMLFile = .XMLFiles()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "15021 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "15021 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "15021 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "15021 Load file (oData) failed with error code " + CStr(oData.parseError)
