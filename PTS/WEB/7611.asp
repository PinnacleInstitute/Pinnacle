...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "7611URL", reqReturnURL
   SetCache "7611DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "7611")
...
Sub LoadProjectMembers()
   ...
   Else
      With oProjectMembers
         ...
         .ListMember CLng(reqProjectID)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         xmlProjectMembers = .XML(13)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCopy):
      ...
      Else
         With oProjectMember
            .SysCurrentLanguage = reqSysLanguage
            Result = CLng(.Copy(CLng(reqProjectID), reqParentID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oProjectMember = Nothing
   ...
   Case CLng(cActionReturn):
      ...
      reqReturnURL = GetCache("7611URL")
      reqReturnData = GetCache("7611DATA")
      SetCache "7611URL", ""
      SetCache "7611DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "7611 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "7611 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "7611 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "7611 Load file (oData) failed with error code " + CStr(oData.parseError)
