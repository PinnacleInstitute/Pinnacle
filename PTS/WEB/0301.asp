...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0301URL", reqReturnURL
   SetCache "0301DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0301")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Set oTrainers = server.CreateObject("ptsTrainerUser.CTrainers")
      If oTrainers Is Nothing Then
         DoError Err.Number, Err.Source, "Unable to Create Object - ptsTrainerUser.CTrainers"
      Else
         With oTrainers
...
xmlTransaction = "<TXN>"
xmlTransaction = xmlTransaction +  xmlTrainers
...
If oLanguage.parseError <> 0 Then
   Response.Write "0301 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0301 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0301 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0301 Load file (oData) failed with error code " + CStr(oData.parseError)
