...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "17103URL", reqReturnURL
   SetCache "17103DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "17103")
...
Sub UpdateBarterCategory()
   ...
   Else
      With oBarterCategory
         ...
         .BarterCategoryName = Request.Form.Item("BarterCategoryName")
         .CustomFields = Request.Form.Item("CustomFields")
         .Options = SetOptions()
...
Sub AddBarterCategory()
   ...
   Else
      With oBarterCategory
         ...
         .CustomFields = Request.Form.Item("CustomFields")
         .Options = SetOptions()
         reqBarterCategoryID = CLng(.Add(CLng(reqSysUserID)))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         If (xmlError = "") And (.ParentID <> 0) Then
            Status = CLng(.UpdateChildren(CLng(.ParentID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      UpdateBarterCategory

      If (xmlError = "") Then
         reqReturnURL = GetCache("17103URL")
         reqReturnData = GetCache("17103DATA")
         SetCache "17103URL", ""
         SetCache "17103DATA", ""
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oBarterCategory
            .SysCurrentLanguage = reqSysLanguage
            .Delete CLng(reqBarterCategoryID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oBarterCategory = Nothing
...
fileLanguage = "Language" + "\BarterCategory[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\BarterCategory[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "17103 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "17103 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "17103 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "17103 Load file (oData) failed with error code " + CStr(oData.parseError)
