...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "4805URL", reqReturnURL
   SetCache "4805DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "4805")
      ...
      If (reqMemberID = reqCode) Then
         ...
         Else
            With oAppt
               ...
               .IsEdit = 0
               tmpApptName = .ApptName + " : " + .StartDate + " : " + .StartTime + " - " + .EndTime 
               .Save CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               xmlAppt = .XML(2)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               tmpEmail = .CalendarEmail(CLng(.CalendarID))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "4805 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "4805 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "4805 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "4805 Load file (oData) failed with error code " + CStr(oData.parseError)
