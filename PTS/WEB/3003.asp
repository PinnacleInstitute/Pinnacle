...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "3003URL", reqReturnURL
   SetCache "3003DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "3003")
...
Sub UpdateOrgCourse()
   ...
   Else
      With oOrgCourse
         ...
         If (reqOrgCourseID = 0) Then
            reqOrgCourseID = CLng(.GetOrgCourse(CLng(reqOrgID), CLng(reqCourseID), CLng(reqSysUserID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         .Load CLng(reqOrgCourseID), CLng(reqSysUserID)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("3003URL")
         reqReturnData = GetCache("3003DATA")
         SetCache "3003URL", ""
         SetCache "3003DATA", ""
      ...
      Else
         With oOrgCourse
            ...
            If (reqOrgCourseID = 0) Then
               ...
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            .Delete CLng(reqOrgCourseID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oOrgCourse = Nothing
...
fileLanguage = "Language" + "\OrgCourse[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\OrgCourse[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "3003 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "3003 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "3003 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "3003 Load file (oData) failed with error code " + CStr(oData.parseError)
