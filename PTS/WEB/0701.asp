...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0701URL", reqReturnURL
   SetCache "0701DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0701")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Set oPromotions = server.CreateObject("ptsPromotionUser.CPromotions")
      If oPromotions Is Nothing Then
         DoError Err.Number, Err.Source, "Unable to Create Object - ptsPromotionUser.CPromotions"
      Else
         With oPromotions
            .FindTypeID = reqFindTypeID
            xmlPromotions = .XML(14)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oPromotions = Nothing

   Case CLng(cActionCancel):

      reqReturnURL = GetCache("0701URL")
      reqReturnData = GetCache("0701DATA")
      SetCache "0701URL", ""
      SetCache "0701DATA", ""
...
xmlTransaction = "<TXN>"
xmlTransaction = xmlTransaction +  xmlPromotions
...
If oLanguage.parseError <> 0 Then
   Response.Write "0701 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0701 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0701 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0701 Load file (oData) failed with error code " + CStr(oData.parseError)
