...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "13213URL", reqReturnURL
   SetCache "13213DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "13213")
...
reqActiveDate =  GetInput("ActiveDate", reqPageData)
reqAvatar =  GetInput("Avatar", reqPageData)
reqURL =  GetInput("URL", reqPageData)
reqURLFacebook =  GetInput("URLFacebook", reqPageData)
reqURLTwitter =  GetInput("URLTwitter", reqPageData)
reqURLGoogle =  GetInput("URLGoogle", reqPageData)
reqURLLinkedIn =  GetInput("URLLinkedIn", reqPageData)
reqTest =  Numeric(GetInput("Test", reqPageData))
reqogtitle =  GetInput("ogtitle", reqPageData)
reqogimage =  GetInput("ogimage", reqPageData)
...
reqCompanyID = 12
If (reqNewsID = 0) And (reqS <> 0) Then
   reqNewsID = reqS
End If
MemberID = Numeric(GetCache("A"))
GroupID = Numeric(GetCache("GROUPID"))

               reqURL = "http://" + reqSysServerName + reqSysServerPath + "13213.asp?s=" + CStr(reqNewsID)
               If GroupID > 0 Then reqURL = reqURL + "&g=" + CStr(GroupID)
               If MemberID > 0 Then reqURL = reqURL + "&a=" + CStr(MemberID)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oNews
            ...
            .Load reqNewsID, CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            ShareTitle = .Title
            tmpCategory = .NewsTopicID
            reqActiveDate = FormatDateTime(.ActiveDate,1)
            ActiveTime = FormatDateTime(.ActiveDate,3)
            If (ActiveTime <> "12:00:00 AM") Then
               reqActiveDate = reqActiveDate + " " + ActiveTime
            End If
            
                  If Len(ShareTitle) <= 80 Then ShareShortTitle = ShareTitle Else ShareShortTitle = Left(ShareTitle,80) + "..."
                  ShareURL = Server.URLEncode(reqURL)
                  ShareImage = "http://" + reqSysServerName + reqSysServerPath + "Images/News/" + CStr(reqCompanyID) + "/" + .Image
                  reqogtitle = ShareTitle
                  reqogimage = ShareImage
 
            xmlNews = .XML(2)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            reqAvatar = .Avatar(.AuthUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oNews = Nothing
      ...
      Else
         With oHTMLFile
            .Filename = "AddThis.htm"
            .Path = reqSysWebDirectory + "Sections\Company\" + CStr(reqCompanyID)
            .Language = reqSysLanguage
            .Project = SysProject
            .Load 
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlAddThis = .XML("AddThis")
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            
                  xmlAddThis = Replace( xmlAddThis, "{title}", ShareTitle )
                  xmlAddThis = Replace( xmlAddThis, "{shorttitle}", ShareShortTitle )
                  xmlAddThis = Replace( xmlAddThis, "{url}", ShareURL )
      ...
      Else
         With oAds
            .SysCurrentLanguage = reqSysLanguage
            .ListAds reqCompanyID, 2, tmpCategory, CLng(reqSysUserGroup), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlAds = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
...
xmlParam = xmlParam + " activedate=" + Chr(34) + CleanXML(reqActiveDate) + Chr(34)
xmlParam = xmlParam + " avatar=" + Chr(34) + CleanXML(reqAvatar) + Chr(34)
xmlParam = xmlParam + " url=" + Chr(34) + CleanXML(reqURL) + Chr(34)
xmlParam = xmlParam + " urlfacebook=" + Chr(34) + CleanXML(reqURLFacebook) + Chr(34)
xmlParam = xmlParam + " urltwitter=" + Chr(34) + CleanXML(reqURLTwitter) + Chr(34)
xmlParam = xmlParam + " urlgoogle=" + Chr(34) + CleanXML(reqURLGoogle) + Chr(34)
xmlParam = xmlParam + " urllinkedin=" + Chr(34) + CleanXML(reqURLLinkedIn) + Chr(34)
xmlParam = xmlParam + " test=" + Chr(34) + CStr(reqTest) + Chr(34)
xmlParam = xmlParam + " ogtitle=" + Chr(34) + CleanXML(reqogtitle) + Chr(34)
xmlParam = xmlParam + " ogimage=" + Chr(34) + CleanXML(reqogimage) + Chr(34)
...
xmlTransaction = xmlTransaction +  xmlAddThis
xmlTransaction = xmlTransaction +  xmlDisqus
...
If oLanguage.parseError <> 0 Then
   Response.Write "13213 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "13213 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "13213 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "13213 Load file (oData) failed with error code " + CStr(oData.parseError)
...
Response.Write Replace( oData.transformNode(oStyle), "charset=UTF-16", "charset=UTF-8" )
