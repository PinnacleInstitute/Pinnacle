...
Dim fileLanguage
'-----object variables
Dim oBarterCampaigns, xmlBarterCampaigns
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "17501URL", reqReturnURL
   SetCache "17501DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "17501")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Set oBarterCampaigns = server.CreateObject("ptsBarterCampaignUser.CBarterCampaigns")
      If oBarterCampaigns Is Nothing Then
         DoError Err.Number, Err.Source, "Unable to Create Object - ptsBarterCampaignUser.CBarterCampaigns"
      Else
         With oBarterCampaigns
            .SysCurrentLanguage = reqSysLanguage
            .FindTypeID = reqFindTypeID
            xmlBarterCampaigns = .XML(14)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oBarterCampaigns = Nothing
...
Else
   With oBarterCampaigns
      ...
      If (reqConsumerID <> 0) Then
         If (reqStatus = 0) Then
            reqBookmark = .Find(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqConsumerID), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqStatus <> 0) Then
            reqBookmark = .FindStatus(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqConsumerID), CLng(reqStatus), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
      End If
      If (reqConsumerID = 0) Then
         If (reqStatus = 0) Then
            reqBookmark = .FindAd(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqBarterAdID), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqStatus <> 0) Then
            reqBookmark = .FindAdStatus(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqBarterAdID), CLng(reqStatus), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
      End If
      xmlBarterCampaigns = .XML(15)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
End If
Set oBarterCampaigns = Nothing
...
xmlTransaction = "<TXN>"
xmlTransaction = xmlTransaction +  xmlBarterCampaigns
...
If oLanguage.parseError <> 0 Then
   Response.Write "17501 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "17501 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "17501 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "17501 Load file (oData) failed with error code " + CStr(oData.parseError)
