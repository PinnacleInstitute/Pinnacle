...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "15002URL", reqReturnURL
   SetCache "15002DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "15002")
...
reqOrg =  Numeric(GetInput("Org", reqPageData))
...
Sub AddMerchant()
   ...
   Else
      With oMerchant
         ...
         .Phone2 = .Phone
         .Options = "ACDE"
         .StoreOptions = "ABCDEFH"
         .Colors = "000000,FFFFFF,,FFFFFF,"
         .CurrencyCode = "USD"
         .Terms = "Rewards are earned on pre-tax and pre-tip amounts. Redeemable for total bill only."
         .ReferRate = 10
         ...
         End If
         If (xmlError = "") Then
            MerchantID = CLng(.Add(CLng(reqSysUserID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (xmlError <> "") Then
            xmlMerchant = .XML(2)
      ...
      If (xmlError = "") Then
         ...
         Else
            With oAward
               ...
               AwardID = CLng(.Add(CLng(reqSysUserID)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
         Set oAward = Nothing
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("15002URL")
         reqReturnData = GetCache("15002DATA")
         SetCache "15002URL", ""
         SetCache "15002DATA", ""
...
xmlParam = xmlParam + " org=" + Chr(34) + CStr(reqOrg) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "15002 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "15002 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "15002 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "15002 Load file (oData) failed with error code " + CStr(oData.parseError)
