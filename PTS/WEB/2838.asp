...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "2838URL", reqReturnURL
   SetCache "2838DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "2838")
...
reqtmpName =  GetInput("tmpName", reqPageData)
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oAddresss
            .SysCurrentLanguage = reqSysLanguage
            .ListOwnerType 04, CLng(reqMemberID), 2
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      ...
      Else
         With oHTMLFile
            ...
            .Path = reqSysWebDirectory + "Sections\Org\" & CStr(reqOrgID)
...
xmlParam = xmlParam + " tmpname=" + Chr(34) + CleanXML(reqtmpName) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "2838 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "2838 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "2838 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "2838 Load file (oData) failed with error code " + CStr(oData.parseError)
