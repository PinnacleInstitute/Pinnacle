...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0405URL", reqReturnURL
   SetCache "0405DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0405")
   ...
   Else
      With oMember
         ...
         If (.Status = 5) Then
            reqExpired = 1
         End If
         If (.Status = 2) And (.BillingID = 0) And (DateAdd( "d", .TrialDays, FormatDateTime(.EnrollDate,2)) > reqSysDate) Then
            reqExpired = 1
         End If
         xmlMember = .XML(2)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpgrade):
      ...
      If (reqBilling = 3) And (reqBillingID = 0) Then
         If reqSysServerName <> "localhost" Then
            tmpRedirect = "https://" + reqSysServerName + reqSysServerPath
         End If
         Response.Redirect tmpRedirect + "2902.asp" & "?MemberID=" & reqMemberID & "&NextURL=" & "0413.asp?MemberID=" & reqMemberID & "%26Status=" & reqStatus & "%26Level=" & reqLevel
      End If

      Response.Redirect "0413.asp" & "?MemberID=" & reqMemberID & "&Status=" & reqStatus & "&Level=" & reqLevel & "&Txn=" & 1

   Case CLng(cActionContinue):

      Response.Redirect "0404.asp" & "?MemberID=" & reqSysMemberID

   Case CLng(cActionExit):
...
If oLanguage.parseError <> 0 Then
   Response.Write "0405 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0405 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0405 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0405 Load file (oData) failed with error code " + CStr(oData.parseError)
