...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "GCRPaymentPortalURL", reqReturnURL
   SetCache "GCRPaymentPortalDATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "gcrpaymentportal")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("GCRPaymentPortalURL")
      reqReturnData = GetCache("GCRPaymentPortalDATA")
      SetCache "GCRPaymentPortalURL", ""
      SetCache "GCRPaymentPortalDATA", ""
...
fileLanguage = "Language" + "\GCRPaymentPortal[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\GCRPaymentPortal[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "GCRPaymentPortal Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "GCRPaymentPortal Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
xslPage = "GCRPaymentPortal.xsl"
Set oStyle = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oStyle.load server.MapPath(xslPage)
If oStyle.parseError <> 0 Then
   Response.Write "GCRPaymentPortal Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "GCRPaymentPortal Load file (oData) failed with error code " + CStr(oData.parseError)
