...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "17701URL", reqReturnURL
   SetCache "17701DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "17701")
...
reqURL = "http://" + reqSysServerName + reqSysServerPath + "Seminar/Seminar"
reqLanguage = "en"
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Set oSeminars = server.CreateObject("ptsSeminarUser.CSeminars")
      If oSeminars Is Nothing Then
         DoError Err.Number, Err.Source, "Unable to Create Object - ptsSeminarUser.CSeminars"
      Else
         With oSeminars
...
xmlTransaction = "<TXN>"
xmlTransaction = xmlTransaction +  xmlSeminars
...
If oLanguage.parseError <> 0 Then
   Response.Write "17701 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "17701 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "17701 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "17701 Load file (oData) failed with error code " + CStr(oData.parseError)
