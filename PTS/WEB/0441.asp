...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0441URL", reqReturnURL
   SetCache "0441DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0441")
...
reqNoClose =  Numeric(GetInput("NoClose", reqPageData))
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionRemoveBilling):
      ...
      Else
         With oBilling
            .SysCurrentLanguage = reqSysLanguage
            .Delete tmpPayID, CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oBilling = Nothing
   ...
   Case CLng(cActionBilling):
      ...
      Response.Redirect "2913.asp" & "?MemberID=" & reqMemberID & "&CompanyID=" & reqCompanyID & "&BillingID=" & reqBillingID & "&contentpage=" & 3 & "&ReturnURL=" & reqPageURL

   Case CLng(cActionAddBilling):

      Response.Redirect "2912.asp" & "?MemberID=" & reqMemberID & "&CompanyID=" & reqCompanyID & "&contentpage=" & 3 & "&ReturnURL=" & reqPageURL
End Select
...
xmlParam = xmlParam + " noclose=" + Chr(34) + CStr(reqNoClose) + Chr(34)
...
fileLanguage = "Language" + "\0441[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\0441[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0441 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0441 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0441 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0441 Load file (oData) failed with error code " + CStr(oData.parseError)
