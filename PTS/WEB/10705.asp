...
Const cActionNew = 0
Const cActionLogon2 = 1
Const cActionLogon = 2
Const cActionForgotPassword = 3
Const cActionForgotPassword2 = 4
'-----page variables
Dim oData
Dim oStyle
'-----system variables
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "10705URL", reqReturnURL
   SetCache "10705DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "10705")
...
reqLogon2 =  GetInput("Logon2", reqPageData)
reqPassword2 =  GetInput("Password2", reqPageData)
reqRemember2 =  Numeric(GetInput("Remember2", reqPageData))
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If (reqLogon <> "") Then
         reqRemember = 1
      End If
      reqLogon2 = GetCookie("LGN2")
      reqPassword2 = GetCookie("PWD2")
      If (reqLogon2 <> "") Then
         reqRemember2 = 1
      End If

   Case CLng(cActionLogon2):
      If (reqRemember2 = "1") Then
         SetCookie "LGN2", reqLogon2
         SetCookie "PWD2", reqPassword2
      End If
      If (reqRemember2 = "") Then
         SetCookie "LGN2", ""
         SetCookie "PWD2", ""
      End If
      ...
      Else
         With oMachine
            ...
            Result = .Logon(reqLogon2, reqPassword2)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oMachine = Nothing
      tmpResultCode = Left(Result,1)
      tmpResultNum = Mid(Result,2)
      If (Result = "E1") Then
         DoError 10129, "", "Oops, Your Logon Email could not be found."
      End If
      If (Result = "E2") Then
         DoError 10130, "", "Oops, Your Password is incorrect."
      End If
      If (tmpResultCode = "C") Then
         ...
         Else
            With oMember
               .SysCurrentLanguage = reqSysLanguage
               .Load tmpResultNum, CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               tmpStatus = .Status
         ...
         Set oMember = Nothing
         If (xmlError = "") Then
            SetCache "CUSTOMER", tmpResultNum

               tmpRedirect = "http://" + reqSysServerName + reqSysServerPath
            Response.Redirect tmpRedirect + "10711.asp" & "?MemberID=" & tmpResultNum & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
         End If
      End If
      If (tmpResultCode = "M") Then
         SetCache "CUSTOMER", tmpResultCode

            tmpRedirect = "http://" + reqSysServerName + reqSysServerPath
         Response.Redirect tmpRedirect + "10711.asp" & "?MachineID=" & tmpResultNum & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
      End If
   ...
   Case CLng(cActionForgotPassword):
      ...
      Else
         With oMachine
            .SysCurrentLanguage = reqSysLanguage
            .FetchEmail reqLogon2
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            tmpMachineID = .MachineID
            If (tmpMachineID = 0) Then
               DoError 10129, "", "Oops, Your Logon Email could not be found."
            End If
         End With
      End If
   ...
   Case CLng(cActionForgotPassword2):
...
xmlParam = xmlParam + " logon2=" + Chr(34) + CleanXML(reqLogon2) + Chr(34)
xmlParam = xmlParam + " password2=" + Chr(34) + CleanXML(reqPassword2) + Chr(34)
xmlParam = xmlParam + " remember2=" + Chr(34) + CStr(reqRemember2) + Chr(34)
...
fileLanguage = "Language" + "\10705[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\10705[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "10705 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "10705 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "10705 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "10705 Load file (oData) failed with error code " + CStr(oData.parseError)
