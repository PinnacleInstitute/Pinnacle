...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0411URL", reqReturnURL
   SetCache "0411DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0411")
...
reqMax =  Numeric(GetInput("Max", reqPageData))
...
Else
   With oMember
      ...
      reqCount = CLng(.ActiveCount(CLng(reqCompanyID), CLng(reqMasterID)))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      reqMax = CLng(.MaxCount(CLng(reqMasterID)))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      If (CLng(reqCount) < 0) Then
...
Else
   With oMembers
      .SysCurrentLanguage = reqSysLanguage
      reqBookmark = .FindMaster(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMasterID), CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
xmlParam = xmlParam + " max=" + Chr(34) + CStr(reqMax) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "0411 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0411 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0411 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0411 Load file (oData) failed with error code " + CStr(oData.parseError)
