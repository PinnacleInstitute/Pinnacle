...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0449URL", reqReturnURL
   SetCache "0449DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0449")
...
End If
If (reqSysUserGroup = 99) And (reqMemberID <> tmpMemberID) Then
...
Else
   With oMembers
      .SysCurrentLanguage = reqSysLanguage
      reqBookmark = .FindCustomer(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
Else
   With oMember
      .SysCurrentLanguage = reqSysLanguage
      reqCount = CLng(.TotalCustomers(CLng(reqMemberID)))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
End If
Set oMember = Nothing
...
fileLanguage = "Language" + "\0449[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\0449[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0449 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0449 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0449 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0449 Load file (oData) failed with error code " + CStr(oData.parseError)
