...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "MemberHome1URL", reqReturnURL
   SetCache "MemberHome1DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "memberhome1")
...
fileLanguage = "Language\MemberHome1[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\MemberHome1[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "MemberHome1 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "MemberHome1 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
s=s+         "<IMAGE name=""MemberDomain.gif"" width=""18"" height=""18""/>"
...
s=s+         "<IMAGE name=""GrpMembers.gif"" width=""18"" height=""18""/>"
...
xslPage = "MemberHome1.xsl"
Set oStyle = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oStyle.load server.MapPath(xslPage)
If oStyle.parseError <> 0 Then
   Response.Write "MemberHome1 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "MemberHome1 Load file (oData) failed with error code " + CStr(oData.parseError)
