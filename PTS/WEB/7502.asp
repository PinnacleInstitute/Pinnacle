...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "7502URL", reqReturnURL
   SetCache "7502DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "7502")
...
Sub LoadLists()
   ...
   If (reqParentID = 0) Then
      ...
      Else
         With oProjectTypes
            .SysCurrentLanguage = reqSysLanguage
            xmlProjectTypes = xmlProjectTypes + .EnumCompany(CLng(reqCompanyID), tmpProjectTypeID, , CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      ...
      Else
         With oProject
            ...
            ProjectID = CLng(.Add(CLng(reqSysUserID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            .Hierarchy = tmpHierarchy & ProjectID & "/"
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("7502URL")
         reqReturnData = GetCache("7502DATA")
         SetCache "7502URL", ""
         SetCache "7502DATA", ""
   ...
   Case CLng(cActionCopy):
      tmpProjectID = Request.Form.Item("ProjectID")

      Response.Redirect "7505.asp" & "?ProjectID=" & tmpProjectID & "&CompanyID=" & reqCompanyID & "&MemberID=" & reqMemberID
...
If oLanguage.parseError <> 0 Then
   Response.Write "7502 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "7502 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "7502 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "7502 Load file (oData) failed with error code " + CStr(oData.parseError)
