...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8001URL", reqReturnURL
   SetCache "8001DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8001")
...
Else
   With oAttachments
      ...
      If (reqCompanyID <> 0) Then
         ...
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      If (reqTrainerID <> 0) Then
         reqBookmark = .FindTrainer(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, reqTrainerID, CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      If (reqCourseID <> 0) Then
         reqBookmark = .FindCourse(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, reqCourseID, CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      xmlAttachments = .XML(15)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
End If
Set oAttachments = Nothing
...
If oLanguage.parseError <> 0 Then
   Response.Write "8001 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8001 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8001 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8001 Load file (oData) failed with error code " + CStr(oData.parseError)
