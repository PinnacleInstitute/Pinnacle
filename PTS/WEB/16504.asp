...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "16504URL", reqReturnURL
   SetCache "16504DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "16504")
...
reqBalance =  GetInput("Balance", reqPageData)
reqDate =  GetInput("Date", reqPageData)
reqTxns =  GetInput("Txns", reqPageData)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oPayment2
            ...
            Check2FAMerchant .MerchantID
            If (.PayType > 2) Then
               CoinType = .PayType-2
               reqURL = CoinBlockchain(CoinType,.Reference)
               nam = CoinName(CoinType)
               adr = .Reference
               If (adr <> "") Then
                  s = CoinInfo(CoinType,adr)
               ...
               End If
               If (.Status <> 3) And (CDBL(.PaidCoins) <> CDBL(tmpPaid)) Then
                  .PaidCoins = tmpPaid
...
xmlParam = xmlParam + " balance=" + Chr(34) + CleanXML(reqBalance) + Chr(34)
xmlParam = xmlParam + " date=" + Chr(34) + CleanXML(reqDate) + Chr(34)
xmlParam = xmlParam + " txns=" + Chr(34) + CleanXML(reqTxns) + Chr(34)
xmlParam = xmlParam + " due=" + Chr(34) + CleanXML(reqDue) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "16504 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "16504 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "16504 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "16504 Load file (oData) failed with error code " + CStr(oData.parseError)
