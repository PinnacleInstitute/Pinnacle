...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "15101URL", reqReturnURL
   SetCache "15101DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "15101")
...
reqPersonal =  Numeric(GetInput("Personal", reqPageData))
reqGroup =  Numeric(GetInput("Group", reqPageData))
reqMerchants =  Numeric(GetInput("Merchants", reqPageData))
...
Sub GetReferralCounts()
   On Error Resume Next
   If (reqReferID <> 0) Then
      tmpConsumerID = reqReferID
      tmpType = 1
   End If
   If (reqMerchantID <> 0) Then
      tmpConsumerID = reqMerchantID
      tmpType = 2
   End If
   If (reqMemberID <> 0) Then
      tmpConsumerID = reqMemberID
      tmpType = 3
   End If
   ...
   Else
      With oConsumer
         ...
         Result = .Custom(tmpConsumerID, 2, tmpType, 0, "")
...
End If
If (reqMerchantID <> 0) Or (reqMemberID <> 0) Or (reqReferID <> 0) Then
   If (reqTitle = "") Then
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If (reqReferID <> 0) Or (reqMerchantID <> 0) Or (reqMemberID <> 0) Then
         GetReferralCounts
      End If
...
Else
   With oConsumers
      .SysCurrentLanguage = reqSysLanguage
      If (reqMerchantID = 0) And (reqMemberID = 0) And (reqReferID = 0) Then
         reqBookmark = .Find(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      If (reqMerchantID <> 0) Then
         reqBookmark = .FindMerchant(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMerchantID), CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      If (reqMemberID <> 0) Then
      ...
      End If
      If (reqReferID <> 0) Then
         reqBookmark = .FindRefer(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqReferID), CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      xmlConsumers = .XML(15)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
xmlParam = xmlParam + " personal=" + Chr(34) + CStr(reqPersonal) + Chr(34)
xmlParam = xmlParam + " group=" + Chr(34) + CStr(reqGroup) + Chr(34)
xmlParam = xmlParam + " merchants=" + Chr(34) + CStr(reqMerchants) + Chr(34)
xmlParam = xmlParam + " />"

'-----get the transaction XML
xmlTransaction = "<TXN>"
xmlTransaction = xmlTransaction +  xmlConsumer
xmlTransaction = xmlTransaction +  xmlConsumers
...
If oLanguage.parseError <> 0 Then
   Response.Write "15101 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "15101 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "15101 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "15101 Load file (oData) failed with error code " + CStr(oData.parseError)
