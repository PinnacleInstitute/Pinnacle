...
pos = InStr(LCASE(reqSysServerPath), "0418")
...
reqNotify =  Numeric(GetInput("Notify", reqPageData))
reqID =  Numeric(GetInput("ID", reqPageData))
reqMailReturnURL =  GetInput("MailReturnURL", reqPageData)
reqIP =  GetInput("IP", reqPageData)
...
If oStyle.parseError <> 0 Then
   Response.Write "0418 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oMember
            ...
            tmpFrom = .Email
            tmpSubject = "MENTOR NOTICE: " + .NameFirst + " " + .NameLast + " - "
            tmpNotifyMentor = .NotifyMentor
            If (reqNotify = 7) Then
               tmpSubject = tmpSubject + "Signed In"
            End If
            If (reqNotify = 1) And (InStr(tmpNotifyMentor, "A") = 0) Then
               response.end
            End If
            If (reqNotify = 2) And (InStr(tmpNotifyMentor, "B") = 0) Then
               response.end
            End If
            If (reqNotify = 3) And (InStr(tmpNotifyMentor, "C") = 0) Then
               response.end
            End If
            If (reqNotify = 4) And (InStr(tmpNotifyMentor, "D") = 0) Then
               response.end
            End If
            If (reqNotify = 5) And (InStr(tmpNotifyMentor, "E") = 0) Then
               response.end
            End If
            If (reqNotify = 6) And (InStr(tmpNotifyMentor, "F") = 0) Then
               response.end
            End If
            If (reqNotify = 7) And (InStr(tmpNotifyMentor, "G") = 0) Then
               response.end
            End If
            If (reqNotify = 8) And (InStr(tmpNotifyMentor, "H") = 0) Then
               response.end
            End If
            If (reqNotify = 9) And (InStr(tmpNotifyMentor, "I") = 0) Then
               response.end
            End If
            If (.MentorID = 0) Then
               response.end
      ...
      If (reqNotify = 1) Then
         ...
         Else
            With oSession
               .SysCurrentLanguage = reqSysLanguage
               .Load reqID, CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               tmpSubject = tmpSubject + "Completed Class"
               xmlSession = .XML()
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
      ...
      If (reqNotify = 2) Or (reqNotify = 3) Then
         ...
         Else
            With oAssessment
               ...
               End If
               If (reqNotify = 2) Then
                  tmpSubject = tmpSubject + "Completed Assessment"
               End If
               If (reqNotify = 3) Then
                  tmpSubject = tmpSubject + "Completed Certification"
               End If
               xmlAssessment = .XML()
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
      ...
      If (reqNotify = 3) Then
         ...
         Else
            With oMemberAssess
               .SysCurrentLanguage = reqSysLanguage
               tmpMemberAssessID = CLng(.CheckAssessment(reqID, CLng(reqMemberID), CLng(reqSysUserID)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               If (tmpMemberAssessID <> 0) Then
                  .Load tmpMemberAssessID, CLng(reqSysUserID)
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  If (.Status = 1) Then
                     tmpSubject = tmpSubject + " - PASSED"
                  End If
                  If (.Status = 2) Then
                     tmpSubject = tmpSubject + " - FAILED"
                  End If
                  tmpSubject = tmpSubject + " " + .Result
               End If
            End With
         End If
      ...
      If (reqNotify = 4) Then
         ...
         Else
            With oGoal
               ...
               .Load reqID, CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               tmpSubject = tmpSubject + "Completed Goal"
               xmlGoal = .XML()
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
      ...
      If (reqNotify = 5) Then
         ...
         Else
            With oProject
               ...
               .Load reqID, CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               tmpSubject = tmpSubject + "Completed Project"
               xmlProject = .XML()
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
      ...
      If (reqNotify = 6) Then
         ...
         Else
            With oProspect
               ...
               .Load reqID, CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               tmpSubject = tmpSubject + "Completed Sale"
               xmlProspect = .XML()
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
      ...
      If (reqNotify = 8) Or (reqNotify = 9) Then
         ...
         Else
            With oMetric
               ...
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               If (reqNotify = 8) Then
                  tmpSubject = tmpSubject + "Logged Activity"
               End If
               If (reqNotify = 9) Then
                  tmpSubject = tmpSubject + "Logged Result"
               End If
               xmlMetric = .XML()
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
...
fileLanguage = "Language" + "\0418[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\0418[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0418 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0418 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
xmlParam = xmlParam + " notify=" + Chr(34) + CStr(reqNotify) + Chr(34)
xmlParam = xmlParam + " id=" + Chr(34) + CStr(reqID) + Chr(34)
xmlParam = xmlParam + " mailreturnurl=" + Chr(34) + CleanXML(reqMailReturnURL) + Chr(34)
xmlParam = xmlParam + " ip=" + Chr(34) + CleanXML(reqIP) + Chr(34)
xmlParam = xmlParam + " />"
...
If oData.parseError <> 0 Then
   Response.Write "0418 Load file (oData) failed with error code " + CStr(oData.parseError)
