...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1083URL", reqReturnURL
   SetCache "1083DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1083")
...
Sub ReloadPayment()
   ...
   Else
      With oPayment
         ...
         .Commission = Request.Form.Item("Commission")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("1083URL")
      reqReturnData = GetCache("1083DATA")
      SetCache "1083URL", ""
      SetCache "1083DATA", ""
      If (Len(reqReturnURL) > 0) Then
         SetCache "RETURNURL", reqReturnURL
...
fileLanguage = "Language\1083.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "1083 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1083 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1083 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1083 Load file (oData) failed with error code " + CStr(oData.parseError)
