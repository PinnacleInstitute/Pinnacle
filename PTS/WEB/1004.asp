...
Const cActionNew = 0
Const cActionRetry = 1
Const cActionContinue = 2
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1004URL", reqReturnURL
   SetCache "1004DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1004")
...
reqTxnStatus =  Numeric(GetInput("TxnStatus", reqPageData))
reqIsTxn =  GetInput("IsTxn", reqPageData)
reqAcct =  GetInput("Acct", reqPageData)
...
Function ProcessPayment(ByVal bvID)
   On Error Resume Next
   ...
   If (reqCompanyID = 0) Then
      ...
      Else
         With oBusiness
            ...
            tmpCardProcessor = .CardProcessor
            tmpCardAcct = .CardAcct
            tmpCheckProcessor = .CheckProcessor
            tmpCheckAcct = .CheckAcct
            tmpMerchantName = .BusinessName
         End With
      End If
...
xmlParam = xmlParam + " txnstatus=" + Chr(34) + CStr(reqTxnStatus) + Chr(34)
xmlParam = xmlParam + " istxn=" + Chr(34) + CleanXML(reqIsTxn) + Chr(34)
xmlParam = xmlParam + " acct=" + Chr(34) + CleanXML(reqAcct) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "1004 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1004 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1004 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1004 Load file (oData) failed with error code " + CStr(oData.parseError)
