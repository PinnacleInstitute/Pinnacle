...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1721URL", reqReturnURL
   SetCache "1721DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1721")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oHTMLFile
            .Path = reqSysWebDirectory + "images\company\" & reqCompanyID & "\question\"
...
If oLanguage.parseError <> 0 Then
   Response.Write "1721 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1721 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1721 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1721 Load file (oData) failed with error code " + CStr(oData.parseError)
