...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_WelcomeURL", reqReturnURL
   SetCache "m_WelcomeDATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_welcome")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionSignIn):
      ...
      Response.Redirect "0101.asp" & "?Lgn=" & tmpLgn & "&Pwd=" & tmpPwd & "&Page=" & "mobile.asp?m={id}"
End Select
...
fileLanguage = "Language" + "\m_Welcome[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\m_Welcome[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "m_Welcome Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_Welcome Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
xslPage = "m_Welcome.xsl"
Set oStyle = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oStyle.load server.MapPath(xslPage)
If oStyle.parseError <> 0 Then
   Response.Write "m_Welcome Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_Welcome Load file (oData) failed with error code " + CStr(oData.parseError)
