...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0461URL", reqReturnURL
   SetCache "0461DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0461")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oMembers
            If (reqReport = 1) Then
               .ReportMembershipDetail CLng(reqCompanyID), reqFromDate, reqToDate, reqIDField
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqReport = 4) Then
               .ReportBillingDetail CLng(reqCompanyID), reqFromDate, reqToDate, reqIDField
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqReport = 5) And (reqOption = 0) Then
               .ReportVisitDetail CLng(reqCompanyID), reqIDField
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqReport = 5) And (reqOption <> 0) Then
               .ReportVisitDetailStatus CLng(reqCompanyID), reqIDField, reqOption
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
...
fileLanguage = "Language\0461.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0461 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0461 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0461 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0461 Load file (oData) failed with error code " + CStr(oData.parseError)
