...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "11711URL", reqReturnURL
   SetCache "11711DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "11711")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      ...
      Response.Redirect "11702.asp" & "?MemberID=" & reqMemberID & "&ReturnURL=" & reqPageURL

   Case CLng(cActionReturn):

      reqReturnURL = GetCache("11711URL")
      reqReturnData = GetCache("11711DATA")
      SetCache "11711URL", ""
      SetCache "11711DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "11711 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "11711 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "11711 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "11711 Load file (oData) failed with error code " + CStr(oData.parseError)
