...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0436URL", reqReturnURL
   SetCache "0436DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0436")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionRemoveBilling):
      ...
      Else
         With oBilling
            .SysCurrentLanguage = reqSysLanguage
            .Delete tmpBillingID, CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oBilling = Nothing
...
fileLanguage = "Language" + "\0436[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\0436[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0436 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0436 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0436 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0436 Load file (oData) failed with error code " + CStr(oData.parseError)
