...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8161URL", reqReturnURL
   SetCache "8161DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8161")
...
If (reqSysUserGroup = 41) Then
   ...
   End If
End If
If (reqM <> 0) Or (reqCompanyID = 0) Then
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("8161URL")
      reqReturnData = GetCache("8161DATA")
      SetCache "8161URL", ""
      SetCache "8161DATA", ""
...
Else
   With oProspects
      .SysCurrentLanguage = reqSysLanguage
      If (reqFolderID <> 0) Then
         If (reqStatus = -20) Then
            reqBookmark = .FindFolder(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqFolderID), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
      End If
      If (reqFolderID = 0) Then
         If (reqStatus = -20) Then
            reqBookmark = .FindLead(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqStatus = -21) Then
            reqBookmark = .FindLeadStatus(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), -1, CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqStatus = -22) Then
            reqBookmark = .FindLeadActive(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqStatus = -23) Then
            reqBookmark = .FindLeadLive(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqStatus > -20) Then
            reqBookmark = .FindLeadStatus(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqStatus), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
      End If
      xmlProspects = .XMLLead(15)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
End If
Set oProspects = Nothing
...
Else
   With oPages
      .SysCurrentLanguage = reqSysLanguage
      xmlPages = .EnumPage(CLng(reqCompanyID), CLng(reqMemberID), tmpGroupID1, tmpGroupID2, tmpGroupID3, 4)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
End If
Set oPages = Nothing
...
Else
   With oFolders
      .SysCurrentLanguage = reqSysLanguage
      xmlAddFolders = xmlAddFolders + .EnumFolderAll(CLng(reqCompanyID), tmpGroupID, CLng(reqMemberID), 22, , "AddFolder", CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      xmlFolders = xmlFolders + .EnumFolder(CLng(reqCompanyID), tmpGroupID, CLng(reqMemberID), 22, , , CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "8161 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8161 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8161 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8161 Load file (oData) failed with error code " + CStr(oData.parseError)
