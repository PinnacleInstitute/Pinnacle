...
pos = InStr(LCASE(reqSysServerPath), "5215")
...
If oStyle.parseError <> 0 Then
   Response.Write "5215 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oSalesOrder
            ...
            tmpBCC = .ProductEmails(CLng(reqSalesOrderID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlSalesOrder = .XML(2)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
...
fileLanguage = "Language" + "\5215[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\5215[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "5215 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "5215 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "5215 Load file (oData) failed with error code " + CStr(oData.parseError)
