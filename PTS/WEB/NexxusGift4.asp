...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "NexxusGift4URL", reqReturnURL
   SetCache "NexxusGift4DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "nexxusgift4")
   ...
   If (CCUR(reqWalletNumber) >= CCUR(reqQty * reqPV)) Then
         ...
         Else
            With oPayout
              ...
              tmpLocation = GetIPCity( IP )
               ...
               .Notes = Left("Course Certificate " + IP + " " + tmpLocation, 500) 
      ...
      If (xmlError = "") Then
         ...
         Else
            With oPayment
               ...
               .Reference = PayoutID
               .CommStatus = 1
               If (LEN(CSTR(reqPV)) = 1) Then
                  .Retail = CSTR(reqQty) + ".0" + CSTR(reqPV)
               End If
               If (LEN(CSTR(reqPV)) <> 1) Then
                  .Retail = CSTR(reqQty) + "." + CSTR(reqPV)
               End If
      ...
      If (xmlError = "") Then
         ...
         Else
            With oCompany
               .SysCurrentLanguage = reqSysLanguage
               Count = CLng(.Custom(reqCompanyID, 99, 0, reqPaymentID, 0))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      If (xmlError = "") Then
         Response.Redirect "CryptoPay.asp" & "?PaymentID=" & reqPaymentID
      End If

   Case CLng(cActionUpdate2):
      AddOrder
      reqMode = 1

   Case CLng(cActionCancel):

      reqReturnURL = GetCache("NexxusGift4URL")
      reqReturnData = GetCache("NexxusGift4DATA")
      SetCache "NexxusGift4URL", ""
      SetCache "NexxusGift4DATA", ""
...
fileLanguage = "Language" + "\NexxusGift4[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\NexxusGift4[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "NexxusGift4 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "NexxusGift4 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
xslPage = "NexxusGift4.xsl"
Set oStyle = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oStyle.load server.MapPath(xslPage)
If oStyle.parseError <> 0 Then
   Response.Write "NexxusGift4 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "NexxusGift4 Load file (oData) failed with error code " + CStr(oData.parseError)
