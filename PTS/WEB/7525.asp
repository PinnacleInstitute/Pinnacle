...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "7525URL", reqReturnURL
   SetCache "7525DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "7525")
...
If (reqActionCode <> 0) Then
   ...
   Else
      With oProjects
         If (reqMemberID = 0) Then
            If (reqReport = 1) Then
               .ListCompanyPending CLng(reqCompanyID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqReport = 2) Then
               .ListCompanyStarted CLng(reqCompanyID), CDate(reqFromDate), CDate(reqToDate)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqReport = 3) Then
               .ListCompanyCompleted CLng(reqCompanyID), CDate(reqFromDate), CDate(reqToDate)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
         End If
         If (reqMemberID <> 0) Then
...
fileLanguage = "Language\7525.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "7525 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "7525 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "7525 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "7525 Load file (oData) failed with error code " + CStr(oData.parseError)
