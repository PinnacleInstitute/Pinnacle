...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "9301URL", reqReturnURL
   SetCache "9301DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "9301")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("9301URL")
      reqReturnData = GetCache("9301DATA")
      SetCache "9301URL", ""
      SetCache "9301DATA", ""
...
Else
   With oPages
      .SysCurrentLanguage = reqSysLanguage
      If (reqMemberID = 0) And (reqCategory = "") Then
         reqBookmark = .FindCompany(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqCompanyID), CLng(reqPageType), CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      If (reqMemberID = 0) And (reqCategory <> "") Then
         reqBookmark = .FindCompanyCategory(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqCompanyID), CLng(reqPageType), reqCategory, CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      If (reqMemberID <> 0) Then
         reqBookmark = .FindMember(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqPageType), CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      xmlPages = .XML(15)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "9301 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "9301 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "9301 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "9301 Load file (oData) failed with error code " + CStr(oData.parseError)
