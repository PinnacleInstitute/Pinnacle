...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "13805URL", reqReturnURL
   SetCache "13805DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "13805")
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oGCR
            ...
            xmlStats = .Custom(12, 0, reqDay, reqOrgID)
      ...
      If (reqOrgID <> 0) Then
         ...
         Else
            With oOrg
               .SysCurrentLanguage = reqSysLanguage
               .Load reqOrgID, CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               xmlOrg = .XML(2)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
...
fileLanguage = "Language" + "\13805[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\13805[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "13805 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "13805 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "13805 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "13805 Load file (oData) failed with error code " + CStr(oData.parseError)
