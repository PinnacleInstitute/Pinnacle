...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "17411URL", reqReturnURL
   SetCache "17411DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "17411")
...
reqCreditType =  Numeric(GetInput("CreditType", reqPageData))
...
Sub LoadBarterCredits()
   ...
   Else
      With oBarterCredit
         .SysCurrentLanguage = reqSysLanguage
         Result = .Total(151, reqConsumerID)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
Sub UpdateBarterCredits()
   On Error Resume Next
   tmpCreditType = Request.Form.Item("CreditType")
   ...
   Else
      With oBarterCredit
         .SysCurrentLanguage = reqSysLanguage
         Result = .Custom(1, CLng(reqBarterAdID), tmpCreditType, "")
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
   Set oBarterCredit = Nothing
...
xmlParam = xmlParam + " credittype=" + Chr(34) + CStr(reqCreditType) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "17411 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "17411 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "17411 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "17411 Load file (oData) failed with error code " + CStr(oData.parseError)
