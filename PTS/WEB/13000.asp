...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "13000URL", reqReturnURL
   SetCache "13000DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "13000")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      If (reqSysUserGroup = 21) Then
         If InStr( ",2,3,5,11,12,21,22,23,24,25,26,29,33,34,39,44,45,46,47,48,49,", CSTR(reqProcess)+"," ) = 0 Then reqProcess = 0
      End If
   ...
   Case CLng(cActionUpdate):
      ...
      If (xmlError = "") Then
         ...
         If (reqProcess = 30) Then
            ...
            Set oSilvers = server.CreateObject("ptsSilverUser.CSilvers")
            If oSilvers Is Nothing Then
               DoError Err.Number, Err.Source, "Unable to Create Object - ptsSilverUser.CSilvers"
            Else
               With oSilvers
                  .SysCurrentLanguage = reqSysLanguage
                  .CustomList 3, 0, 0, 0
         ...
         If (reqProcess = 35) Then
            ...
            If (tmpTotal <> 0) Then
               URL = "http://www.pinnaclep.com/orders/" + CStr(reqCompanyID) + "/" + tmpFile
               LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
               MSG = CSTR(tmpTotal) + " Silver Submitted Orders (" & FormatCurrency(tmpAmount,2) & ")  "
               reqResult = reqResult + MSG + LINK + "<br>"
               NOTE = NOTE + MSG + URL + "  " + VBLF
            End If
            If (Total <> 0) Then
...
xmlTransaction = xmlTransaction +  xmlSilvers
...
fileLanguage = "Language" + "\13000[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\13000[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "13000 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "13000 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "13000 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "13000 Load file (oData) failed with error code " + CStr(oData.parseError)
