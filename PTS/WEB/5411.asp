...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "5411URL", reqReturnURL
   SetCache "5411DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "5411")
...
Else
   With oDownlines
      .SysCurrentLanguage = reqSysLanguage
      reqBookmark = .Find(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqLine), reqMemberID, CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      xmlDownlines = .XML(15)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
...
If oLanguage.parseError <> 0 Then
   Response.Write "5411 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "5411 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "5411 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "5411 Load file (oData) failed with error code " + CStr(oData.parseError)
