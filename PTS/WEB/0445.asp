...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0445URL", reqReturnURL
   SetCache "0445DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0445")
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oCoption
            ...
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (tmpLevel = 0) Then
               reqOptions = GetUserOptions(.FreeOptions, reqOptions)
            End If
            If (tmpLevel = 1) Then
               reqOptions = GetUserOptions(.Options, reqOptions)
            End If
            If (tmpLevel = 2) Then
               reqOptions = GetUserOptions(.Options2, reqOptions)
            End If
            If (tmpLevel = 3) Then
               reqOptions = GetUserOptions(.Options3, reqOptions)
            End If
         End With
      End If
      ...
      Else
         With oMember
            .SysCurrentLanguage = reqSysLanguage
            xmlPerformance = .Performance(CLng(reqMemberID), reqFromDate, reqToDate, reqOptions)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oMember = Nothing
...
xmlTransaction = xmlTransaction +  xmlPerformance
...
fileLanguage = "Language" + "\0445[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\0445[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0445 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0445 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0445 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0445 Load file (oData) failed with error code " + CStr(oData.parseError)
