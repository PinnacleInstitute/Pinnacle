...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "3814URL", reqReturnURL
   SetCache "3814DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "3814")
...
Sub UpdateCompany()
   ...
   Else
      With oCompany
         ...
         .MenuBarColor = Request.Form.Item("MenuBarColor")
...
Select Case CLng(reqActionCode)
   ...
   FilePath = Request.ServerVariables("APPL_PHYSICAL_PATH") + "Images\Company\" + CStr(reqCompanyID) + "\Header[" + reqSysLanguage + "]"
   Set oFileSys = CreateObject("Scripting.FileSystemObject")
   Exists = oFileSys.FileExists(FilePath + ".gif")
   If Exists Then
      reqHeader = "Header[" + reqSysLanguage + "].gif"
   Else
      Exists = oFileSys.FileExists(FilePath + ".jpg")
      If Exists Then
         reqHeader = "Header[" + reqSysLanguage + "].jpg"
      End If
   End If
...
Set oFileSys = Nothing
   ...
   Case CLng(cActionUploadTBImage):
      UpdateCompany

      If (xmlError = "") Then
         Response.Redirect "3820.asp" & "?CompanyID=" & reqCompanyID & "&ImageName=" & "tbimage"
      End If
End Select
...
fileLanguage = "Language\3814.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "3814 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "3814 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "3814 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "3814 Load file (oData) failed with error code " + CStr(oData.parseError)
