...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "9501URL", reqReturnURL
   SetCache "9501DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "9501")
...
reqAssignedTo =  GetInput("AssignedTo", reqPageData)
...
If (reqCompanyID = 0) Then
   response.write "System Error! Company # Required"
End If
If (reqSysUserName <> "") Then
   reqAssignedTo = Left(reqSysUserName,InStr(reqSysUserName," ")-1)
End If
...
If (reqActionCode <> 0) Then
   ...
   Else
      With oIssues
         .SysCurrentLanguage = reqSysLanguage
         If (reqSysUserGroup <= 21) Or (reqSysUserGroup = 51) Or (reqSysUserGroup = 52) Then
            If (reqStatus = 0) Then
            ...
            End If
         End If
         If (reqSysUserGroup > 21) And (reqSysUserGroup <> 51) And (reqSysUserGroup <> 52) Then
            If (reqStatus = 0) Then
               reqBookmark = .FindAssigned(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, reqAssignedTo, CLng(reqSysUserID))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqStatus <> 0) Then
               reqBookmark = .FindAssignedStatus(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, reqAssignedTo, CLng(reqStatus), CLng(reqSysUserID))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
         End If
...
xmlParam = xmlParam + " assignedto=" + Chr(34) + CleanXML(reqAssignedTo) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "9501 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "9501 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "9501 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "9501 Load file (oData) failed with error code " + CStr(oData.parseError)
