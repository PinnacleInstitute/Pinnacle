...
Const cActionNew = 0
Const cActionResetPassword = 5
Const cActionDisableWallet = 6
'-----page variables
Dim oData
Dim oStyle
'-----system variables
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "WalletInfoURL", reqReturnURL
   SetCache "WalletInfoDATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "walletinfo")
...
reqShowAdmin =  Numeric(GetInput("ShowAdmin", reqPageData))
...
Sub LoadMember()
   ...
   Else
      With oHTMLFile
         ...
         xmlHTMLFile = .XML()
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         If (reqShowAdmin <> 1) Or (reqMember2FA <> 1) Then
            tmpEmail = "**********"
            tmpReference = "**********"
         End If
...
If (reqSysUserGroup = 41) And (reqMemberID <> reqSysMemberID) Then
   AbortUser()
End If
If (reqSysUserGroup <= 23) Or (reqSysUserGroup = 51) Or (reqSysUserGroup = 41) Then
   reqShowAdmin = 1
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionResetPassword):
...
xmlParam = xmlParam + " showadmin=" + Chr(34) + CStr(reqShowAdmin) + Chr(34)
...
fileLanguage = "Language" + "\WalletInfo[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\WalletInfo[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "WalletInfo Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "WalletInfo Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
xslPage = "WalletInfo.xsl"
Set oStyle = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oStyle.load server.MapPath(xslPage)
If oStyle.parseError <> 0 Then
   Response.Write "WalletInfo Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "WalletInfo Load file (oData) failed with error code " + CStr(oData.parseError)
