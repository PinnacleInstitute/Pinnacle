...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1112URL", reqReturnURL
   SetCache "1112DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1112")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("1112URL")
      reqReturnData = GetCache("1112DATA")
      SetCache "1112URL", ""
      SetCache "1112DATA", ""
   ...
   Case CLng(cActionAdd):
      ...
      Response.Redirect "1102.asp" & "?TrainerID=" & reqTrainerID & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
...
Else
   With oCourses
      .SysCurrentLanguage = reqSysLanguage
      reqBookmark = .FindTrainer(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqTrainerID), CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      xmlCourses = .XML(15)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
...
If oLanguage.parseError <> 0 Then
   Response.Write "1112 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1112 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1112 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1112 Load file (oData) failed with error code " + CStr(oData.parseError)
