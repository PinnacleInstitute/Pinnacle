...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "14401URL", reqReturnURL
   SetCache "14401DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "14401")
...
Else
   With oBroadcasts
      .SysCurrentLanguage = reqSysLanguage
      If (reqFriendGroupID = 0) Then
         reqBookmark = .Find(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      If (reqFriendGroupID <> 0) Then
         reqBookmark = .FindGroup(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqFriendGroupID), CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      xmlBroadcasts = .XML(15)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
End If
Set oBroadcasts = Nothing
...
If oLanguage.parseError <> 0 Then
   Response.Write "14401 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "14401 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "14401 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "14401 Load file (oData) failed with error code " + CStr(oData.parseError)
