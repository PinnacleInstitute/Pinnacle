...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0442URL", reqReturnURL
   SetCache "0442DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0442")
...
reqIsBuy =  Numeric(GetInput("IsBuy", reqPageData))
reqIsJoin =  Numeric(GetInput("IsJoin", reqPageData))
...
Sub AddMember()
   ...
   Else
      With oMember
         ...
         End If
         If (xmlError = "") And (reqReferID <> 0) Then
   ...
   If (xmlError = "") Then
      ...
      Set oAddress = Nothing
   End If
   If (reqIsBuy <> 0) Or (reqIsJoin <> 0) Then
      ...
      Else
         With oSalesOrder
            ...
            If (reqIsBuy <> 0) Then
               ...
               .Shipping = 13.55
            End If
            reqSalesOrderID = CLng(.Add(CLng(reqSysUserID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      ...
      Else
         With oSalesItem
            ...
            .Load 0, CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            .SalesOrderID = reqSalesOrderID
            .Status = 1
            .Quantity = 1
            If (reqIsBuy <> 0) Then
               .ProductID = 74
               .Price = 99.95
               reqSalesItemID = CLng(.Add(CLng(reqSysUserID)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqIsJoin <> 0) Then
               .ProductID = 75
      ...
      reqTotal = 0
      If (reqIsBuy <> 0) Then
         reqTotal = reqTotal + 113.50
      End If
      If (reqIsJoin <> 0) Then
         reqTotal = reqTotal + 85
      End If
      ...
      If (xmlError = "") Then
         ...
         Else
            With oPayment
               ...
               If (reqIsBuy <> 0) And (reqIsJoin <> 0) Then
                  .Purpose = "FBMS + Super Sampler Pack"
               End If
               If (reqIsBuy <> 0) And (reqIsJoin = 0) Then
                  .Purpose = "Super Sampler Pack"
               End If
               If (reqIsBuy = 0) And (reqIsJoin <> 0) Then
                  .Purpose = "FBMS"
               End If
...
xmlParam = xmlParam + " isbuy=" + Chr(34) + CStr(reqIsBuy) + Chr(34)
xmlParam = xmlParam + " isjoin=" + Chr(34) + CStr(reqIsJoin) + Chr(34)
...
fileLanguage = "Language" + "\0442[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\0442[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0442 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0442 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0442 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0442 Load file (oData) failed with error code " + CStr(oData.parseError)
