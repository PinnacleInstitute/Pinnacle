...
Const cActionDefaults = 5
Const cActionGraphics = 6
Const cActionReferrals = 9
Const cActionReferralIDs = 10
'-----page variables
Dim oData
Dim oStyle
'-----system variables
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "3803URL", reqReturnURL
   SetCache "3803DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "3803")
...
reqDomain =  GetInput("Domain", reqPageData)
...
Sub UpdateCompany()
   ...
   Else
      With oCompany
         ...
         .Subnet = Request.Form.Item("Subnet")
...
End Sub
   ...
   reqDomain = "." + reqSysServerName + reqSysServerPath
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("3803URL")
      reqReturnData = GetCache("3803DATA")
      SetCache "3803URL", ""
      SetCache "3803DATA", ""
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oCompany
            .SysCurrentLanguage = reqSysLanguage
            .Delete CLng(reqCompanyID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oCompany = Nothing
      ...
      If (xmlError = "") Then
         ...
         Else
            With oCoption
               ...
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               .Delete .CoptionID, CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
   ...
   Case CLng(cActionDefaults):
      UpdateCompany

      If (xmlError = "") Then
         Response.Redirect "4903.asp" & "?CompanyID=" & reqCompanyID
      End If

   Case CLng(cActionGraphics):
      UpdateCompany

      If (xmlError = "") Then
         Response.Redirect "4904.asp" & "?CompanyID=" & reqCompanyID
      End If

   Case CLng(cActionReferrals):
      UpdateCompany

      If (xmlError = "") Then
         Response.Redirect "3850.asp" & "?CompanyID=" & reqCompanyID
      End If

   Case CLng(cActionReferralIDs):
      UpdateCompany
      ...
      Else
         With oCompany
            ...
            xmlCompany = .XML(2)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            tmpCount = CLng(.ReferralIDs(CLng(reqCompanyID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            
               DoError 0, "Referrals", tmpCount & " Referrals Linked!"
...
xmlParam = xmlParam + " domain=" + Chr(34) + CleanXML(reqDomain) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "3803 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "3803 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "3803 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "3803 Load file (oData) failed with error code " + CStr(oData.parseError)
