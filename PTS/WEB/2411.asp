...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "2411URL", reqReturnURL
   SetCache "2411DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "2411")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oSessionLessons
            .SysCurrentLanguage = reqSysLanguage
            .ListSessionLesson CLng(reqSessionID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("2411URL")
      reqReturnData = GetCache("2411DATA")
      SetCache "2411URL", ""
      SetCache "2411DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "2411 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "2411 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "2411 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "2411 Load file (oData) failed with error code " + CStr(oData.parseError)
