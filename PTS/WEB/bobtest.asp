...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "bobtestURL", reqReturnURL
   SetCache "bobtestDATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "bobtest")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If (reqSysUserGroup <> 41) Then
         ...
         Set oMember = Nothing
      End If
      GetMenuColors reqCompanyID, reqMemberID, reqMenuBehindColor, MenuTopColor, MenuTopBGColor, MenuColor, MenuBGColor, MenuShadowColor, MenuBDColor, MenuOverColor, MenuOverBGColor, MenuDividerColor, reqMenuBehindImage, MenuTopImage, MenuImage
      ...
      Else
         With oNewsTopics
            ...
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  ...
                  s = "<MENU name=""TopicMenu="""" type=""bar="""" menuwidth=""125"""" height=""30"""" top-color=""" & MenuTopColor & """ top-bgcolor=""" & MenuTopBGColor & """ color=""" & MenuColor & """ bgcolor=""" & MenuBGColor & """ shadow-color=""" & MenuShadowColor & """ bdcolor=""" & MenuBDColor & """ over-color=""" & MenuOverColor & """ over-bgcolor=""" & MenuOverBGColor & """ divider-color=""" & MenuDividerColor & """ top-bgimg=""" & MenuTopImage & """ bgimg=""" & MenuImage & """>"
                  Child = -1
                  MainName = ""
                  MainID = 0
                  ...
                  If Child >= 0 Then
                  If Child = 0 Then
                  s=s+ "<LINK name=""JAVA(ShowTopic=""(" + CSTR(MainID) + "))""/>"
                  s=s+ "</ITEM>"
                  Else
                  s=s+ "</ITEM>"
                  End If
                  ...
                  s=s+ "<ITEM value=""" + MainName + """ menuwidth=""200"" width=""" + CStr(width) + """>"
                  Else
                  ...
                  s=s+ "</ITEM>"
                  End If
                  s=s+ "<ITEM value=""" + .NewTopicName + """>"
                  s=s+ "<LINK name=""JAVA(ShowTopic=""(" + CSTR(id) + "))""/>"
                  s=s+ "</ITEM>"
                  Child = Child + 1
                  End If
                  cnt = cnt + 1
...
fileLanguage = "Language\13210[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\13210[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "bobtest Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "bobtest Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "bobtest Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "bobtest Load file (oData) failed with error code " + CStr(oData.parseError)
