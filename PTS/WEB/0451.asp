...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0451URL", reqReturnURL
   SetCache "0451DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0451")
...
Else
   With oMembers
      .SysCurrentLanguage = reqSysLanguage
      reqBookmark = .FindGroup(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqGroupID), CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "0451 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0451 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0451 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0451 Load file (oData) failed with error code " + CStr(oData.parseError)
