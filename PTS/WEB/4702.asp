...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "4702URL", reqReturnURL
   SetCache "4702DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "4702")
   ...
   Case CLng(cActionAdd):
      ...
      Else
         With oCalendar
            ...
            CalendarID = CLng(.Add(CLng(reqSysUserID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (xmlError <> "") Then
               xmlCalendar = .XML(2)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      ...
      If (xmlError = "") Then
         Response.Redirect "4703.asp" & "?CalendarID=" & CalendarID & "&Updated=" & 3
      End If
End Select
...
If oLanguage.parseError <> 0 Then
   Response.Write "4702 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "4702 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "4702 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "4702 Load file (oData) failed with error code " + CStr(oData.parseError)
