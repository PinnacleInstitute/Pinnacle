...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1103URL", reqReturnURL
   SetCache "1103DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1103")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      Else
         With oCourse
            ...
            .NoEvaluation = Request.Form.Item("NoEvaluation")
            ...
            .Pre = Request.Form.Item("Pre")
            .ExamID = Request.Form.Item("ExamID")
            .ExamWeight = Request.Form.Item("ExamWeight")
            .IsCustomCertificate = Request.Form.Item("IsCustomCertificate")
            .NoCertificate = Request.Form.Item("NoCertificate")
            .Description = Request.Form.Item("Description")
            newCompanyID = .CompanyID
            If (Request.Form.Item("MakePending") = "on") Then
               .Status = 1
            End If
            If (Request.Form.Item("MakeReview") = "on") Then
               .Status = 2
            End If
            If (tmpStatus <> 3) And (.Status = 3) Then
               .CourseDate = reqSysDate
            End If
            .Save CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (xmlError = "") And (.Status = 3) Then
               Result = CLng(.UpdateFT(CLng(.CourseID), .CourseName, .TrainerName, .Description))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (xmlError <> "") Then
               xmlCourse = .XML(2)
      ...
      If (newCompanyID <> oldCompanyID) Then
         ...
         Else
            With oAttachment
               .SysCurrentLanguage = reqSysLanguage
               return = CLng(.UpdateCompanyID(23, CLng(reqCourseID), newCompanyID))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("1103URL")
         reqReturnData = GetCache("1103DATA")
         SetCache "1103URL", ""
         SetCache "1103DATA", ""
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oCourse
            .SysCurrentLanguage = reqSysLanguage
            .Delete CLng(reqCourseID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oCourse = Nothing
...
If oLanguage.parseError <> 0 Then
   Response.Write "1103 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1103 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1103 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1103 Load file (oData) failed with error code " + CStr(oData.parseError)
