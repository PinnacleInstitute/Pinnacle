...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0402URL", reqReturnURL
   SetCache "0402DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0402")
...
reqMemberAdded =  GetInput("MemberAdded", reqPageData)
...
reqTry =  Numeric(GetInput("Try", reqPageData))
...
reqAR =  Numeric(GetInput("AR", reqPageData))
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      If (reqAR = 0) Or (reqAR <> 0) And (reqA <> 0) Then
         If (reqCompanyID <> 0) And (reqSysCompanyID = 0) Then
            GetCompany(reqCompanyID)
            If (reqGroupID <> 0) Then
               SetCompanyGrpHeader reqCompanyID, reqGroupID, reqSysLanguage
            End If
         End If
   ...
   Case CLng(cActionNext):
      ...
      If (reqStatus = 4) Then
         reqStatus = 3
      End If
      If (reqMembership = 1) And ((reqStatus = 1) Or (reqMembership = 2)) Then
         ActiveMembers reqCompanyID, reqMasterID
      End If
      ...
      If (xmlError = "") Then
         ...
         Response.Redirect "0412.asp" & "?CompanyID=" & reqCompanyID & "&MasterID=" & reqMasterID & "&Membership=" & reqMembership & "&Status=" & reqStatus & "&Level=" & tmpLevel & "&GroupID=" & reqGroupID & "&Refer=" & reqRefer
      End If
...
xmlParam = xmlParam + " memberadded=" + Chr(34) + CleanXML(reqMemberAdded) + Chr(34)
...
xmlParam = xmlParam + " try=" + Chr(34) + CStr(reqTry) + Chr(34)
...
xmlParam = xmlParam + " ar=" + Chr(34) + CStr(reqAR) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "0402 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0402 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0402 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0402 Load file (oData) failed with error code " + CStr(oData.parseError)
