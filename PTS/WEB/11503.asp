...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "11503URL", reqReturnURL
   SetCache "11503DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "11503")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("11503URL")
         reqReturnData = GetCache("11503DATA")
         SetCache "11503URL", ""
         SetCache "11503DATA", ""
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oDripPage
            .SysCurrentLanguage = reqSysLanguage
            .Delete CLng(reqDripPageID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oDripPage = Nothing
...
If oLanguage.parseError <> 0 Then
   Response.Write "11503 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "11503 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "11503 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "11503 Load file (oData) failed with error code " + CStr(oData.parseError)
