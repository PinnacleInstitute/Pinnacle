...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "5202URL", reqReturnURL
   SetCache "5202DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "5202")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      ...
      If (xmlError = "") Then
         Response.Redirect "5203.asp" & "?SalesOrderID=" & SalesOrderID & "&ContentPage=" & ContentPage & "&ReturnURL=" & GetCache("5202URL")
      End If

   Case CLng(cActionCancel):

      reqReturnURL = GetCache("5202URL")
      reqReturnData = GetCache("5202DATA")
      SetCache "5202URL", ""
      SetCache "5202DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "5202 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "5202 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "5202 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "5202 Load file (oData) failed with error code " + CStr(oData.parseError)
