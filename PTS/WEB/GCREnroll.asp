...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "GCREnrollURL", reqReturnURL
   SetCache "GCREnrollDATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "gcrenroll")
...
reqReferID =  GetInput("ReferID", reqPageData)
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      If (reqIsAgree = 0) And (reqIsAgree2 = 0) Then
...
xmlParam = xmlParam + " referid=" + Chr(34) + CleanXML(reqReferID) + Chr(34)
...
fileLanguage = "Language" + "\GCREnroll[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\GCREnroll[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "GCREnroll Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "GCREnroll Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
xslPage = "GCREnroll.xsl"
Set oStyle = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oStyle.load server.MapPath(xslPage)
If oStyle.parseError <> 0 Then
   Response.Write "GCREnroll Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "GCREnroll Load file (oData) failed with error code " + CStr(oData.parseError)
