...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "9002iURL", reqReturnURL
   SetCache "9002iDATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "9002i")
...
reqPath =  GetInput("Path", reqPageData)
...
Sub SendIssueMsg()
   ...
   If (tmpToName <> "") Then
      ...
      Else
         With oOrg
            .SysCurrentLanguage = reqSysLanguage
            tmpTo = .GetEmail(tmpCompanyID, tmpToName)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oOrg = Nothing
      If (InStr(tmpTo,"@") <> 0) Then
         tmpSubject = "SUPPORT TICKET #" + CStr(reqI) + " " + tmpSubmittedBy
...
xmlParam = xmlParam + " path=" + Chr(34) + CleanXML(reqPath) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "9002i Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "9002i Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "9002i Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "9002i Load file (oData) failed with error code " + CStr(oData.parseError)
