...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0802URL", reqReturnURL
   SetCache "0802DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0802")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      ...
      If (xmlError = "") And (reqPopup = 0) Then
         reqReturnURL = GetCache("0802URL")
         reqReturnData = GetCache("0802DATA")
         SetCache "0802URL", ""
         SetCache "0802DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "0802 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0802 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0802 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0802 Load file (oData) failed with error code " + CStr(oData.parseError)
