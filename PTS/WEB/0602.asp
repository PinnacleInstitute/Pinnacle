...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0602URL", reqReturnURL
   SetCache "0602DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0602")
...
Sub LoadAffiliate()
   ...
   Else
      With oAffiliate
         ...
         .LeadCampaigns = Request.Form.Item("LeadCampaigns")
   ...
   Case CLng(cActionAdd):
      ...
      If (xmlError = "") Then
         ...
         If (reqProspectID <> 0) Then
            ...
            Else
               With oProspect
                  .SysCurrentLanguage = reqSysLanguage
                  .Load reqProspectID, CLng(reqSysUserID)
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  tmpAffiliateName = .ProspectName
         ...
         Else
            With oAffiliate
               ...
               If (reqProspectID <> 0) Then
                  .Status = 2
                  .AffiliateName = tmpAffiliateName
               ...
               End If
               If (xmlError = "") Then
                  reqAffiliateID = CLng(.Add(CLng(reqSysUserID)))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               End If
      ...
      If (xmlError = "") And (tmpAuthUser > 0) Then
         ...
         If (reqSysUserGroup = 61) Then
            Response.Redirect "0105.asp" & "?AuthUserID=" & tmpAuthUser & "&MailReturnURL=" & "0630.asp?AffiliateID=" & reqAffiliateID
         End If

         If (reqSysUserGroup <> 61) Then
            Response.Redirect "0105.asp" & "?AuthUserID=" & tmpAuthUser & "&MailReturnURL=" & Replace(GetCache("0602URL"), "%26", "&")
         End If
      End If

      If (xmlError = "") Then
         reqReturnURL = GetCache("0602URL")
         reqReturnData = GetCache("0602DATA")
         SetCache "0602URL", ""
         SetCache "0602DATA", ""
...
fileLanguage = "Language" + "\Affiliate[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\Affiliate[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0602 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0602 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0602 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0602 Load file (oData) failed with error code " + CStr(oData.parseError)
