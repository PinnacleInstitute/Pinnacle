...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0453URL", reqReturnURL
   SetCache "0453DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0453")
...
Sub AddMember()
   ...
   Else
      With oMember
         ...
         End If
         .Price = 69.95
         .PaidDate = DATEADD("m", 12, reqSysDate)
         .StatusDate = .PaidDate
         .StatusChange = 5
         .LevelChange = 1
...
fileLanguage = "Language" + "\0453[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\0453[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0453 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0453 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0453 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0453 Load file (oData) failed with error code " + CStr(oData.parseError)
