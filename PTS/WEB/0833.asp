...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0833URL", reqReturnURL
   SetCache "0833DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0833")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      ...
      If (reqTxn = 0) And (IsNumeric(reqTransferAmount) = 0) Then
         ...
         NewTransfer
      End If
      If (reqTxn = 0) And (reqOverride = 0) Then
      ...
      If (reqTxn = 0) Then
         ...
         Else
            With oPayout
               ...
               .Reference = CSTR(reqMemberID) + "-" + CSTR(reqSysUserID)
      ...
      If (xmlError = "") And (reqTxn = 0) Then
         reqReturnURL = GetCache("0833URL")
         reqReturnData = GetCache("0833DATA")
         SetCache "0833URL", ""
         SetCache "0833DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "0833 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0833 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0833 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0833 Load file (oData) failed with error code " + CStr(oData.parseError)
