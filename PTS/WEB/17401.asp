...
Dim fileLanguage
'-----object variables
Dim oBarterCredits, xmlBarterCredits
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "17401URL", reqReturnURL
   SetCache "17401DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "17401")
...
If (reqSysUserGroup > 23) And (reqSysUserGroup <> 51) Then
   If (reqOwnerType = 04) Then
      reqUser2FA = Is2FA(reqSysUserID)
   End If
   If (reqOwnerType = 151) Then
      reqUser2FA = Is2FAConsumer(reqOwnerID)
   End If
End If
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Set oBarterCredits = server.CreateObject("ptsBarterCreditUser.CBarterCredits")
      If oBarterCredits Is Nothing Then
         DoError Err.Number, Err.Source, "Unable to Create Object - ptsBarterCreditUser.CBarterCredits"
      Else
         With oBarterCredits
            .SysCurrentLanguage = reqSysLanguage
            .FindTypeID = reqFindTypeID
            xmlBarterCredits = .XML(14)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oBarterCredits = Nothing
...
Else
   With oBarterCredits
      .SysCurrentLanguage = reqSysLanguage
      reqBookmark = .Find(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqOwnerType), CLng(reqOwnerID), CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      xmlBarterCredits = .XML(15)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
End If
Set oBarterCredits = Nothing
...
xmlTransaction = "<TXN>"
xmlTransaction = xmlTransaction +  xmlBarterCredits
...
If oLanguage.parseError <> 0 Then
   Response.Write "17401 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "17401 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "17401 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "17401 Load file (oData) failed with error code " + CStr(oData.parseError)
