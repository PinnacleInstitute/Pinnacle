...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_2201URL", reqReturnURL
   SetCache "m_2201DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_2201")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("m_2201URL")
      reqReturnData = GetCache("m_2201DATA")
      SetCache "m_2201URL", ""
      SetCache "m_2201DATA", ""
...
Else
   With oLeads
      ...
      End If
      If (reqStatus = -1) Then
         reqBookmark = .FindStatus(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), 0, CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      If (reqStatus = -2) Then
         reqBookmark = .FindActive(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      If (reqStatus = -3) Then
         reqBookmark = .FindLive(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      If (reqStatus > 0) Then
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_2201 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_2201 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_2201 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_2201 Load file (oData) failed with error code " + CStr(oData.parseError)
