...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8512URL", reqReturnURL
   SetCache "8512DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8512")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oMessages
            .SysCurrentLanguage = reqSysLanguage
            .ListThreadTree CLng(reqThreadID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlMessages = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
...
If oLanguage.parseError <> 0 Then
   Response.Write "8512 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8512 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8512 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8512 Load file (oData) failed with error code " + CStr(oData.parseError)
