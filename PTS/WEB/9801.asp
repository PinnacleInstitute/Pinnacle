...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "9801URL", reqReturnURL
   SetCache "9801DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "9801")
...
reqShowSummary =  Numeric(GetInput("ShowSummary", reqPageData))
...
Else
   With oNewss
      ...
      End If
      If (reqNewsTopicID <> 0) Then
         reqBookmark = .FindTopic(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqCompanyID), CLng(reqNewsTopicID), CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      xmlNewss = .XML(15)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
xmlParam = xmlParam + " showsummary=" + Chr(34) + CStr(reqShowSummary) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "9801 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "9801 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "9801 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "9801 Load file (oData) failed with error code " + CStr(oData.parseError)
