...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "9303URL", reqReturnURL
   SetCache "9303DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "9303")
...
Sub LoadPage()
   ...
   Else
      With oPage
         ...
         .Load CLng(reqPageID), CLng(reqSysUserID)
...
Sub UpdatePage()
   ...
   Else
      With oPage
         ...
         .PageName = Request.Form.Item("PageName")
         .Subject = Request.Form.Item("Subject")
         .Status = Request.Form.Item("Status")
         .Language = Request.Form.Item("Language")
         .Fields = Request.Form.Item("Fields")
         .Form = Request.Form.Item("Form")
         .IsPrivate = Request.Form.Item("IsPrivate")
         .Fields = Request.Form.Item("Fields")
         .Save CLng(reqSysUserID)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         If (xmlError = "") And (.Status = 1) And (.PageType = 7) Then
            Result = CLng(.UpdateFT(CLng(.PageID), .PageName, .Fields))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (xmlError <> "") Then
            xmlPage = .XML(2)
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("9303URL")
         reqReturnData = GetCache("9303DATA")
         SetCache "9303URL", ""
         SetCache "9303DATA", ""
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oPage
            .SysCurrentLanguage = reqSysLanguage
            .Delete CLng(reqPageID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oPage = Nothing
   ...
   Case CLng(cActionUploadImage):
      ...
      If (xmlError = "") Then
         Response.Redirect "9321.asp" & "?PageID=" & reqPageID
      End If
End Select
...
If oLanguage.parseError <> 0 Then
   Response.Write "9303 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "9303 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "9303 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "9303 Load file (oData) failed with error code " + CStr(oData.parseError)
