...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_0408URL", reqReturnURL
   SetCache "m_0408DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_0408")
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oMembers
            ...
            .ListReferral CLng(reqReferralID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlMembers = .XML(13)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            reqTotal = CLng(.Count(CLng(reqSysUserID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_0408 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_0408 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_0408 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_0408 Load file (oData) failed with error code " + CStr(oData.parseError)
