...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "2902URL", reqReturnURL
   SetCache "2902DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "2902")
...
Sub LoadList()
   ...
   If (reqTrialDays <> 0) Then
      Set oHTMLFile = server.CreateObject("wtHTMLFile.CHTMLFile")
      If oHTMLFile Is Nothing Then
         DoError Err.Number, Err.Source, "Unable to Create Object - wtHTMLFile.CHTMLFile"
      Else
         With oHTMLFile
            .Filename = "trialbilling.htm"
...
Function GoNext(txn)
   ...
   If (reqNextURL = "") Then
      reqReturnURL = GetCache("2902URL")
      reqReturnData = GetCache("2902DATA")
      SetCache "2902URL", ""
      SetCache "2902DATA", ""
         ...
         Else
            With oBilling
               ...
               If (xmlError = "") Then
                  ...
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               End If
               If (xmlError = "") And (reqCompanyID = 21) And (tmpPayType = 1) Then
                  .Token = SetVault( reqCompanyID, tmpBillingID )
               End If
            End With
         End If
         Set oBilling = Nothing
...
If oLanguage.parseError <> 0 Then
   Response.Write "2902 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "2902 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "2902 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "2902 Load file (oData) failed with error code " + CStr(oData.parseError)
