...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_8161URL", reqReturnURL
   SetCache "m_8161DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_8161")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("m_8161URL")
      reqReturnData = GetCache("m_8161DATA")
      SetCache "m_8161URL", ""
      SetCache "m_8161DATA", ""
...
Else
   With oProspects
      ...
      If (reqStatus = -21) Then
         reqBookmark = .FindLeadStatus(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), 0, CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_8161 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_8161 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_8161 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_8161 Load file (oData) failed with error code " + CStr(oData.parseError)
