...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_NewShopperURL", reqReturnURL
   SetCache "m_NewShopperDATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_newshopper")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("m_NewShopperURL")
      reqReturnData = GetCache("m_NewShopperDATA")
      SetCache "m_NewShopperURL", ""
      SetCache "m_NewShopperDATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_NewShopper Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_NewShopper Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
xslPage = "m_NewShopper.xsl"
Set oStyle = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oStyle.load server.MapPath(xslPage)
If oStyle.parseError <> 0 Then
   Response.Write "m_NewShopper Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_NewShopper Load file (oData) failed with error code " + CStr(oData.parseError)
