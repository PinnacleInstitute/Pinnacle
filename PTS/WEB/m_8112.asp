...
Dim reqLeadPageID
Dim reqProspectNameFirst
Dim reqProspectNameLast
Dim reqProspectEmail
Dim reqSubject
Dim reqTemplateID
Dim reqEntity
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_8112URL", reqReturnURL
   SetCache "m_8112DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_8112")
...
reqProspectNameFirst =  GetInput("ProspectNameFirst", reqPageData)
reqProspectNameLast =  GetInput("ProspectNameLast", reqPageData)
reqProspectEmail =  GetInput("ProspectEmail", reqPageData)
...
Sub LoadList()
   ...
   Else
      With oLeadCampaigns
         .SysCurrentLanguage = reqSysLanguage
         xmlLeadCampaigns = .EnumPresent(CLng(reqCompanyID), tmpGroupID, tmpGroupID1, tmpGroupID2, tmpGroupID3, CLng(reqMemberID), CLng(reqLeadCampaignID), , CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
   ...
   Else
      With oProspect
         ...
         tmpZip = .Zip
         tmpCountry = .Country
         reqProspectNameFirst = .NameFirst
         reqProspectNameLast = .NameLast
         reqProspectEmail = .Email
         tmpPhone = .Phone1
         If (.NextEvent <> 0) Then
            tmpEvent = .NextDate + " " + .NextTime
         End If
         
                  If reqProspectNameFirst = "" Then
                  tmpName = .ProspectName
                  pos = InStrRev(tmpName, " ")
                  If pos > 0 Then
                  reqProspectNameFirst = Left(tmpName, pos-1)
                  reqProspectNameLast = Mid(tmpName, pos+1)
                  End If
                  End If
   ...
   End If
               ...
               xmlHTMLFile = Replace( xmlHTMLFile, "{firstname}", CleanXMLComment(reqProspectNameFirst) )
               xmlHTMLFile = Replace( xmlHTMLFile, "{lastname}", CleanXMLComment(reqProspectNameLast) )
               xmlHTMLFile = Replace( xmlHTMLFile, "{email}", CleanXMLComment(reqProspectEmail) )
...
Sub EmailPresentation()
   On Error Resume Next
   If (reqLeadCampaignID = 0) Then
      DoError -2147220523, "", "Oops, Please select a Presentation Website to email to the prospect."
   End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionEmail):
      ...
      reqReturnURL = GetCache("m_8112URL")
      reqReturnData = GetCache("m_8112DATA")
      SetCache "m_8112URL", ""
      SetCache "m_8112DATA", ""
...
xmlParam = xmlParam + " prospectnamefirst=" + Chr(34) + CleanXML(reqProspectNameFirst) + Chr(34)
xmlParam = xmlParam + " prospectnamelast=" + Chr(34) + CleanXML(reqProspectNameLast) + Chr(34)
xmlParam = xmlParam + " prospectemail=" + Chr(34) + CleanXML(reqProspectEmail) + Chr(34)
...
fileLanguage = "Language" + "\m_8112[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\m_8112[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "m_8112 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_8112 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_8112 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_8112 Load file (oData) failed with error code " + CStr(oData.parseError)
