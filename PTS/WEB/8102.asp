...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8102URL", reqReturnURL
   SetCache "8102DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8102")
...
Sub LoadLists()
   ...
   Else
      With oProspectTypes
         .SysCurrentLanguage = reqSysLanguage
         xmlProspectTypes = xmlProspectTypes + .EnumCompany(CLng(reqCompanyID), tmpProspectTypeID, , CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
      ...
      With oSalesCampaigns
         ...
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               ...
               For Each oSalesCampaign in oSalesCampaigns
                  ...
                  End With 
               Next
               If cnt = 1 Then tmpSalesCampaignID = tmp
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      ...
      If (xmlError = "") Then
         Response.Redirect "8103.asp" & "?ProspectID=" & reqProspectID & "&ReturnURL=" & GetCache("8102URL")
      End If

   Case CLng(cActionCancel):

      reqReturnURL = GetCache("8102URL")
      reqReturnData = GetCache("8102DATA")
      SetCache "8102URL", ""
      SetCache "8102DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "8102 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8102 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8102 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8102 Load file (oData) failed with error code " + CStr(oData.parseError)
