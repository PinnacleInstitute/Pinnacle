...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "3411URL", reqReturnURL
   SetCache "3411DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "3411")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If (reqRemoveID <> 0) Then
         ...
         If (xmlError = "") Then
            Response.Redirect "3411.asp" & "?MemberID=" & reqMemberID & "&CompanyID=" & reqCompanyID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup & "&RemoveID=" & 0 & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
         End If
      End If
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("3411URL")
      reqReturnData = GetCache("3411DATA")
      SetCache "3411URL", ""
      SetCache "3411DATA", ""
   ...
   Case CLng(cActionNewAssess):
      ...
      Response.Redirect "3112.asp" & "?MemberID=" & reqMemberID & "&CompanyID=" & reqCompanyID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup & "&ReturnURL=" & reqPageURL
End Select
...
If oLanguage.parseError <> 0 Then
   Response.Write "3411 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "3411 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "3411 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "3411 Load file (oData) failed with error code " + CStr(oData.parseError)
