...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8017URL", reqReturnURL
   SetCache "8017DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8017")
...
Sub LoadDocuments()
   ...
   Else
      With oAttachments
         ...
         .ListAttachments CLng(reqParentID), CLng(reqParentType), CLng(reqSysUserID)
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("8017URL")
      reqReturnData = GetCache("8017DATA")
      SetCache "8017URL", ""
      SetCache "8017DATA", ""
   ...
   Case CLng(cActionAttachment):

      Response.Redirect "8002.asp" & "?ParentType=" & reqParentType & "&ParentID=" & reqParentID & "&ContentPage=" & 1 & "&Mini=" & 1 & "&ReturnURL=" & reqPageURL
End Select
...
If oLanguage.parseError <> 0 Then
   Response.Write "8017 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8017 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8017 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8017 Load file (oData) failed with error code " + CStr(oData.parseError)
