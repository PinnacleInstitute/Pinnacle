...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_3105URL", reqReturnURL
   SetCache "m_3105DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_3105")
...
Sub Register()
   ...
   Else
      With oMemberAssess
         ...
         reqMemberAssessID = CLng(.CheckAssessment(CLng(reqAssessmentID), CLng(reqMemberID), CLng(reqSysUserID)))
   ...
   Response.Redirect "m_3105.asp" & "?AssessmentID=" & reqAssessmentID & "&MemberAssessID=" & reqMemberAssessID & "&MemberID=" & reqMemberID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
End Sub
      ...
      Else
         With oHTMLFile
            ...
            If (Len(.Data) < 20) Then
               ...
               Response.Redirect "m_3212.asp" & "?AssessmentID=" & reqAssessmentID & "&MemberAssessID=" & reqMemberAssessID & "&MemberID=" & reqMemberID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
            End If
            If (xmlError = "") Then
               xmlHTMLFile = .XML()
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      If (reqRegister <> 0) And (reqMemberAssessID = 0) And (reqMemberID <> 0) And (reqAssessmentID <> 0) Then
         Register
      End If
      LoadAssessment

   Case CLng(cActionContinue):

      Response.Redirect "m_3212.asp" & "?AssessmentID=" & reqAssessmentID & "&MemberAssessID=" & reqMemberAssessID & "&MemberID=" & reqMemberID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData

   Case CLng(cActionCancel):

      reqReturnURL = GetCache("m_3105URL")
      reqReturnData = GetCache("m_3105DATA")
      SetCache "m_3105URL", ""
      SetCache "m_3105DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_3105 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_3105 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_3105 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_3105 Load file (oData) failed with error code " + CStr(oData.parseError)
