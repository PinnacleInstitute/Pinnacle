...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "3413URL", reqReturnURL
   SetCache "3413DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "3413")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If (tmpAssessmentType = 1) Then
         ...
         Response.Redirect "3105.asp" & "?AssessmentID=" & reqAssessmentID & "&MemberAssessID=" & reqMemberAssessID & "&MemberID=" & reqMemberID & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
      End If
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("3413URL")
      reqReturnData = GetCache("3413DATA")
      SetCache "3413URL", ""
      SetCache "3413DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "3413 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "3413 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "3413 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "3413 Load file (oData) failed with error code " + CStr(oData.parseError)
