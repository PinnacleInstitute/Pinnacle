...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1712URL", reqReturnURL
   SetCache "1712DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1712")
...
Sub InitQuestions()
   ...
   Else
      With oQuestionTypes
         ...
         xmlQuestionTypes = xmlQuestionTypes + .EnumOnly(CLng(reqCompanyID), 1, tmpSecurityLevel, reqTopic, , CLng(reqSysUserID))
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionList):
      ...
      Else
         With oQuestions
            .SysCurrentLanguage = reqSysLanguage
            .List reqTopic, 0, CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlQuestions = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
...
If oLanguage.parseError <> 0 Then
   Response.Write "1712 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1712 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1712 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1712 Load file (oData) failed with error code " + CStr(oData.parseError)
