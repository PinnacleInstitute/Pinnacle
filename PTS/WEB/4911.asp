...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "4911URL", reqReturnURL
   SetCache "4911DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "4911")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("4911URL")
         reqReturnData = GetCache("4911DATA")
         SetCache "4911URL", ""
         SetCache "4911DATA", ""
...
fileLanguage = "Language" + "\4911[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\4911[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "4911 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "4911 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "4911 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "4911 Load file (oData) failed with error code " + CStr(oData.parseError)
