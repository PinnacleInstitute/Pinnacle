...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "3403URL", reqReturnURL
   SetCache "3403DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "3403")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      Else
         With oMemberAssess
            ...
            .ExternalID = Request.Form.Item("ExternalID")
            .Result = Request.Form.Item("Result")
      ...
      If (xmlError = "") Then
         Response.Redirect "3411.asp" & "?MemberID=" & reqMemberID & "&ReturnURL=" & reqPageURL
      End If
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oMemberAssess
            .SysCurrentLanguage = reqSysLanguage
            .Delete CLng(reqMemberAssessID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oMemberAssess = Nothing
   ...
   Case CLng(cActionCalc):
      ...
      Else
         With oMemberAssess
            ...
            .Save CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            Result = .CalcResult(CLng(reqMemberAssessID), "")
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            .Load CLng(reqMemberAssessID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlMemberAssess = .XML(2)
   ...
   Case CLng(cActionReset):
      ...
      Else
         With oMemberAssess
            .SysCurrentLanguage = reqSysLanguage
            Result = CLng(.Reset(CLng(reqMemberAssessID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oMemberAssess = Nothing
...
If oLanguage.parseError <> 0 Then
   Response.Write "3403 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "3403 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "3403 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "3403 Load file (oData) failed with error code " + CStr(oData.parseError)
