...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1005URL", reqReturnURL
   SetCache "1005DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1005")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If (reqOwnerType = 38) Then
         Set oCoption = server.CreateObject("ptsCoptionUser.CCoption")
         If oCoption Is Nothing Then
            DoError Err.Number, Err.Source, "Unable to Create Object - ptsCoptionUser.CCoption"
         Else
            With oCoption
               .FetchCompany CLng(reqOwnerID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               .Load .CoptionID, CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               tmpBillingID = .BillingID
      ...
      If (tmpBillingID <> 0) Then
         ...
         Else
            With oBilling
               ...
               tmpToken = .Token
               tmpBilling = BillingPayment( oBilling )
   ...
   Case CLng(cActionAdd):
      ...
      Else
         With oPayment
            ...
            .TokenType = Request.Form.Item("TokenType")
            ...
            If (xmlError = "") Then
               If (.Status = 3) Then
                  ...
                  Else
                     With oCompany
                        Count = CLng(.Custom(CLng(reqCompanyID), 99, 0, PaymentID, 0))
                        If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                     End With
                  End If
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("1005URL")
         reqReturnData = GetCache("1005DATA")
         SetCache "1005URL", ""
         SetCache "1005DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "1005 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1005 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1005 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1005 Load file (oData) failed with error code " + CStr(oData.parseError)
