...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "7004URL", reqReturnURL
   SetCache "7004DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "7004")
...
Sub UpdateGoal()
   ...
   Else
      With oGoal
         ...
         .AssignID = Request.Form.Item("AssignID")
         .Template = Request.Form.Item("Template")
         .ProspectID = Request.Form.Item("ProspectID")
         .Status = Request.Form.Item("Status")
         .GoalName = Request.Form.Item("GoalName")
         .GoalType = Request.Form.Item("GoalType")
         .Priority = Request.Form.Item("Priority")
         .Qty = Request.Form.Item("Qty")
         .ActQty = Request.Form.Item("ActQty")
         .CommitDate = Request.Form.Item("CommitDate")
         .CompleteDate = Request.Form.Item("CompleteDate")
         ...
         If (xmlError = "") Then
            If (tmpCommitDate <> "0") And (tmpCommitDate <> .CommitDate) Then
               AddNote("Projected Date Changed from " + tmpCommitDate + " to " + .CommitDate)
            End If
            If (tmpCompleteDate <> "0") And (tmpCompleteDate <> .CompleteDate) Then
               AddNote("Complete Date Changed from " + tmpCompleteDate + " to " + .CompleteDate)
            End If
            reqTxn = 1
         End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      If (xmlError = "") And (reqPopup = 0) Then
         reqReturnURL = GetCache("7004URL")
         reqReturnData = GetCache("7004DATA")
         SetCache "7004URL", ""
         SetCache "7004DATA", ""
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oGoal
            ...
            .Delete CLng(reqGoalID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (xmlError = "") Then
               reqTxn = 2
            End If
            If (xmlError = "") And (prmParentID <> 0) Then
               .CalcChildren prmParentID, CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (xmlError <> "") Then
               LoadGoal
      ...
      If (tmpTemplate = 1) Or (tmpTemplate = 2) Then
         ...
         Response.Redirect "7014.asp" & "?CompanyID=" & tmpCompanyID & "&MemberID=" & prmMemberID & "&Template=" & tmpTemplate
      End If
      If (tmpTemplate <> 1) And (tmpTemplate <> 2) Then

         If (prmParentID = 0) And (tmpCompanyID = 0) And (reqPopup = 0) Then
            Response.Redirect "7001.asp" & "?MemberID=" & prmMemberID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup
         End If

         If (prmParentID = 0) And (tmpCompanyID <> 0) And (reqPopup = 0) Then
            Response.Redirect "7011.asp" & "?CompanyID=" & tmpCompanyID
         End If

         If (prmParentID <> 0) And (reqPopup = 0) Then
            Response.Redirect "7003.asp" & "?GoalID=" & prmParentID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup & "&ReturnURL=" & reqPageURL
         End If
      End If
      If (xmlError = "") And (reqPopup = 0) Then

         reqReturnURL = GetCache("DeleteURL")
         reqReturnData = GetCache("DeleteDATA")
         SetCache "DeleteURL", ""
         SetCache "DeleteDATA", ""
         If (Len(reqReturnURL) > 0) Then
            SetCache "RETURNURL", reqReturnURL
...
If oLanguage.parseError <> 0 Then
   Response.Write "7004 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "7004 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "7004 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "7004 Load file (oData) failed with error code " + CStr(oData.parseError)
