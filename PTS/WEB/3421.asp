...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "3421URL", reqReturnURL
   SetCache "3421DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "3421")
...
reqAssessmentID =  GetInput("AssessmentID", reqPageData)
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oMemberAssesss
            If (reqReport = 1) Then
               .CertifyMember CLng(reqMemberID), reqFromDate, reqToDate
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqReport = 2) Then
               .CertifyBusiness reqMasterID, reqFromDate, reqToDate, CLng(reqAssessmentID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqReport = 3) Then
               .CertifyCompany CLng(reqCompanyID), reqFromDate, reqToDate, CLng(reqAssessmentID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqReport = 4) Then
               .CertifyCompanyGrp CLng(reqCompanyID), reqFromDate, reqToDate, CLng(reqAssessmentID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqReport > 1) Then
            ...
            End If
            xmlMemberAssesss = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oMemberAssesss = Nothing

      If (reqReport = 1) Or (reqReport = 2) Then
         ...
         Else
            With oMember
               ...
               End If
               If (reqReport = 2) Then
                  .Load reqMasterID, CLng(reqSysUserID)
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               End If
               If (reqReport = 2) Then
                  reqTitle = .CompanyName
               End If
            End With
         End If
         Set oMember = Nothing
      ...
      If (reqReport = 3) Or (reqReport = 4) Then
      ...
      If (reqReport = 2) Or (reqReport = 3) Or (reqReport = 4) Then
...
fileLanguage = "Language\3421.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "3421 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "3421 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "3421 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "3421 Load file (oData) failed with error code " + CStr(oData.parseError)
