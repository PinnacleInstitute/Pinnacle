...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1911URL", reqReturnURL
   SetCache "1911DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1911")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oQuizAnswers
            .SysCurrentLanguage = reqSysLanguage
            .ListQuizAnswer CLng(reqSessionLessonID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlQuizAnswers = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("1911URL")
      reqReturnData = GetCache("1911DATA")
      SetCache "1911URL", ""
      SetCache "1911DATA", ""
...
fileLanguage = "Language" + "\QuizAnswer[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\QuizAnswer[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "1911 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1911 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1911 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1911 Load file (oData) failed with error code " + CStr(oData.parseError)
