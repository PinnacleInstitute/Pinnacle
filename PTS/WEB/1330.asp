...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1330URL", reqReturnURL
   SetCache "1330DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1330")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If (tmpNoEvaluation = 0) And (tmpStatus > 2) And (tmpRating1 = 0) Then
         ...
         Response.Redirect "1304.asp" & "?SessionID=" & reqSessionID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup
      End If
      ...
      If (tmpLessonID <> 0) Then
         ...
         Response.Redirect "2430.asp" & "?LessonID=" & tmpLessonID & "&SessionID=" & reqSessionID & "&CourseID=" & reqCourseID & "&MemberID=" & reqMemberID & "&Lessons=" & reqLessons & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup
      End If

   Case CLng(cActionReturnClasses):

      Response.Redirect "1311.asp" & "?MemberID=" & reqMemberID

   Case CLng(cActionCancel):

      If (reqSysUserGroup = 41) And (reqSessionID = 0) Then
         Response.Redirect "0404.asp"
      End If

      reqReturnURL = GetCache("1330URL")
      reqReturnData = GetCache("1330DATA")
      SetCache "1330URL", ""
      SetCache "1330DATA", ""
   ...
   Case CLng(cActionEvaluation):
      ...
      Response.Redirect "1304.asp" & "?SessionID=" & reqSessionID
...
If oLanguage.parseError <> 0 Then
   Response.Write "1330 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1330 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1330 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1330 Load file (oData) failed with error code " + CStr(oData.parseError)
