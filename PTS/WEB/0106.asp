...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0106URL", reqReturnURL
   SetCache "0106DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0106")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      Else
         With oAuthUser
            ...
            .ConfirmPassword = Request.Form.Item("ConfirmPassword")
            newLogon = .Logon
            ...
            End If
            If (xmlError = "") Then
               .ChangePassword CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
      ...
      If (xmlError = "") Then
         reqAction = 1
         If (reqSysCompanyID = 17) And (reqSysMemberID > 0) Then
         End If
      End If

      If (xmlError = "") And (reqPopup = 0) Then
         Response.Redirect "0105.asp" & "?AuthUserID=" & reqSysUserID & "&MailReturnURL=" & GetCache("0106URL")
      End If
   ...
   Case CLng(cActionCancel):
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("0106URL")
         reqReturnData = GetCache("0106DATA")
         SetCache "0106URL", ""
         SetCache "0106DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "0106 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0106 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0106 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0106 Load file (oData) failed with error code " + CStr(oData.parseError)
