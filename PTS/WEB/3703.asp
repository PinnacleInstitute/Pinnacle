...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "3703URL", reqReturnURL
   SetCache "3703DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "3703")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      If (xmlError = "") And (reqPopup = 0) Then
         reqReturnURL = GetCache("3703URL")
         reqReturnData = GetCache("3703DATA")
         SetCache "3703URL", ""
         SetCache "3703DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "3703 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "3703 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "3703 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "3703 Load file (oData) failed with error code " + CStr(oData.parseError)
