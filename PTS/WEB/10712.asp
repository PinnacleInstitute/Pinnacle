...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "10712URL", reqReturnURL
   SetCache "10712DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "10712")
...
reqBillingName =  GetInput("BillingName", reqPageData)
...
Sub LoadMachine()
   ...
   Else
      With oMachine
         ...
         tmpEmail = .Email
         reqEmail = .Email
         reqBillingName = tmpNameFirst + " " + tmpNameLast
      End With
   End If
...
Sub UpdateMember()
   ...
   Else
      With oMember
         ...
         .NotifyMentor = "ABCDEF"
         .ReferralID = tmpMemberID
         .Level = 0
         .Title = 0
         .Status = 0
         tmpDays = DateDiff("d",tmpActiveDate,reqSysDate)
         .TrialDays = tmpDays
         ...
         If (xmlError = "") Then
            ...
            GetToken reqMemberID, reqBillingID, tmpName, .Email, reqProcessor, reqMemberToken, reqToken, reqPayDesc 
            If (xmlError = "") Then
         ...
         End If
         If (xmlError = "") Then
            GetPayment reqMemberID, .Price, reqPayType, reqPayDesc, reqProcessor, reqMemberToken, reqToken
            If (xmlError = "") Then
               .Status = 1
               .PaidDate = DATEADD("m", 1, .EnrollDate)
               .Save CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
         End If
   ...
   End If
   If (xmlError = "") Then
      LogMemberNote reqMemberID, "Customer created from their referring Affiliate's computer."
   End If
End Sub
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      reqMemberID = Request.Form.Item("MemberID")
      reqBillingID = Request.Form.Item("BillingID")
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("10712URL")
         reqReturnData = GetCache("10712DATA")
         SetCache "10712URL", ""
         SetCache "10712DATA", ""
...
xmlParam = xmlParam + " billingname=" + Chr(34) + CleanXML(reqBillingName) + Chr(34)
...
fileLanguage = "Language" + "\10712[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\10712[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "10712 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "10712 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "10712 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "10712 Load file (oData) failed with error code " + CStr(oData.parseError)
