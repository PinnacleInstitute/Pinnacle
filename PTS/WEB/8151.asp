...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8151URL", reqReturnURL
   SetCache "8151DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8151")
...
If (reqSysUserGroup = 41) And (InStr(reqSysUserOptions,"6") = 0) And (reqM = 0) Then
...
Else
   With oSalesCampaigns
      .SysCurrentLanguage = reqSysLanguage
      xmlSalesCampaigns = xmlSalesCampaigns + .EnumCompany(CLng(reqCompanyID), tmpGroupID, tmpSalesCampaignID, , CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
End If
...
xmlSalesCampaigns = Replace( xmlSalesCampaigns, "name=" + CHR(34) + " " + CHR(34), "name=" + CHR(34) + "All" + CHR(34)) 
xmlProspectTypes = Replace( xmlProspectTypes, "name=" + CHR(34) + " " + CHR(34), "name=" + CHR(34) + "All" + CHR(34)) 
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("8151URL")
      reqReturnData = GetCache("8151DATA")
      SetCache "8151URL", ""
      SetCache "8151DATA", ""
   ...
   Else
      With oProspects
         ...
         If (reqMemberID <> 0) Then
            If (reqSalesCampaignID = 0) And (reqProspectTypeID = 0) Then
               reqBookmark = .FindMemCust(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqSysUserID))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqSalesCampaignID <> 0) And (reqProspectTypeID = 0) Then
               reqBookmark = .FindMemCustCamp(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqSalesCampaignID), CLng(reqSysUserID))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqSalesCampaignID = 0) And (reqProspectTypeID <> 0) Then
               reqBookmark = .FindMemCustType(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqProspectTypeID), CLng(reqSysUserID))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqSalesCampaignID <> 0) And (reqProspectTypeID <> 0) Then
               reqBookmark = .FindMemCustCampType(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqSalesCampaignID), CLng(reqProspectTypeID), CLng(reqSysUserID))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
         End If
         If (reqMemberID = 0) Then
            If (reqSalesCampaignID = 0) And (reqProspectTypeID = 0) Then
               reqBookmark = .FindComCust(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqCompanyID), CLng(reqSysUserID))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqSalesCampaignID <> 0) And (reqProspectTypeID = 0) Then
               reqBookmark = .FindComCustCamp(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqCompanyID), CLng(reqSalesCampaignID), CLng(reqSysUserID))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqSalesCampaignID = 0) And (reqProspectTypeID <> 0) Then
               reqBookmark = .FindComCustType(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqCompanyID), CLng(reqProspectTypeID), CLng(reqSysUserID))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqSalesCampaignID <> 0) And (reqProspectTypeID <> 0) Then
               reqBookmark = .FindComCustCampType(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqCompanyID), CLng(reqSalesCampaignID), CLng(reqProspectTypeID), CLng(reqSysUserID))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
         End If
         xmlProspects = .XMLCustomer(15)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "8151 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8151 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8151 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8151 Load file (oData) failed with error code " + CStr(oData.parseError)
