...
pos = InStr(LCASE(reqSysServerPath), "nexxusproduct")
...
Sub LoadMember()
   On Error Resume Next
   reqProduct = 0
   If (InStr( "38567,37702,37703,37704,37720,37731,38886,39060,39501,39431,37801,39045,37823,37736,39229,39032 ", reqMemberID) > 0) Then
      reqProduct = 205
   End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "NexxusProduct Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
xslPage = "NexxusProduct.xsl"
Set oStyle = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oStyle.load server.MapPath(xslPage)
If oStyle.parseError <> 0 Then
   Response.Write "NexxusProduct Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "NexxusProduct Load file (oData) failed with error code " + CStr(oData.parseError)
