...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "5203URL", reqReturnURL
   SetCache "5203DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "5203")
...
'                     If pos > 0 Then .InputValues = Left(.InputValues, pos-1)
         ...
         xmlSalesItems = xmlSalesItems + .XML()
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
   ...
   If (reqProspectID = 0) Then
      ...
      Else
         With oPayments
            ...
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlPayments = xmlPayments + .XML(13)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      Else
         With oSalesOrder
            ...
            .PartyID = Request.Form.Item("PartyID")
            .OrderDate = Request.Form.Item("OrderDate")
            .Amount = Request.Form.Item("Amount")
            .Discount = Request.Form.Item("Discount")
            .Tax = Request.Form.Item("Tax")
            .Shipping = Request.Form.Item("Shipping")
            .Total = Request.Form.Item("Total")
            .Status = Request.Form.Item("Status")
            .AffiliateID = Request.Form.Item("AffiliateID")
            .PromotionID = Request.Form.Item("PromotionID")
            .IsTaxable = Request.Form.Item("IsTaxable")
            .IsRecur = Request.Form.Item("IsRecur")
            .AutoShip = Request.Form.Item("AutoShip")
            .IsActive = Request.Form.Item("IsActive")
            .Notes = Request.Form.Item("Notes")
            .Ship = Request.Form.Item("Ship")
            .Track = Request.Form.Item("Track")
            .PinnDate = Request.Form.Item("PinnDate")
            .PinnAmount = Request.Form.Item("PinnAmount")
            .CommDate = Request.Form.Item("CommDate")
            .CommAmount = Request.Form.Item("CommAmount")
      ...
      If (xmlError = "") And (reqPopup = 0) Then
         reqReturnURL = GetCache("5203URL")
         reqReturnData = GetCache("5203DATA")
         SetCache "5203URL", ""
         SetCache "5203DATA", ""
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oSalesOrder
            .SysCurrentLanguage = reqSysLanguage
            .Delete CLng(reqSalesOrderID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oSalesOrder = Nothing
...
If oLanguage.parseError <> 0 Then
   Response.Write "5203 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "5203 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "5203 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "5203 Load file (oData) failed with error code " + CStr(oData.parseError)
