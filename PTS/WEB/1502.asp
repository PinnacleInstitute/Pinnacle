...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1502URL", reqReturnURL
   SetCache "1502DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1502")
   ...
   Case CLng(cActionAdd):
      ...
      Else
         With oLeadPage
            ...
            .IsCapture = 1
            .IsProspect = 1
            .Inputs = "*First Name:type=text" + VBCR + "*Last Name:type=text" + VBCR + "*Email:type=text" + VBCR + "Phone:type=text"

            .LeadPageName = Request.Form.Item("LeadPageName")
            .Seq = Request.Form.Item("Seq")
            .Language = Request.Form.Item("Language")
            LeadPageID = CLng(.Add(CLng(reqSysUserID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (xmlError <> "") Then
               xmlLeadPage = .XML(2)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("1502URL")
         reqReturnData = GetCache("1502DATA")
         SetCache "1502URL", ""
         SetCache "1502DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "1502 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1502 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1502 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1502 Load file (oData) failed with error code " + CStr(oData.parseError)
