         ...
         <xsl:element name="TABLE">
            ...
            <xsl:element name="TR">
               <xsl:element name="TD">
                  ...
                  <xsl:if test="(/DATA/PARAM/@notify = 8) or (/DATA/PARAM/@notify = 9)">
                        <xsl:if test="(/DATA/PARAM/@notify = 8)">
                           <xsl:value-of select="/DATA/LANGUAGE/LABEL[@name='Activity']"/>
                           <xsl:text>:</xsl:text>
                           <xsl:text disable-output-escaping="yes">&amp;#160;&amp;#160;</xsl:text>
                        </xsl:if>
                        <xsl:if test="(/DATA/PARAM/@notify = 9)">
                           <xsl:value-of select="/DATA/LANGUAGE/LABEL[@name='Result']"/>
                           <xsl:text>:</xsl:text>
                           <xsl:text disable-output-escaping="yes">&amp;#160;&amp;#160;</xsl:text>
                        </xsl:if>
                        <xsl:if test="(/DATA/TXN/PTSMETRIC/@qty != 1)">
                           <xsl:value-of select="/DATA/TXN/PTSMETRIC/@qty"/>
                        </xsl:if>
                        <xsl:value-of select="/DATA/TXN/PTSMETRIC/@metrictypename"/>
                        <xsl:text disable-output-escaping="yes">&amp;#160;&amp;#160;</xsl:text>
                        <xsl:value-of select="/DATA/TXN/PTSMETRIC/@note"/>
                     <xsl:if test="(/DATA/TXN/PTSMETRIC/@inputvalues != '')">
                           <xsl:element name="BR"/>
                           InputValues)
                     </xsl:if>
                  </xsl:if>
                  ...
                  <xsl:if test="(/DATA/PARAM/@notify != 7)">
                     ...
                     <xsl:element name="BR"/>
                     <xsl:if test="(/DATA/PARAM/@ip != '')">
                        <xsl:value-of select="/DATA/LANGUAGE/LABEL[@name='IP']"/>
                        <xsl:text>:</xsl:text>
                        <xsl:text disable-output-escaping="yes">&amp;#160;&amp;#160;</xsl:text>
                        <xsl:value-of select="/DATA/PARAM/@ip" disable-output-escaping="yes"/>
                        <xsl:element name="BR"/>
                     </xsl:if>
               </xsl:element>
            </xsl:element>
