...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8704URL", reqReturnURL
   SetCache "8704DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8704")
   ...
   Case CLng(cActionNew):
      ...
      If (reqSource = 1) Then
         ...
         Else
            With oEmailSource
               ...
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               tmpSourceFields = .EmailSourceFields
               tmpSourceFrom = .EmailSourceFrom
            End With
         End If
      ...
      Else
         With oEmailees
            .SysCurrentLanguage = reqSysLanguage
            If (reqSource = 0) Then
               .ListStandard CLng(reqEmailListID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqSource = 1) Then
               .ListUserDefined tmpQuery, tmpSourceFields, tmpSourceFrom
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqSource = 2) Then
               .ListCustom tmpCompanyID, tmpCustomID, tmpParam1, tmpParam2, tmpParam3, tmpParam4, tmpParam5
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            xmlEmailees = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
xmlTransaction = xmlTransaction +  xmlEmailees
...
If oLanguage.parseError <> 0 Then
   Response.Write "8704 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8704 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8704 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8704 Load file (oData) failed with error code " + CStr(oData.parseError)
