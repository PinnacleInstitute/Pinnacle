...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0424URL", reqReturnURL
   SetCache "0424DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0424")
...
reqNewMentor =  GetInput("NewMentor", reqPageData)
...
Sub LoadMentor()
   ...
   Else
      With oHTMLFile
         .Filename = "mentor.htm"
         .Path = reqSysWebDirectory + "Sections\Company/" & reqSysCompanyID 
...
Sub LoadNewMentor()
   ...
   Else
      With oMember
         ...
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         reqNewMentor = .NameFirst + " " + .NameLast
      End With
   End If
   Set oMember = Nothing
End Sub
...
xmlParam = xmlParam + " newmentor=" + Chr(34) + CleanXML(reqNewMentor) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "0424 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0424 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0424 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0424 Load file (oData) failed with error code " + CStr(oData.parseError)
