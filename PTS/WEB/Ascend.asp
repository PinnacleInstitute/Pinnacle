...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "AscendURL", reqReturnURL
   SetCache "AscendDATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "ascend")
...
reqSignIn =  Numeric(GetInput("SignIn", reqPageData))
reqJoinNow =  Numeric(GetInput("JoinNow", reqPageData))
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      reqSignIn = GetCache("COMPANYSIGNIN")
      reqJoinNow = GetCache("COMPANYJOINNOW")
...
xmlParam = xmlParam + " signin=" + Chr(34) + CStr(reqSignIn) + Chr(34)
xmlParam = xmlParam + " joinnow=" + Chr(34) + CStr(reqJoinNow) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "Ascend Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "Ascend Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "Ascend Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "Ascend Load file (oData) failed with error code " + CStr(oData.parseError)
