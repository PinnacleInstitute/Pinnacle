...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "2111URL", reqReturnURL
   SetCache "2111DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "2111")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("2111URL")
      reqReturnData = GetCache("2111DATA")
      SetCache "2111URL", ""
      SetCache "2111DATA", ""
   ...
   Case CLng(cActionQuizIntro):

      If (xmlError = "") Then
         Response.Redirect "2113.asp" & "?LessonID=" & reqLessonID & "&CourseID=" & reqCourseID & "&ReturnURL=" & GetCache("2111URL")
      End If
End Select
...
If oLanguage.parseError <> 0 Then
   Response.Write "2111 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "2111 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "2111 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "2111 Load file (oData) failed with error code " + CStr(oData.parseError)
