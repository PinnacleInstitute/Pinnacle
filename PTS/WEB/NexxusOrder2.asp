...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "NexxusOrder2URL", reqReturnURL
   SetCache "NexxusOrder2DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "nexxusorder2")
...
reqPackage =  Numeric(GetInput("Package", reqPageData))
reqAutoOrder =  Numeric(GetInput("AutoOrder", reqPageData))
...
Sub AddOrder()
   On Error Resume Next
   GetWallet
   reqPackage = Request.Form.Item("Package")
   BillingPrice = 0
   If (reqPackage = 101) Or (reqPackage = 106) Then
      BillingPrice = 10
   End If
   If (reqPackage = 102) Or (reqPackage = 107) Then
      BillingPrice = 25
   End If
   If (reqPackage = 103) Or (reqPackage = 108) Then
      BillingPrice = 50
   End If
   If (BillingPrice <> 0) And (CCUR(reqWalletNumber) >= CCUR(BillingPrice)) Then
         ...
         Else
            With oPayout
              ...
              tmpLocation = GetIPCity( IP )
               ...
               .Notes = Left("Affiliate Pack " + IP + " " + tmpLocation, 500) 
   ...
   If (BillingPrice <> 0) And (CCUR(reqWalletNumber) < CCUR(BillingPrice)) Then
      GetBilling
      tmpBillingPrice = BillingPrice
      If (reqPayType <= 4) Then
         If (BillingPrice = 10) Then
            tmpBillingPrice = 11
         End If
         If (BillingPrice = 25) Then
            tmpBillingPrice = 27
         End If
         If (BillingPrice = 50) Then
            tmpBillingPrice = 53
         End If
      End If
      ...
      If (xmlError = "") And (reqBillingID <> 0) Then
         ...
         Result = GetPayment( reqCompanyID, reqMemberID, 0, reqPayType, reqPayDesc, tmpBillingPrice, reqPackage, tmpProcessor, tmpAcct, PostProcess, EmailBonuses )
   ...
   End If
   If (xmlError = "") Then
      If (reqAutoOrder <> 0) Or (BillingPrice = 0) Then
         ...
         Else
            With oMember
               ...
               .Price = BillingPrice
               Options2 = .Options2
               Options2 = Replace( Options2, "101", "")
               Options2 = Replace( Options2, "102", "")
               Options2 = Replace( Options2, "103", "")
               ...
               If (BillingPrice <> 0) Then
                  .Options2 = Options2 + "," + CSTR(reqPackage)
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("NexxusOrder2URL")
      reqReturnData = GetCache("NexxusOrder2DATA")
      SetCache "NexxusOrder2URL", ""
      SetCache "NexxusOrder2DATA", ""
...
xmlParam = xmlParam + " package=" + Chr(34) + CStr(reqPackage) + Chr(34)
xmlParam = xmlParam + " autoorder=" + Chr(34) + CStr(reqAutoOrder) + Chr(34)
...
fileLanguage = "Language" + "\NexxusOrder2[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\NexxusOrder2[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "NexxusOrder2 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "NexxusOrder2 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
xslPage = "NexxusOrder2.xsl"
Set oStyle = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oStyle.load server.MapPath(xslPage)
If oStyle.parseError <> 0 Then
   Response.Write "NexxusOrder2 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "NexxusOrder2 Load file (oData) failed with error code " + CStr(oData.parseError)
