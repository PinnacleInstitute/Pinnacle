...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "15001URL", reqReturnURL
   SetCache "15001DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "15001")
...
If (reqSysUserGroup > 23) And (reqSysUserGroup <> 51) And (reqSysUserGroup <> 52) And (reqSysUserGroup <> 41) Then
   AbortUser()
End If
If (reqSysUserGroup = 41) Then
   If (reqMemberID = 0) Or (reqMemberID <> reqSysMemberID) Then
      AbortUser()
   End If
End If
If (reqMemberID <> 0) And (reqTitle = "") Then
...
xmlTransaction = xmlTransaction +  xmlMerchants
...
If oLanguage.parseError <> 0 Then
   Response.Write "15001 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "15001 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "15001 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "15001 Load file (oData) failed with error code " + CStr(oData.parseError)
