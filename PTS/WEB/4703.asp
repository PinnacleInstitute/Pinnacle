...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "4703URL", reqReturnURL
   SetCache "4703DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "4703")
...
reqCalendarID =  Numeric(GetInput("CalendarID", reqPageData))
...
Sub LoadCalendar()
   ...
   Else
      With oCalendar
         ...
         .Load CLng(reqCalendarID), CLng(reqSysUserID)
...
Sub UpdateCalendar()
   ...
   Else
      With oCalendar
         ...
         .CalendarName = Request.Form.Item("CalendarName")
         .Seq = Request.Form.Item("Seq")
         .Timezone = Request.Form.Item("Timezone")
         .IsPrivate = Request.Form.Item("IsPrivate")
         .Description = Request.Form.Item("Description")
         .IsAppt = Request.Form.Item("IsAppt")
         .IsGoal = Request.Form.Item("IsGoal")
         .IsLead = Request.Form.Item("IsLead")
         .IsSales = Request.Form.Item("IsSales")
         .IsActivities = Request.Form.Item("IsActivities")
         .IsService = Request.Form.Item("IsService")
         .IsEvents = Request.Form.Item("IsEvents")
         .IsClass = Request.Form.Item("IsClass")
         .IsAssess = Request.Form.Item("IsAssess")
         .IsProject = Request.Form.Item("IsProject")
         .IsTask = Request.Form.Item("IsTask")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oCalendar
            .SysCurrentLanguage = reqSysLanguage
            .Delete CLng(reqCalendarID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oCalendar = Nothing
...
xmlConfig = xmlConfig + " />"
xmlParam = "<PARAM"
xmlParam = xmlParam + " calendarid=" + Chr(34) + CStr(reqCalendarID) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "4703 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "4703 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "4703 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "4703 Load file (oData) failed with error code " + CStr(oData.parseError)
