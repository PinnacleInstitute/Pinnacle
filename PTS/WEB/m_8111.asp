...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_8111URL", reqReturnURL
   SetCache "m_8111DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_8111")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      ...
      If (xmlError <> "") Then
         LoadList
      End If
      If (xmlError = "") And (reqProspectID <> 0) Then

         If (reqEntity = 81) Then
            Response.Redirect "m_8103.asp" & "?ProspectID=" & reqProspectID & "&CompanyID=" & reqCompanyID & "&MemberID=" & reqMemberID
         End If

         If (reqEntity = 22) Then
            Response.Redirect "m_8163.asp" & "?ProspectID=" & reqProspectID & "&CompanyID=" & reqCompanyID & "&MemberID=" & reqMemberID
         End If
      End If

   Case CLng(cActionCancel):

      reqReturnURL = GetCache("m_8111URL")
      reqReturnData = GetCache("m_8111DATA")
      SetCache "m_8111URL", ""
      SetCache "m_8111DATA", ""
...
fileLanguage = "Language" + "\m_8111[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\m_8111[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "m_8111 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_8111 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_8111 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_8111 Load file (oData) failed with error code " + CStr(oData.parseError)
