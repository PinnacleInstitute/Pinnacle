...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8153URL", reqReturnURL
   SetCache "8153DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8153")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdateExit):
      ...
      If (xmlError = "") And (reqPopup = 0) Then
         reqReturnURL = GetCache("8153URL")
         reqReturnData = GetCache("8153DATA")
         SetCache "8153URL", ""
         SetCache "8153DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "8153 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8153 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
s=s+   "<ITEM label=""ViewContactInfo"" width=""80"">"
s=s+      "<LINK name=""JAVA(ShowTab('TabContactInfo',0))""/>"
s=s+   "</ITEM>"
s=s+   "<ITEM label=""ViewDescription"" width=""80"">"
s=s+      "<LINK name=""JAVA(ShowTab('TabDescription',1))""/>"
s=s+   "</ITEM>"
s=s+   "<ITEM label=""Notes"" width=""80"">"
s=s+      "<LINK name=""JAVA(ShowTab('TabNotes',0))""/>"
s=s+   "</ITEM>"
s=s+   "<ITEM label=""Goals"" width=""80"">"
s=s+      "<LINK name=""JAVA(ShowTab('TabGoals',0))""/>"
s=s+   "</ITEM>"
s=s+   "<ITEM label=""Events"" width=""80"">"
s=s+      "<LINK name=""JAVA(ShowTab('TabEvents',0))""/>"
s=s+   "</ITEM>"
s=s+   "<ITEM label=""Mail"" width=""80"">"
s=s+      "<LINK name=""JAVA(ShowTab('TabMail',0))""/>"
s=s+   "</ITEM>"
s=s+   "<ITEM label=""Documents"" width=""80"">"
s=s+      "<LINK name=""JAVA(ShowTab('TabDocuments',0))""/>"
s=s+   "</ITEM>"
s=s+"</TAB>"
xmlProspectTab = s
...
If oStyle.parseError <> 0 Then
   Response.Write "8153 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8153 Load file (oData) failed with error code " + CStr(oData.parseError)
