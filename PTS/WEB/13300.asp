...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "13300URL", reqReturnURL
   SetCache "13300DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "13300")
   ...
   Case CLng(cActionUpdate):
      ...
      If (xmlError = "") Then
         ...
         If (reqProcess = 2) Then
            ...
            Set oGftg = server.CreateObject("ptsGftgUser.CGftg")
            If oGftg Is Nothing Then
               DoError Err.Number, Err.Source, "Unable to Create Object - ptsGftgUser.CGftg"
            Else
               With oGftg
                  .SysCurrentLanguage = reqSysLanguage
                  Count = CLng(.Custom(200, reqToDate, 0, 0))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  reqResult = Count & " New Autoship Payments Successfully Created!"
               End With
            End If
            Set oGftg = Nothing
         End If
         ...
         If (reqProcess = 24) Then
            ...
            End If
            ExportWallet reqCompanyID, 11, reqToDate, -1, tmpTotal, tmpFile, tmpAmount
            Total = Total + tmpTotal
            If (tmpTotal <> 0) Then
               URL = "http://www.pinnaclep.com/payout/" + CStr(reqCompanyID) + "/" + tmpFile
               LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
               ...
               NOTE = NOTE + MSG + URL + "  " + VBLF
            End If
            ExportWallet reqCompanyID, 13, reqToDate, -1, tmpTotal, tmpFile, tmpAmount
            Total = Total + tmpTotal
            If (tmpTotal <> 0) Then
               URL = "http://www.pinnaclep.com/payout/" + CStr(reqCompanyID) + "/" + tmpFile
               LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
               MSG = CSTR(tmpTotal) + " Solid Trust eWallets (" & FormatCurrency(tmpAmount,2) & ")  "
               reqResult = reqResult + MSG + LINK + "<br>"
               NOTE = NOTE + MSG + URL + "  " + VBLF
            End If
            If (Total = 0) Then
         ...
         If (reqProcess = 30) Then
            ...
            Set oGftgs = server.CreateObject("ptsGftgUser.CGftgs")
            If oGftgs Is Nothing Then
               DoError Err.Number, Err.Source, "Unable to Create Object - ptsGftgUser.CGftgs"
            Else
               With oGftgs
                  .SysCurrentLanguage = reqSysLanguage
                  .CustomList 3, 0, 0, 0
         ...
         If (reqProcess = 35) Then
            ...
            If (tmpTotal <> 0) Then
               URL = "http://www.pinnaclep.com/orders/" + CStr(reqCompanyID) + "/" + tmpFile
               LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
               MSG = CSTR(tmpTotal) + " Polydot Submitted Orders (" & FormatCurrency(tmpAmount,2) & ")  "
               reqResult = reqResult + MSG + LINK + "<br>"
               NOTE = NOTE + MSG + URL + "  " + VBLF
            End If
            If (Total <> 0) Then
...
xmlTransaction = xmlTransaction +  xmlGftgs
...
fileLanguage = "Language" + "\13300[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\13300[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "13300 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "13300 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "13300 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "13300 Load file (oData) failed with error code " + CStr(oData.parseError)
