...
<!--#include file="Include\R180Ads.asp"-->
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "13200URL", reqReturnURL
   SetCache "13200DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "13200")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      If (reqSysUserGroup = 21) Then
         If InStr( ",50,51,", CSTR(reqProcess)+"," ) = 0 Then reqProcess = 0
      End If
   ...
   Case CLng(cActionUpdate):
      If (reqProcess = 12) Or (reqProcess = 39) Or (reqProcess = 47) Or (reqProcess = 50) Or (reqProcess = 51) Then
      ...
      If (xmlError = "") Then
         ...
         If (reqProcess = 2) Then
            ...
            Set oR180 = server.CreateObject("ptsR180User.CR180")
            If oR180 Is Nothing Then
               DoError Err.Number, Err.Source, "Unable to Create Object - ptsR180User.CR180"
            Else
               With oR180
                  .SysCurrentLanguage = reqSysLanguage
                  Count = CLng(.Custom(200, CDate(reqToDate), 0, 0))
         ...
         If (reqProcess = 11) Then
            ...
            Else
               With oR180
                  .SysCurrentLanguage = reqSysLanguage
                  Count = CLng(.Custom(3, CDate(reqToDate), 0, 0))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         ...
         If (reqProcess = 12) Then
            ...
            Else
               With oCommission
                  .SysCurrentLanguage = reqSysLanguage
                  Count = CLng(.CustomCommissions(reqCompanyID, 1, CDate(reqFromDate), CDate(reqToDate)))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         ...
         If (reqProcess = 21) Then
            ...
            Else
               With oPayout
                  .SysCurrentLanguage = reqSysLanguage
                  Count = CLng(.CompanyPayouts(reqCompanyID, CDate(reqToDate)))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         ...
         If (reqProcess = 22) Then
            ...
            Else
               With oCompany
                  .SysCurrentLanguage = reqSysLanguage
                  Count = CLng(.Custom(reqCompanyID, 4, CDate(reqToDate), 0, 0))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         ...
         If (reqProcess = 23) Then
            ...
            Else
               With oR180
                  .SysCurrentLanguage = reqSysLanguage
                  Count = CLng(.Custom(6, CDate(reqToDate), 0, 0))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         ...
         If (reqProcess = 24) Then
            ...
            If (tmpTotal <> 0) Then
               URL = "http://www.right180.com/payout/" + CStr(reqCompanyID) + "/" + tmpFile
         ...
         If (reqProcess = 25) Then
            ...
            Else
               With oPayout
                  .SysCurrentLanguage = reqSysLanguage
                  Count = CLng(.CompanyStatus2(reqCompanyID, CDate(reqToDate), CDate(reqSysDate), 0, "3"))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         ...
         If (reqProcess = 28) Then
            ...
            Else
               With oR180
                  .SysCurrentLanguage = reqSysLanguage
                  Count = CLng(.Custom(2, CDate(reqToDate), 0, 0))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         ...
         If (reqProcess = 35) Then
            ...
            Total = Total + tmpTotal
            If (tmpTotal <> 0) Then
               URL = "http://www.right180.com/orders/" + CStr(reqCompanyID) + "/" + tmpFile
               LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
            ...
            If (tmpTotal <> 0) Then
               URL = "http://www.right180.com/orders/" + CStr(reqCompanyID) + "/" + tmpFile
               LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
               MSG = CSTR(tmpTotal) + " R180 Submitted Orders (" & FormatCurrency(tmpAmount,2) & ")  "
               reqResult = reqResult + MSG + LINK + "<br>"
               NOTE = NOTE + MSG + URL + "  " + VBLF
            End If
            If (Total <> 0) Then
            ...
            End If
         End If
         If (reqProcess = 36) Then
            Total = 0
            ExportOrder reqCompanyID, 102, 1, tmpFile, tmpTotal, tmpAmount
            Total = Total + tmpTotal
            If (tmpTotal <> 0) Then
               URL = "http://www.right180.com/orders/" + CStr(reqCompanyID) + "/" + tmpFile
               LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
               MSG = CSTR(tmpTotal) + " GFT Inprocess Orders (" & FormatCurrency(tmpAmount,2) & ")  "
               reqResult = reqResult + MSG + LINK + "<br>"
               NOTE = NOTE + MSG + URL + "  " + VBLF
            End If
            ExportOrder reqCompanyID, 102, 2, tmpFile, tmpTotal, tmpAmount
            Total = Total + tmpTotal
            If (tmpTotal <> 0) Then
               URL = "http://www.right180.com/orders/" + CStr(reqCompanyID) + "/" + tmpFile
               LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
               MSG = CSTR(tmpTotal) + " R180 Inprocess Orders (" & FormatCurrency(tmpAmount,2) & ")  "
               reqResult = reqResult + MSG + LINK + "<br>"
               NOTE = NOTE + MSG + URL + "  " + VBLF
            End If
            If (Total <> 0) Then
         ...
         If (reqProcess = 37) Then
            ...
            Else
               With oSalesOrder
                  .SysCurrentLanguage = reqSysLanguage
                  .Custom reqCompanyID, 101, 0
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  reqResult = "Order Status Updated!"
               End With
            End If
            Set oSalesOrder = Nothing
         End If
         If (reqProcess = 38) Then
      ...
      If (reqProcess = 48) Then
         ...
         Else
            With oCompany
               .SysCurrentLanguage = reqSysLanguage
               Count = CLng(.Custom(reqCompanyID, 207, CDate(reqToDate), 0, 0))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      ...
      If (reqProcess = 49) Then
         ...
         Else
            With oCompany
               .SysCurrentLanguage = reqSysLanguage
               Count = CLng(.Custom(reqCompanyID, 208, CDate(reqToDate), 0, 0))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      ...
      If (reqProcess = 50) Then
         Total = 0
         AdTracks 1, reqFromDate, reqToDate, tmpFile, tmpTotal
         Total = Total + tmpTotal
         If (tmpTotal <> 0) Then
            URL = "http://www.right180.com/ads/" + tmpFile
            LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
            MSG = CSTR(tmpTotal) + " Member Ads "
            reqResult = reqResult + MSG + LINK + "<br>"
            NOTE = NOTE + MSG + URL + "  " + VBLF
         End If
         If (Total <> 0) Then
      ...
      If (reqProcess = 51) Then
         Total = 0
         AdTracks 2, reqFromDate, reqToDate, tmpFile, tmpTotal
         Total = Total + tmpTotal
         If (tmpTotal <> 0) Then
            URL = "http://www.right180.com/ads/" + tmpFile
            LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
            MSG = CSTR(tmpTotal) + " Advertiser Ads "
            reqResult = reqResult + MSG + LINK + "<br>"
            NOTE = NOTE + MSG + URL + "  " + VBLF
         End If
         If (Total <> 0) Then
...
xmlTransaction = xmlTransaction +  xmlR180s
...
fileLanguage = "Language" + "\13200[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\13200[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "13200 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "13200 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "13200 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "13200 Load file (oData) failed with error code " + CStr(oData.parseError)
