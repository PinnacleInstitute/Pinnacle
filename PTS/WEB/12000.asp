...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "12000URL", reqReturnURL
   SetCache "12000DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "12000")
      ...
      If (xmlError = "") Then
         ...
         If (reqProcess = 22) Then
            ...
            Else
               With oCompany
                  .SysCurrentLanguage = reqSysLanguage
                  Count = CLng(.Custom(reqCompanyID, 4, 0, 0, 0))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         ...
         If (reqProcess = 24) Then
            ...
            End If
            ExportCheck reqCompanyID, reqToDate, -1, tmpTotal, tmpFile, tmpAmount
            Total = Total + tmpTotal
            If (tmpTotal <> 0) Then
               URL = "http://www.pinnaclep.com/payout/" + CStr(reqCompanyID) + "/" + tmpFile
               LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
         ...
         If (reqProcess = 25) Then
            ...
            Else
               With oPayout
                  .SysCurrentLanguage = reqSysLanguage
                  Count = CLng(.CompanyStatus2(reqCompanyID, reqToDate, CDate(reqSysDate), -1, "23"))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         ...
         If (reqProcess = 41) Then
            PaymentOwnerType = 52
            ExportPayments reqCompanyID, PaymentOwnerType, reqSysDate, CCTotal, CKTotal, CCFile, CKFile, ErrorMsg
            If (ErrorMsg <> "") Then
...
fileLanguage = "Language" + "\12000[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\12000[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "12000 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "12000 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "12000 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "12000 Load file (oData) failed with error code " + CStr(oData.parseError)
