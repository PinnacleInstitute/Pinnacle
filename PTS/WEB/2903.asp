...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "2903URL", reqReturnURL
   SetCache "2903DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "2903")
...
If (reqSysUserGroup = 41) And (reqMemberID <> reqSysMemberID) Then
   LogFile "2903", "ILLEGAL ACCESS by Member #" + CStr(reqSysMemberID) + " of #" + CStr(reqMemberID)

   Response.Redirect "0101.asp" & "?ActionCode=" & 9
End If
If (reqSysUserGroup > 52) And (reqV = 0) Then
   LogFile "2903", "ILLEGAL ACCESS by Anonymous of " + CStr(reqMemberID)
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      Else
         With oBilling
            ...
            If (xmlError = "") Then
               ...
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (xmlError = "") And (tmpPayType = 1) Then
               If (reqCompanyID = 21) Or (tmpToken <> 0) Then
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("2903URL")
         reqReturnData = GetCache("2903DATA")
         SetCache "2903URL", ""
         SetCache "2903DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "2903 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "2903 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "2903 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "2903 Load file (oData) failed with error code " + CStr(oData.parseError)
