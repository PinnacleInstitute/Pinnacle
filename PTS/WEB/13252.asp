...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "13252URL", reqReturnURL
   SetCache "13252DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "13252")
...
reqTopicTitle =  GetInput("TopicTitle", reqPageData)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oNewsTopic
            ...
            .Load reqT, CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            reqTopicTitle = UCASE(.NewsTopicName)
         End With
      End If
      ...
      Else
         With oNewss
            .SysCurrentLanguage = reqSysLanguage
            .ListNews reqCompanyID, 4, reqT
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlNewss = .XML(13)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            .ListNews reqCompanyID, 5, reqT
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      ...
      Else
         With oAds
            .SysCurrentLanguage = reqSysLanguage
            .ListAds reqCompanyID, 2, reqT, CLng(reqSysUserGroup), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlAds = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
...
xmlParam = xmlParam + " topictitle=" + Chr(34) + CleanXML(reqTopicTitle) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "13252 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "13252 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
s=s+   "</ITEM>"
      End If
      If (reqSysUserGroup = 41) Then
s=s+   "<ITEM label=""mMyFriends"" width=""100"">"
...
If oStyle.parseError <> 0 Then
   Response.Write "13252 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "13252 Load file (oData) failed with error code " + CStr(oData.parseError)
