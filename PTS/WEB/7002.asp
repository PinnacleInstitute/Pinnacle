...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "7002URL", reqReturnURL
   SetCache "7002DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "7002")
...
reqTemplate =  Numeric(GetInput("Template", reqPageData))
   ...
   Case CLng(cActionAdd):
      ...
      Else
         With oGoal
            ...
            .GoalID = Request.Form.Item("GoalID")
            ...
            GoalID = CLng(.Add(CLng(reqSysUserID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (xmlError = "") And (reqParentID <> 0) Then
               .CalcChildren CLng(reqParentID), CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (xmlError <> "") Then
               xmlGoal = .XML(2)
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("7002URL")
         reqReturnData = GetCache("7002DATA")
         SetCache "7002URL", ""
         SetCache "7002DATA", ""
   ...
   Case CLng(cActionCopy):
      ...
      If (tmpGoalID <> 0) Then
         ...
         Else
            With oGoal
               .SysCurrentLanguage = reqSysLanguage
               CopyGoalID = CLng(.Copy(tmpGoalID, CLng(reqMemberID), CLng(reqProspectID), "", CLng(reqSysUserID)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
         Set oGoal = Nothing
...
xmlParam = xmlParam + " template=" + Chr(34) + CStr(reqTemplate) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "7002 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "7002 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "7002 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "7002 Load file (oData) failed with error code " + CStr(oData.parseError)
