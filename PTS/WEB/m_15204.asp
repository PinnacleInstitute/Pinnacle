...
Const cActionNew = 0
Const cActionConsumer = 1
Const cActionAdd = 2
Const cActionCancel = 3
Const cActionNotConsumer = 4
'-----page variables
Dim oData
Dim oStyle
'-----system variables
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_15204URL", reqReturnURL
   SetCache "m_15204DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_15204")
...
reqPurchase =  GetInput("Purchase", reqPageData)
reqTotalCashBack =  GetInput("TotalCashBack", reqPageData)
reqTotalAwards =  GetInput("TotalAwards", reqPageData)
reqRewardType =  Numeric(GetInput("RewardType", reqPageData))
...
Sub GetConsumerName()
   ...
   Else
      With oConsumer
         ...
         Result = CLng(.Logon(reqLogonConsumer, reqPassword))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         If (Result > 0) Then
            reqConsumerID = Result
            .Load CLng(reqConsumerID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            reqConsumerName = .NameFirst + " " + .NameLast
         End If
         ...
         If (Result = -1000004) Then
            DoError 10130, "", "Oops, The Password is incorrect."
         End If
      End With
   End If
...
Sub NewReward()
   ...
   Else
      With oReward
         .SysCurrentLanguage = reqSysLanguage
         reqTotalCashBack = CCur(.TotalReward(CLng(reqConsumerID), 0))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         If (reqIsAwards <> 0) Then
            reqTotalAwards = CCur(.TotalReward(CLng(reqConsumerID), CLng(reqMerchantID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
      End With
   End If
...
Sub AddReward()
   ...
   Else
      With oReward
         ...
         .Status = 1
         .RewardType = reqRewardType

         .RewardDate = Request.Form.Item("RewardDate")
         .Reference = Request.Form.Item("Reference")
         .Note = Request.Form.Item("Note")
         tmpAmount = Round(reqPurchase * -1,2)
         .Amount = toSatoshi(tmpAmount)
         RewardID = CLng(.Add(CLng(reqSysUserID)))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         If (xmlError = "") Then
            reqReward = FormatCurrency(tmpAmount)
         End If
         If (xmlError <> "") Then
            xmlReward = .XML(2)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionConsumer):
      GetConsumerName
      If (reqConsumerName <> "") Then
         NewReward
      End If
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("m_15204URL")
      reqReturnData = GetCache("m_15204DATA")
      SetCache "m_15204URL", ""
      SetCache "m_15204DATA", ""
...
xmlParam = xmlParam + " purchase=" + Chr(34) + CleanXML(reqPurchase) + Chr(34)
xmlParam = xmlParam + " totalcashback=" + Chr(34) + CStr(reqTotalCashBack) + Chr(34)
xmlParam = xmlParam + " totalawards=" + Chr(34) + CStr(reqTotalAwards) + Chr(34)
xmlParam = xmlParam + " rewardtype=" + Chr(34) + CStr(reqRewardType) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_15204 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_15204 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_15204 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_15204 Load file (oData) failed with error code " + CStr(oData.parseError)
