...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "14701URL", reqReturnURL
   SetCache "14701DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "14701")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Set oAdTracks = server.CreateObject("ptsAdTrackUser.CAdTracks")
      If oAdTracks Is Nothing Then
         DoError Err.Number, Err.Source, "Unable to Create Object - ptsAdTrackUser.CAdTracks"
      Else
         With oAdTracks
...
Else
   With oAdTracks
      ...
      reqBookmark = .Find(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CDate(reqFromDate), tmpToDate, CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      xmlAdTracks = .XML(15)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
...
xmlTransaction = "<TXN>"
xmlTransaction = xmlTransaction +  xmlAdTracks
...
fileLanguage = "Language" + "\AdTrack[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\AdTrack[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "14701 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "14701 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "14701 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "14701 Load file (oData) failed with error code " + CStr(oData.parseError)
