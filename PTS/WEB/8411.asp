...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8411URL", reqReturnURL
   SetCache "8411DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8411")
...
reqForumName =  GetInput("ForumName", reqPageData)
...
Sub LoadThreads()
   ...
   Else
      With oMessages
         ...
         .ListForum CLng(reqForumID), reqPage, reqBookmark, CLng(reqSysUserID)
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If (reqTotalPages = 0) And (reqForumID > 0) Then
         ...
         Else
            With oMessage
               .SysCurrentLanguage = reqSysLanguage
               reqTotalPages = CLng(.GetForumPages(CLng(reqForumID), CLng(reqSysUserID)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
xmlParam = xmlParam + " bookmark=" + Chr(34) + CStr(reqBookmark) + Chr(34)
xmlParam = xmlParam + " totalpages=" + Chr(34) + CStr(reqTotalPages) + Chr(34)
xmlParam = xmlParam + " forumname=" + Chr(34) + CleanXML(reqForumName) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "8411 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8411 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8411 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8411 Load file (oData) failed with error code " + CStr(oData.parseError)
