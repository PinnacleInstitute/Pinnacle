...
Const cActionNew = 0
Const cActionCountry = 1
Const cActionBarterArea = 2
Const cActionBack = 3
'-----page variables
Dim oData
Dim oStyle
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "17010URL", reqReturnURL
   SetCache "17010DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "17010")
...
reqBack =  Numeric(GetInput("Back", reqPageData))
reqURL =  GetInput("URL", reqPageData)
reqArea =  Numeric(GetInput("Area", reqPageData))
...
Sub LoadBarterAreas()
   ...
   If (reqBarterAreaID <> 0) Then
      ...
      Else
         With oBarterArea
            ...
            .Load CLng(reqBarterAreaID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (reqBack <> 0) Then
               reqBarterAreaID = .ParentID
               reqTitle = ""
               If (reqBarterAreaID <> 0) Then
                  .Load CLng(reqBarterAreaID), CLng(reqSysUserID)
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            ...
            If (reqBarterAreaID <> 0) Then
               ...
               reqTitle = .BarterAreaName
               BarterAreaType = .BarterAreaType
            End If
            xmlBarterArea = .XML(2)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
   ...
   Else
      With oBarterAreas
         .SysCurrentLanguage = reqSysLanguage
         .ListActive CLng(reqBarterAreaID), CLng(reqCountryID)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   ...
   Else
      With oCountrys
         .SysCurrentLanguage = reqSysLanguage
         xmlCountrys = xmlCountrys + .EnumCompany(21, , , CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      If (reqCountryID = 0) Then
         reqCountryID = Numeric(GetCache("BARTERCOUNTRY"))
      End If
      If (reqCountryID = 0) Then
         reqCountryID = 224
      End If
   ...
   Case CLng(cActionCountry):
      reqCountryID = Request.Form.Item("CountryID")
      If (reqCountryID = 0) Then
         reqCountryID = 224
      End If
      SetCache "BARTERCOUNTRY", reqCountryID
      reqBarterAreaID = 0
      LoadBarterAreas

   Case CLng(cActionBarterArea):
      reqBarterAreaID = Request.Form.Item("BarterAreaID")
      LoadBarterAreas

   Case CLng(cActionBack):
      reqBarterAreaID = Request.Form.Item("BarterAreaID")
      reqBack = 1
      LoadBarterAreas
End Select
...
xmlParam = xmlParam + " back=" + Chr(34) + CStr(reqBack) + Chr(34)
xmlParam = xmlParam + " url=" + Chr(34) + CleanXML(reqURL) + Chr(34)
xmlParam = xmlParam + " area=" + Chr(34) + CStr(reqArea) + Chr(34)
xmlParam = xmlParam + " />"
...
fileLanguage = "Language" + "\BarterArea[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\BarterArea[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "17010 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "17010 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "17010 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "17010 Load file (oData) failed with error code " + CStr(oData.parseError)
