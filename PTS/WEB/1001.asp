...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1001URL", reqReturnURL
   SetCache "1001DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1001")
   ...
   Else
      With oPayments
         If (reqCompanyID = 0) Then
            If (reqStatus = 0) Then
               reqBookmark = .FindOwner(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqOwnerType), CLng(reqOwnerID), CLng(reqSysUserID))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqStatus <> 0) Then
               reqBookmark = .FindOwnerStatus(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqOwnerType), CLng(reqOwnerID), CLng(reqStatus), CLng(reqSysUserID))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
         End If
         If (reqCompanyID <> 0) Then
...
If oLanguage.parseError <> 0 Then
   Response.Write "1001 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1001 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1001 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1001 Load file (oData) failed with error code " + CStr(oData.parseError)
