...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "17003URL", reqReturnURL
   SetCache "17003DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "17003")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oBarterArea
            ...
            If (reqBarterAreaID = 0) Then
               ...
               .Status = 2
               If (reqParentType = 0) Then
                  .BarterAreaType = 1
               End If
               If (reqParentType = 1) Then
                  .BarterAreaType = 2
               End If
               If (reqParentType = 2) Then
                  .BarterAreaType = 3
               End If
            End If
            xmlBarterArea = .XML(2)
      ...
      If (tmpParentID <> 0) Then
         ...
         Else
            With oBarterArea
               .SysCurrentLanguage = reqSysLanguage
               .Load tmpParentID, CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               reqParentType = .BarterAreaType
            End With
         End If
   ...
   Case CLng(cActionUpdate):
      ...
      Else
         With oBarterArea
            ...
            .BarterAreaName = Request.Form.Item("BarterAreaName")
            .BarterAreaType = Request.Form.Item("BarterAreaType")
            .Save CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (xmlError = "") And (.ParentID <> 0) Then
               Status = CLng(.UpdateChildren(CLng(.ParentID)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (xmlError <> "") Then
               xmlBarterArea = .XML(2)
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("17003URL")
         reqReturnData = GetCache("17003DATA")
         SetCache "17003URL", ""
         SetCache "17003DATA", ""
   ...
   Case CLng(cActionAdd):
      ...
      Else
         With oBarterArea
            ...
            .BarterAreaName = Request.Form.Item("BarterAreaName")
            .BarterAreaType = Request.Form.Item("BarterAreaType")
            BarterAreaID = CLng(.Add(CLng(reqSysUserID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (xmlError = "") And (.ParentID <> 0) Then
               Status = CLng(.UpdateChildren(CLng(.ParentID)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oBarterArea
            .SysCurrentLanguage = reqSysLanguage
            .Delete CLng(reqBarterAreaID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oBarterArea = Nothing
...
If oLanguage.parseError <> 0 Then
   Response.Write "17003 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "17003 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "17003 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "17003 Load file (oData) failed with error code " + CStr(oData.parseError)
