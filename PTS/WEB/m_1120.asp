...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_1120URL", reqReturnURL
   SetCache "m_1120DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_1120")
...
reqOrgCourseID =  Numeric(GetInput("OrgCourseID", reqPageData))
...
Sub Register()
   ...
   Else
      With oSession
         ...
         SessionID = CLng(.Add(CLng(reqSysUserID)))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         If (xmlError <> "") Then
            xmlSession = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   ...
   Response.Redirect "m_1121.asp" & "?CourseID=" & reqCourseID & "&MemberID=" & reqMemberID & "&OrgCourseID=" & reqOrgCourseID & "&ReturnURL=" & GetCache("m_1120URL")
End Sub
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If (reqSysUserGroup > 23) And (reqMemberID > 0) Then
         ...
         Else
            With oSession
               .SysCurrentLanguage = reqSysLanguage
               reqSessionID = CLng(.CheckCourse(CLng(reqCourseID), reqMemberID, CLng(reqSysUserID)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
         Set oSession = Nothing
      End If
      If (reqSessionID = 0) And (reqRegister > 0) Then
         Register
      End If
   ...
   Case CLng(cActionEnroll):
      Register

   Case CLng(cActionCancel):

      reqReturnURL = GetCache("m_1120URL")
      reqReturnData = GetCache("m_1120DATA")
      SetCache "m_1120URL", ""
      SetCache "m_1120DATA", ""
...
xmlParam = xmlParam + " orgcourseid=" + Chr(34) + CStr(reqOrgCourseID) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_1120 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_1120 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_1120 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_1120 Load file (oData) failed with error code " + CStr(oData.parseError)
