...
Const cActionBilling = 3
Const cActionAddBilling = 2
'-----page variables
Dim oData
Dim oStyle
'-----system variables
...
Dim oSalesOrders, xmlSalesOrders
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0433URL", reqReturnURL
   SetCache "0433DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0433")
...
reqBillTo =  GetInput("BillTo", reqPageData)
...
Sub LoadMember()
   ...
   Else
      With oSalesOrders
         .SysCurrentLanguage = reqSysLanguage
         .ListAutoShip CLng(reqMemberID)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         xmlSalesOrders = .XML(13)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
...
If (reqActiveID <> 0) Then
   ...
   Else
      With oSalesOrder
         .SysCurrentLanguage = reqSysLanguage
         .SetAutoShip CLng(reqMemberID), reqActiveID
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
   Set oSalesOrder = Nothing

   Response.Redirect "0433.asp" & "?MemberID=" & reqMemberID
End If
   ...
   Case CLng(cActionAddBilling):
      ...
      If (xmlError = "") Then
         If reqSysServerName <> "localhost" Then
            tmpRedirect = "https://" + reqSysServerName + reqSysServerPath
         End If
         Response.Redirect tmpRedirect + "2902.asp" & "?MemberID=" & reqMemberID & "&CompanyID=" & reqCompanyID & "&AutoShip=" & 1 & "&ContentPage=" & 3 & "&ReturnURL=" & reqPageURL
      End If
End Select
...
xmlParam = xmlParam + " billto=" + Chr(34) + CleanXML(reqBillTo) + Chr(34)
xmlParam = xmlParam + " />"
...
fileLanguage = "Language" + "\0433[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\0433[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0433 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0433 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0433 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0433 Load file (oData) failed with error code " + CStr(oData.parseError)
