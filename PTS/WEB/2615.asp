...
pos = InStr(LCASE(reqSysServerPath), "2615")
...
reqAll =  Numeric(GetInput("All", reqPageData))
...
If oStyle.parseError <> 0 Then
   Response.Write "2615 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oParty
            ...
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            tmpFrom = .Email
            tmpSubject = "Invitation from " + .HostName
         End With
      End If
      Set oParty = Nothing
...
fileLanguage = "Language" + "\2615[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\2615[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "2615 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "2615 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
xmlParam = xmlParam + " all=" + Chr(34) + CStr(reqAll) + Chr(34)
xmlParam = xmlParam + " />"
...
If oData.parseError <> 0 Then
   Response.Write "2615 Load file (oData) failed with error code " + CStr(oData.parseError)
...
End If
      ...
      Else
         With oGuest
            .SysCurrentLanguage = reqSysLanguage
            .UpdateEmailStatus CLng(reqPartyID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oGuest = Nothing
