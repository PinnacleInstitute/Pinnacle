...
pos = InStr(LCASE(reqSysServerPath), "0437")
...
If oStyle.parseError <> 0 Then
   Response.Write "0437 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oMember
            ...
            If (reqType = 0) Then
               tmpSponsorID = .ReferralID
            End If
            If (reqType = 1) Then
               tmpSponsorID = .SponsorID
            End If
            If (reqType = 2) Then
               tmpSponsorID = .Sponsor2ID
            End If
            reqNewMemberName = .NameFirst + " " + .NameLast
            If (reqCustomer = 0) Then
               tmpMember = "Member"
            End If
            If (reqCustomer <> 0) Then
               If (.Status = 1) Then
      ...
      If (reqLeaders <> 0) Then
         ...
         Else
            With oCloudZow
               .SysCurrentLanguage = reqSysLanguage
               tmpCC = .Custom(104, 0, tmpSponsorID, 0)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
      ...
      End If
      tmpSubject = "New " + tmpMember + ": " + reqNewMemberName
...
fileLanguage = "Language" + "\0437[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\0437[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0437 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0437 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
xmlParam = xmlParam + " msg=" + Chr(34) + CStr(reqMsg) + Chr(34)
xmlParam = xmlParam + " sponsorname=" + Chr(34) + CleanXML(reqSponsorName) + Chr(34)
xmlParam = xmlParam + " newmembername=" + Chr(34) + CleanXML(reqNewMemberName) + Chr(34)
...
xmlParam = xmlParam + " leaders=" + Chr(34) + CStr(reqLeaders) + Chr(34)
xmlParam = xmlParam + " />"
...
If oData.parseError <> 0 Then
   Response.Write "0437 Load file (oData) failed with error code " + CStr(oData.parseError)
