...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "15100URL", reqReturnURL
   SetCache "15100DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "15100")
...
If (reqSysUserGroup > 23) And (reqSysUserGroup <> 51) Then
   ...
   If (reqSysUserGroup = 41) And (tmpConsumerID = 0) Then
      ...
      Else
         With oMember
            ...
            tmpConsumerID = .SponsorID
         End With
      End If
      Set oMember = Nothing
      reqUser2FA = Is2FAConsumer(reqConsumerID)
      If (reqSysUserGroup = 41) And (reqUser2FA = 1) Then
         Member2FA = Is2FA(reqSysUserID)
         If (Member2FA = 0) Then
            tmpConsumerID = 0
         End If
      End If
   End If
   If (reqConsumerID <> CLng(tmpConsumerID)) Then
      SetCache "CONSUMER", ""
...
fileLanguage = "Language" + "\15100[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\15100[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "15100 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "15100 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
s=s+   "<ITEM label=""mDashboard"" width=""75"" menuwidth=""200"" height=""30"" menuheight=""30"">"
...
s=s+      "</ITEM>"
s=s+   "</ITEM>"
s=s+   "<ITEM label=""mShopping"" width=""105"" menuwidth=""125"" height=""30"" menuheight=""30"">"
...
s=s+      "</ITEM>"
s=s+      "<DIVIDER/>"
s=s+      "<ITEM label=""mBarter"">"
s=s+         "<IMAGE name=""sales.gif"" width=""18"" height=""18""/>"
s=s+         "<LINK name=""JAVA(ShowTab('TabRewards',4))""/>"
s=s+      "</ITEM>"
s=s+   "</ITEM>"
s=s+   "<ITEM label=""mMerchants"" width=""115"" menuwidth=""125"" height=""30"" menuheight=""30"">"
...
If oStyle.parseError <> 0 Then
   Response.Write "15100 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "15100 Load file (oData) failed with error code " + CStr(oData.parseError)
