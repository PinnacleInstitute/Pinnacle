...
Const cActionImages = 4
Const cActionFiles = 5
Const cActionQuiz = 6
'-----page variables
Dim oData
Dim oStyle
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "2304URL", reqReturnURL
   SetCache "2304DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "2304")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("2304URL")
         reqReturnData = GetCache("2304DATA")
         SetCache "2304URL", ""
         SetCache "2304DATA", ""
   ...
   Case CLng(cActionImages):
      UpdateContent

      If (xmlError = "") Then
         Response.Redirect "2305.asp" & "?LessonID=" & reqLessonID & "&CourseID=" & reqCourseID & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
      End If

   Case CLng(cActionFiles):
      UpdateContent

      If (xmlError = "") Then
         Response.Redirect "2306.asp" & "?LessonID=" & reqLessonID & "&CourseID=" & reqCourseID & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
      End If
   ...
   Case CLng(cActionQuiz):
      ...
      If (xmlError = "") Then
         Response.Redirect "2111.asp" & "?LessonID=" & reqLessonID & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
      End If
End Select
...
If oLanguage.parseError <> 0 Then
   Response.Write "2304 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "2304 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "2304 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "2304 Load file (oData) failed with error code " + CStr(oData.parseError)
