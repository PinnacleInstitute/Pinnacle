...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "9310URL", reqReturnURL
   SetCache "9310DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "9310")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("9310URL")
      reqReturnData = GetCache("9310DATA")
      SetCache "9310URL", ""
      SetCache "9310DATA", ""
...
Else
   With oPages
      .SysCurrentLanguage = reqSysLanguage
      reqBookmark = .FindCompany(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqCompanyID), 6, CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "9310 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "9310 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "9310 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "9310 Load file (oData) failed with error code " + CStr(oData.parseError)
