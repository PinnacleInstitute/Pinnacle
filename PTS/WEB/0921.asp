...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0921URL", reqReturnURL
   SetCache "0921DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0921")
...
Else
   With oCommissions
      ...
      reqBookmark = .FindOwnerShow(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqOwnerType), CLng(reqOwnerID), CDate(reqFromDate), tmpToDate, tmpShow, CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "0921 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0921 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0921 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0921 Load file (oData) failed with error code " + CStr(oData.parseError)
