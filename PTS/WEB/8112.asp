...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8112URL", reqReturnURL
   SetCache "8112DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8112")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionEmail):
      ...
      If (reqPopup = 0) Then
         reqReturnURL = GetCache("8112URL")
         reqReturnData = GetCache("8112DATA")
         SetCache "8112URL", ""
         SetCache "8112DATA", ""
...
fileLanguage = "Language" + "\8112[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\8112[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "8112 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8112 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8112 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8112 Load file (oData) failed with error code " + CStr(oData.parseError)
