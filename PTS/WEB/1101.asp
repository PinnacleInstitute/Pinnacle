...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1101URL", reqReturnURL
   SetCache "1101DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1101")
...
reqOrgCourseAction =  Numeric(GetInput("OrgCourseAction", reqPageData))
...
If (reqOrgCourseAction > 0) Then
   ...
   Else
      With oOrgCourse
         ...
         tmpOrgCourseID = CLng(.GetOrgCourse(CLng(reqOrgID), reqID, CLng(reqSysUserID)))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         If (reqOrgCourseAction = 2) Then
            .Delete tmpOrgCourseID, CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqOrgCourseAction = 1) Then
   ...
   Response.Redirect "1101.asp" & "?SearchText=" & reqSearchText & "&FindTypeID=" & reqFindTypeID & "&Bookmark=" & reqBookmark & "&Direction=" & reqDirection & "&contentpage=" & reqcontentpage & "&CompanyID=" & reqCompanyID & "&OrgID=" & reqOrgID & "&Mode=" & reqMode & "&OrgCourseAction=" & 0 & "&ID=" & 0 & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
End If
...
xmlParam = xmlParam + " orgcourseaction=" + Chr(34) + CStr(reqOrgCourseAction) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "1101 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1101 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1101 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1101 Load file (oData) failed with error code " + CStr(oData.parseError)
