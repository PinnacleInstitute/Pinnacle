...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0103URL", reqReturnURL
   SetCache "0103DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0103")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      Else
         With oAuthUser
            ...
            .Load CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            .Logon = Request.Form.Item("Logon")
            .NewLogon = Request.Form.Item("NewLogon")
            .ConfirmLogon = Request.Form.Item("ConfirmLogon")
            newLogon = .NewLogon
      ...
      If (xmlError = "") Then
         Response.Redirect "0105.asp" & "?AuthUserID=" & reqSysUserID & "&MailReturnURL=" & GetCache("0103URL")
      End If
   ...
   Case CLng(cActionCancel):
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("0103URL")
         reqReturnData = GetCache("0103DATA")
         SetCache "0103URL", ""
         SetCache "0103DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "0103 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0103 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0103 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0103 Load file (oData) failed with error code " + CStr(oData.parseError)
