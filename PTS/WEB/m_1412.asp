...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_1412URL", reqReturnURL
   SetCache "m_1412DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_1412")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oLeadCampaigns
            .SysCurrentLanguage = reqSysLanguage
            If (reqPageType = 1) Then
               .ListMember CLng(reqCompanyID), tmpGroupID, tmpGroupID1, tmpGroupID2, tmpGroupID3, reqMemberID
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqPageType = 2) Then
               .ListPresent CLng(reqCompanyID), tmpGroupID, tmpGroupID1, tmpGroupID2, tmpGroupID3, reqMemberID
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqCompanyID = 5) And (tmpTitle = 1) Then
               
                     For Each oItem in oLeadCampaigns
                     With oItem
                     If .LeadCampaignID = 3 OR .LeadCampaignID = 7 Then .LeadCampaignName = ""
                     End With
                     Next
                  
            End If
            xmlLeadCampaigns = .XML(13)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
   ...
   Case CLng(cActionReturn):
      ...
      reqReturnURL = GetCache("m_1412URL")
      reqReturnData = GetCache("m_1412DATA")
      SetCache "m_1412URL", ""
      SetCache "m_1412DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_1412 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_1412 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_1412 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_1412 Load file (oData) failed with error code " + CStr(oData.parseError)
