...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "4501URL", reqReturnURL
   SetCache "4501DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "4501")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("4501URL")
      reqReturnData = GetCache("4501DATA")
      SetCache "4501URL", ""
      SetCache "4501DATA", ""
...
If (reqActionCode <> 0) Or (reqOrgID > 0) Or (reqCompanyID > 0) Then
   Set oSuggestions = server.CreateObject("ptsSuggestionUser.CSuggestions")
   If oSuggestions Is Nothing Then
      DoError Err.Number, Err.Source, "Unable to Create Object - ptsSuggestionUser.CSuggestions"
   Else
      With oSuggestions
         .SysCurrentLanguage = reqSysLanguage
         If (reqOrgID > 0) And (reqCompanyID = 0) And (reqStatus = 0) Then
         ...
         End If
         If (reqOrgID > 0) And (reqCompanyID = 0) And (reqStatus <> 0) Then
            reqBookmark = .FindOrgStatus(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqOrgID), CLng(reqStatus), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqOrgID = 0) And (reqCompanyID > 0) And (reqStatus <> 0) Then
...
If oLanguage.parseError <> 0 Then
   Response.Write "4501 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "4501 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "4501 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "4501 Load file (oData) failed with error code " + CStr(oData.parseError)
