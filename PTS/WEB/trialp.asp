...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "trialpURL", reqReturnURL
   SetCache "trialpDATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "trialp")
...
Sub UpdateMember()
   ...
   If (xmlError = "") Then
      ...
      Else
         With oCloudZow
            .SysCurrentLanguage = reqSysLanguage
            Count = CLng(.Custom(100, 0, reqMemberID, 0))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
   ...
   If (xmlError = "") Then
      ...
      Else
         With oNote
            ...
            .OwnerType = 4
            .OwnerID = reqMemberID
            .Notes = "Paid Customer from free trial"
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      ...
      If (xmlError = "") Then
         Response.Redirect "Trialr.asp" & "?MemberID=" & reqMemberID & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
      End If
End Select
...
fileLanguage = "Language" + "\trialp[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\trialp[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "trialp Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "trialp Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "trialp Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "trialp Load file (oData) failed with error code " + CStr(oData.parseError)
