...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "15703URL", reqReturnURL
   SetCache "15703DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "15703")
...
reqCoinID =  Numeric(GetInput("CoinID", reqPageData))
...
Sub LoadCoin()
   ...
   Else
      With oCoin
         ...
         .Load CLng(reqCoinID), CLng(reqSysUserID)
...
Sub UpdateCoin()
   ...
   Else
      With oCoin
         ...
         .CoinDate = Request.Form.Item("CoinDate")
         .Amount = Request.Form.Item("Amount")
         .Status = Request.Form.Item("Status")
         .CoinType = Request.Form.Item("CoinType")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("15703URL")
         reqReturnData = GetCache("15703DATA")
         SetCache "15703URL", ""
         SetCache "15703DATA", ""
   ...
   Case CLng(cActionDelete):
      ...
      If (reqSysUserGroup = 1) Then
         ...
         Else
            With oCoin
               .SysCurrentLanguage = reqSysLanguage
               .Delete CLng(reqCoinID), CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
         Set oCoin = Nothing
...
xmlConfig = xmlConfig + " />"
xmlParam = "<PARAM"
xmlParam = xmlParam + " coinid=" + Chr(34) + CStr(reqCoinID) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "15703 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "15703 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "15703 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "15703 Load file (oData) failed with error code " + CStr(oData.parseError)
