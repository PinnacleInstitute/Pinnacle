...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_3414URL", reqReturnURL
   SetCache "m_3414DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_3414")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oMemberAssess
            ...
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            tmpStatus = CLng(.CalcMaster(CLng(reqMemberAssessID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlMemberAssess = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
   ...
   Case CLng(cActionReturn):
      ...
      Response.Redirect "m_3411.asp" & "?MemberID=" & reqMemberID & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
End Select
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_3414 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_3414 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_3414 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_3414 Load file (oData) failed with error code " + CStr(oData.parseError)
