...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "9503URL", reqReturnURL
   SetCache "9503DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "9503")
...
reqIssueID =  Numeric(GetInput("IssueID", reqPageData))
reqCompanyID =  Numeric(GetInput("CompanyID", reqPageData))
reqUserName =  GetInput("UserName", reqPageData)
reqSubmitLink =  GetInput("SubmitLink", reqPageData)
reqOldStatus =  Numeric(GetInput("OldStatus", reqPageData))
...
Sub LoadIssue()
   ...
   Else
      With oIssue
         ...
         .Load CLng(reqIssueID), CLng(reqSysUserID)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         reqCompanyID = .CompanyID
         reqOldStatus = .Status
         reqUserName = Left(reqSysUserName,InStr(reqSysUserName," ")-1)
         If (.SubmitType = 04) Then
            reqSubmitLink = "0403.asp?memberid=" + CSTR(.SubmitID) + "&contentpage=3&popup=1"
         End If
         If (.Status < 4) Then
            .Quantity = DATEDIFF("h", .IssueDate, Now() )
         End If
         xmlIssue = .XML(2)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      tmpFrom = ""
      tmpCC = ""
      tmpOldStatus = Request.Form.Item("OldStatus")
      tmpStatus = Request.Form.Item("Status")
      tmpIssueCategoryID = Request.Form.Item("IssueCategoryID")

      If (tmpStatus <> tmpOldStatus) And (tmpStatus >= 2) Then
  ...
  If .DueDate <> "0" Then tmpSubject = tmpSubject + " (DUE: " + .DueDate + ")"
      ...
      If (xmlError = "") And (tmpSubmitType = 04) And (tmpEmail <> 0) Then
         ...
         Else
            With oMember
               .SysCurrentLanguage = reqSysLanguage
               .Load tmpSubmitID, CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               If (tmpEmail = 2) Then
                  tmpFrom = .Email
               End If
         ...
         Set oMember = Nothing
      End If
      If (xmlError = "") And (tmpEmail <> 0) Then
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("9503URL")
         reqReturnData = GetCache("9503DATA")
         SetCache "9503URL", ""
         SetCache "9503DATA", ""
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oIssue
            .SysCurrentLanguage = reqSysLanguage
            .Delete CLng(reqIssueID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oIssue = Nothing
...
xmlConfig = xmlConfig + " />"
xmlParam = "<PARAM"
xmlParam = xmlParam + " issueid=" + Chr(34) + CStr(reqIssueID) + Chr(34)
xmlParam = xmlParam + " companyid=" + Chr(34) + CStr(reqCompanyID) + Chr(34)
xmlParam = xmlParam + " username=" + Chr(34) + CleanXML(reqUserName) + Chr(34)
xmlParam = xmlParam + " submitlink=" + Chr(34) + CleanXML(reqSubmitLink) + Chr(34)
xmlParam = xmlParam + " oldstatus=" + Chr(34) + CStr(reqOldStatus) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "9503 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "9503 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
s = "<TAB name=""IssueTab"">"
s=s+   "<ITEM label=""ViewResponse"" width=""200"">"
s=s+      "<LINK name=""JAVA(ShowTab('TabResponse'))""/>"
s=s+   "</ITEM>"
s=s+   "<ITEM label=""ViewReplies"" width=""200"">"
s=s+      "<LINK name=""JAVA(ShowTab('TabReplies'))""/>"
s=s+   "</ITEM>"
s=s+"</TAB>"
xmlIssueTab = s
...
If oStyle.parseError <> 0 Then
   Response.Write "9503 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "9503 Load file (oData) failed with error code " + CStr(oData.parseError)
