...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "11770URL", reqReturnURL
   SetCache "11770DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "11770")
         ...
         With oMetrics
            ...
            If (reqRpt <> 0) Then
               If (reqPrivate = 0) Then
                  If (reqScope = 1) Then
                     .ExpLeaderCompany CLng(reqCompanyID), CDate(reqFromDate), CDate(reqToDate), CLng(reqRpt)
                     If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  End If
                  If (reqScope = 2) Then
                     .ExpLeaderGroup CLng(reqGroupID), CDate(reqFromDate), CDate(reqToDate), CLng(reqRpt)
                     If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  End If
                  If (reqScope = 3) Then
                     .ExpLeaderMentor CLng(reqSysMemberID), CDate(reqFromDate), CDate(reqToDate), CLng(reqRpt)
                     If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  End If
               End If
               If (reqPrivate <> 0) Then
                  .ExpLeaderPrivate CLng(reqContestID), CDate(reqFromDate), CDate(reqToDate), CLng(reqRpt)
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               End If
            End If
            If (reqRpt = 0) Then
               If (reqPrivate = 0) Then
                  If (reqScope = 1) Then
                     .ExpLeaderCompanyCustom CLng(reqCompanyID), CDate(reqFromDate), CDate(reqToDate), CLng(reqCustom1), CLng(reqCustom2), CLng(reqCustom3), CLng(reqCustom4), CLng(reqCustom5)
                     If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  End If
                  If (reqScope = 2) Then
                     .ExpLeaderGroupCustom CLng(reqGroupID), CDate(reqFromDate), CDate(reqToDate), CLng(reqCustom1), CLng(reqCustom2), CLng(reqCustom3), CLng(reqCustom4), CLng(reqCustom5)
                     If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  End If
                  If (reqScope = 3) Then
                     .ExpLeaderMentorCustom CLng(reqSysMemberID), CDate(reqFromDate), CDate(reqToDate), CLng(reqCustom1), CLng(reqCustom2), CLng(reqCustom3), CLng(reqCustom4), CLng(reqCustom5)
                     If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  End If
               End If
               If (reqPrivate <> 0) Then
                  .ExpLeaderPrivateCustom CLng(reqContestID), CDate(reqFromDate), CDate(reqToDate), CLng(reqCustom1), CLng(reqCustom2), CLng(reqCustom3), CLng(reqCustom4), CLng(reqCustom5)
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               End If
            End If
            xmlMetrics = .XML(13)
...
If oLanguage.parseError <> 0 Then
   Response.Write "11770 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "11770 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "11770 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "11770 Load file (oData) failed with error code " + CStr(oData.parseError)
