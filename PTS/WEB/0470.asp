...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0470URL", reqReturnURL
   SetCache "0470DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0470")
...
reqDownlineB =  Numeric(GetInput("DownlineB", reqPageData))
reqDashboard =  GetInput("Dashboard", reqPageData)
...
reqViewCodes =  Numeric(GetInput("ViewCodes", reqPageData))
...
reqQVLeft =  GetInput("QVLeft", reqPageData)
reqQVRight =  GetInput("QVRight", reqPageData)
reqBVLeft =  GetInput("BVLeft", reqPageData)
reqBVRight =  GetInput("BVRight", reqPageData)
...
Sub CheckUpline()
   ...
   If (reqSysUserGroup = 41) And (reqMemberID <> reqSysMemberID) Then
      If (reqLine = 1) Or (reqLine = 2) Then
         ...
         Else
            With oMember
               ...
               Result = CLng(.CheckUpline(CLng(reqSysMemberID), CLng(reqMemberID), reqLine))
...
Select Case CLng(reqActionCode)
      ...
      Else
         With oMember
            ...
            If (reqMemberID < 0) Then
               If (reqMemberID = -1) Then
                  tmpStatus = 1001
               End If
               If (reqMemberID = -2) Then
                  tmpStatus = 1002
               End If
               reqMemberID = .Custom(CLng(reqCompanyID), reqTopMemberID, tmpStatus)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
   ...
   reqMemberQV = FormatCurrency(.QV2, 0 )
            ...
            End If
            If (reqLine = 5) And (reqDownlineB <> 0) Then
               reqBinary = 1
               reqLevels = 4
            End If
            xmlMember = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (reqBinary <> 0) Then
               Result = .Custom(CLng(reqCompanyID), CLng(reqMemberID), 1000)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
xmlParam = xmlParam + " downlineb=" + Chr(34) + CStr(reqDownlineB) + Chr(34)
xmlParam = xmlParam + " dashboard=" + Chr(34) + CleanXML(reqDashboard) + Chr(34)
...
xmlParam = xmlParam + " viewcodes=" + Chr(34) + CStr(reqViewCodes) + Chr(34)
...
xmlParam = xmlParam + " qvleft=" + Chr(34) + CleanXML(reqQVLeft) + Chr(34)
xmlParam = xmlParam + " qvright=" + Chr(34) + CleanXML(reqQVRight) + Chr(34)
xmlParam = xmlParam + " bvleft=" + Chr(34) + CleanXML(reqBVLeft) + Chr(34)
xmlParam = xmlParam + " bvright=" + Chr(34) + CleanXML(reqBVRight) + Chr(34)
xmlParam = xmlParam + " />"
...
fileLanguage = "Language" + "\0470[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\0470[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0470 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0470 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0470 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0470 Load file (oData) failed with error code " + CStr(oData.parseError)
