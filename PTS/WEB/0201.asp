...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0201URL", reqReturnURL
   SetCache "0201DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0201")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Set oEmployees = server.CreateObject("ptsEmployeeUser.CEmployees")
      If oEmployees Is Nothing Then
         DoError Err.Number, Err.Source, "Unable to Create Object - ptsEmployeeUser.CEmployees"
      Else
         With oEmployees
...
If oLanguage.parseError <> 0 Then
   Response.Write "0201 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0201 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0201 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0201 Load file (oData) failed with error code " + CStr(oData.parseError)
