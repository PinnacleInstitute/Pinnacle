...
pos = InStr(LCASE(reqSysServerPath), "5217")
...
reqDownload =  GetInput("Download", reqPageData)
...
If oStyle.parseError <> 0 Then
   Response.Write "5217 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      reqDownload = Replace(reqDownload, "%26", "&")
      ...
      Else
         With oCompany
            ...
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            tmpFrom = .Email
            tmpSubject = "Download from " + .CompanyName
            xmlCompany = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
...
fileLanguage = "Language" + "\5217[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\5217[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "5217 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "5217 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
xmlParam = xmlParam + " download=" + Chr(34) + CleanXML(reqDownload) + Chr(34)
xmlParam = xmlParam + " />"
...
xmlTransaction = xmlTransaction +  xmlDownloads
...
If oData.parseError <> 0 Then
   Response.Write "5217 Load file (oData) failed with error code " + CStr(oData.parseError)
