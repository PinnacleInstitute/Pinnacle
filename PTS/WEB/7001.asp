...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "7001URL", reqReturnURL
   SetCache "7001DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "7001")
...
reqPrimary =  Numeric(GetInput("Primary", reqPageData))
...
If (reqSysUserGroup = 41) And (InStr(reqSysUserOptions,"H") = 0) And (reqM = 0) Then
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If (tmpMentoringID = "") Then
         tmpMentoringID = -1
      End If
      If (reqSysUserGroup = 41) And (reqMemberID <> reqSysMemberID) And (reqMemberID <> CLng(tmpMentoringID)) Then
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("7001URL")
      reqReturnData = GetCache("7001DATA")
      SetCache "7001URL", ""
      SetCache "7001DATA", ""
...
Else
   With oGoals
      .SysCurrentLanguage = reqSysLanguage
      If (reqStatus = 0) And (reqPrimary = 0) Then
         reqBookmark = .Find(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      If (reqStatus <> 0) And (reqPrimary = 0) Then
         reqBookmark = .FindStatus(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqStatus), CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      If (reqStatus = 0) And (reqPrimary <> 0) Then
         reqBookmark = .FindPrimary(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      If (reqStatus <> 0) And (reqPrimary <> 0) Then
         reqBookmark = .FindPrimaryStatus(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMemberID), CLng(reqStatus), CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
      xmlGoals = .XML(15)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
xmlParam = xmlParam + " primary=" + Chr(34) + CStr(reqPrimary) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "7001 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "7001 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "7001 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "7001 Load file (oData) failed with error code " + CStr(oData.parseError)
