...
Dim reqFromDate
Dim reqToDate
Dim reqTotalCompleted
Dim reqTotalSubmitted
Dim reqTotalAbandoned
Dim reqTotalInventory
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "5010URL", reqReturnURL
   SetCache "5010DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "5010")
...
reqTotalCompleted =  Numeric(GetInput("TotalCompleted", reqPageData))
reqTotalSubmitted =  Numeric(GetInput("TotalSubmitted", reqPageData))
reqTotalAbandoned =  Numeric(GetInput("TotalAbandoned", reqPageData))
reqTotalInventory =  Numeric(GetInput("TotalInventory", reqPageData))
...
Sub LoadSummary()
   ...
   Else
      With oProducts
         ...
         .Summary CLng(reqProductID), CLng(reqProductTypeID), CDate(reqFromDate), CDate(reqToDate)
   ...
   If (reqProductTypeID <> 0) Then
      ...
      Else
         With oProductType
            .SysCurrentLanguage = reqSysLanguage
            .Load CLng(reqProductTypeID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlProductType = .XML(2)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
...
xmlParam = xmlParam + " totalcompleted=" + Chr(34) + CStr(reqTotalCompleted) + Chr(34)
xmlParam = xmlParam + " totalsubmitted=" + Chr(34) + CStr(reqTotalSubmitted) + Chr(34)
xmlParam = xmlParam + " totalabandoned=" + Chr(34) + CStr(reqTotalAbandoned) + Chr(34)
xmlParam = xmlParam + " totalinventory=" + Chr(34) + CStr(reqTotalInventory) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "5010 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "5010 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "5010 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "5010 Load file (oData) failed with error code " + CStr(oData.parseError)
