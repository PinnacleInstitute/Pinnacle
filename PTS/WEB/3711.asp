...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "3711URL", reqReturnURL
   SetCache "3711DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "3711")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If (reqCopyID <> 0) Then
         ...
         Response.Redirect "3711.asp" & "?OwnerType=" & reqOwnerType & "&OwnerID=" & reqOwnerID & "&CompanyID=" & reqCompanyID
      End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "3711 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "3711 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "3711 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "3711 Load file (oData) failed with error code " + CStr(oData.parseError)
