...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "18207URL", reqReturnURL
   SetCache "18207DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "18207")
...
Select Case CLng(reqActionCode)
  ...
  xmlHTMLFile = Replace( xmlHTMLFile, "{merchants}", tmpMerchants )
   ...
   Case CLng(cActionReturn):
      ...
      Response.Redirect "18203.asp" & "?MarketID=" & reqMarketID & "&ReturnURL=" & GetCache("18207URL")
End Select
...
If oLanguage.parseError <> 0 Then
   Response.Write "18207 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "18207 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "18207 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "18207 Load file (oData) failed with error code " + CStr(oData.parseError)
