...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8813URL", reqReturnURL
   SetCache "8813DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8813")
...
reqShortcutName =  GetInput("ShortcutName", reqPageData)
...
Sub LoadEmail()
   ...
   Else
      With oEmail
         ...
         tmpFile = .FileName
         reqShortcutName = Replace(.EmailName, "#","")
         xmlEmail = .XML(2)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("8813URL")
         reqReturnData = GetCache("8813DATA")
         SetCache "8813URL", ""
         SetCache "8813DATA", ""
   ...
   Case CLng(cActionTestEmail):
      ...
      If (tmpEmail <> "") Then
         ...
         If (xmlError = "") Then
            Response.Redirect "8808.asp" & "?EmailID=" & reqEmailID & "&MailReturnURL=" & "8813.asp?EmailID=" & reqEmailID & "%26TestEmail=" + tmpEmail 
         End If
      End If
...
xmlParam = xmlParam + " shortcutname=" + Chr(34) + CleanXML(reqShortcutName) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "8813 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8813 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8813 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8813 Load file (oData) failed with error code " + CStr(oData.parseError)
