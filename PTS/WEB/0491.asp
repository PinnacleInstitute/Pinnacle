...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0491URL", reqReturnURL
   SetCache "0491DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0491")
...
reqStartDate =  GetInput("StartDate", reqPageData)
reqEndDate =  GetInput("EndDate", reqPageData)
reqWeek =  Numeric(GetInput("Week", reqPageData))
...
If (reqSysUserGroup > 23) And (reqSysUserGroup <> 51) And (reqMemberID <> reqSysMemberID) Then
...
Select Case CLng(reqActionCode)
   ...
   reqEndDate = DateAdd("d", 6, reqStartDate) 
            ...
            result = .Custom(16, reqStartDate, CLng(reqMemberID), 0)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
xmlParam = xmlParam + " startdate=" + Chr(34) + CStr(reqStartDate) + Chr(34)
xmlParam = xmlParam + " enddate=" + Chr(34) + CStr(reqEndDate) + Chr(34)
xmlParam = xmlParam + " week=" + Chr(34) + CStr(reqWeek) + Chr(34)
xmlParam = xmlParam + " />"
...
xmlTransaction = xmlTransaction +  xmlIC
...
fileLanguage = "Language" + "\0491[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\0491[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0491 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0491 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0491 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0491 Load file (oData) failed with error code " + CStr(oData.parseError)
