<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
   ...
   <xsl:template match="/">
      ...
      <xsl:call-template name="HTMLHeading">
         <xsl:with-param name="pagename" select="/DATA/LANGUAGE/LABEL[@name='Documents']"/>
                                       ...
                                       <xsl:element name="TD">
                                          ...
                                          <xsl:if test="(@islink = 0)">
                                                <xsl:if test="(/DATA/SYSTEM/@usergroup != 41) or (contains(/DATA/SYSTEM/@useroptions, 's'))">
                                                   <xsl:element name="A">
                                                      <xsl:attribute name="onclick">w=window.open(this.href,"shortcut","width=400, height=150");if (window.focus) {w.focus();};return false;</xsl:attribute>
                                                      <xsl:attribute name="href">9202.asp?EntityID=80&amp;ItemID=<xsl:value-of select="@attachmentid"/>&amp;Name=<xsl:value-of select="translate(@attachname,'&amp;',' ')"/>&amp;URL=Attachments/<xsl:value-of select="concat(/DATA/PARAM/@parenttype,'/',/DATA/PARAM/@parentid,'/',@filename)"/>&amp;IsPopup=1</xsl:attribute>
                                                      <xsl:element name="IMG">
                                                         <xsl:attribute name="src">Images/Shortcut.gif</xsl:attribute>
                                                         <xsl:attribute name="border">0</xsl:attribute>
                                                      </xsl:element>
