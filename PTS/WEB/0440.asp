...
<!--#include file="Include\TB.asp"-->
<!--#include file="Include\MemberOptions.asp"-->
<% Response.Buffer=true
'-----action code constants
Const cActionNew = 0
Const cActionRefresh = 1
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0440URL", reqReturnURL
   SetCache "0440DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0440")
...
reqTime =  Numeric(GetInput("Time", reqPageData))
reqVisit =  Numeric(GetInput("Visit", reqPageData))
reqVisitDate =  GetInput("VisitDate", reqPageData)
reqDate30 =  GetInput("Date30", reqPageData)
reqContentPage =  Numeric(GetInput("ContentPage", reqPageData))
reqRefresh =  Numeric(GetInput("Refresh", reqPageData))
reqTBImage =  GetInput("TBImage", reqPageData)
reqTBPage =  GetInput("TBPage", reqPageData)
...
Function LoadTB(bvLoad)
   On Error Resume Next
   reqTBImage = GetMemberOptions("TBI")
   reqTBPage = GetMemberOptions("TBP")
   ...
   Else
      With oMember
         ...
         If (bvLoad = 1) Then
            xmlTB = .TBCount(CLng(reqMemberID), CDate(reqVisitDate))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (bvLoad = 1) Then
            SetTB(xmlTB)
...
End If
If (reqSysUserGroup = 41) And (InStr(reqSysUserOptions,"J") = 0) Then
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("0440URL")
      reqReturnData = GetCache("0440DATA")
      SetCache "0440URL", ""
      SetCache "0440DATA", ""
...
xmlParam = xmlParam + " time=" + Chr(34) + CStr(reqTime) + Chr(34)
xmlParam = xmlParam + " visit=" + Chr(34) + CStr(reqVisit) + Chr(34)
xmlParam = xmlParam + " visitdate=" + Chr(34) + CleanXML(reqVisitDate) + Chr(34)
xmlParam = xmlParam + " date30=" + Chr(34) + CleanXML(reqDate30) + Chr(34)
xmlParam = xmlParam + " contentpage=" + Chr(34) + CStr(reqContentPage) + Chr(34)
xmlParam = xmlParam + " refresh=" + Chr(34) + CStr(reqRefresh) + Chr(34)
xmlParam = xmlParam + " tbimage=" + Chr(34) + CleanXML(reqTBImage) + Chr(34)
xmlParam = xmlParam + " tbpage=" + Chr(34) + CleanXML(reqTBPage) + Chr(34)
xmlParam = xmlParam + " />"
...
xmlTransaction = xmlTransaction +  xmlTB
...
fileLanguage = "Language" + "\0440[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\0440[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0440 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0440 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0440 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0440 Load file (oData) failed with error code " + CStr(oData.parseError)
