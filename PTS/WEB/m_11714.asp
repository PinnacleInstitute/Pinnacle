...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_11714URL", reqReturnURL
   SetCache "m_11714DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_11714")
...
Sub ViewMetrics()
   ...
   Else
      With oMetrics
         .SysCurrentLanguage = reqSysLanguage
         If (reqRpt <> 0) Then
            .LeaderSummary CLng(reqMemberID), CDate(reqFromDate), CDate(reqToDate), CLng(reqRpt)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqRpt = 0) Then
            .LeaderSummaryCustom CLng(reqMemberID), CDate(reqFromDate), CDate(reqToDate), CLng(reqCustom1), CLng(reqCustom2), CLng(reqCustom3), CLng(reqCustom4), CLng(reqCustom5)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionReturn):
      ...
      reqReturnURL = GetCache("m_11714URL")
      reqReturnData = GetCache("m_11714DATA")
      SetCache "m_11714URL", ""
      SetCache "m_11714DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_11714 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_11714 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_11714 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_11714 Load file (oData) failed with error code " + CStr(oData.parseError)
