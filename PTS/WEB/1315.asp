...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1315URL", reqReturnURL
   SetCache "1315DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1315")
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oSessionLessons
            .SysCurrentLanguage = reqSysLanguage
            .ListSession CLng(reqCourseID), CLng(reqSessionID), "", CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "1315 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1315 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1315 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1315 Load file (oData) failed with error code " + CStr(oData.parseError)
