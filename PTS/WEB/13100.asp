...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "13100URL", reqReturnURL
   SetCache "13100DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "13100")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      If (reqSysUserGroup = 21) Then
         If InStr( ",2,3,5,11,12,21,22,23,24,25,26,29,33,34,39,44,45,46,47,48,49,50,", CSTR(reqProcess)+"," ) = 0 Then reqProcess = 0
      End If
   ...
   Case CLng(cActionUpdate):
      If (reqProcess = 12) Or (reqProcess = 39) Or (reqProcess = 47) Then
         ...
         End If
      End If
      If (reqProcess = 2) Or (reqProcess = 11) Or (reqProcess = 21) Or (reqProcess = 24) Or (reqProcess = 25) Or (reqProcess = 35) Or (reqProcess = 28) Or (reqProcess = 48) Or (reqProcess = 49) Or (reqProcess = 50) Then
         If (reqToDate = "") Then
            DoError 10115, "", "Oops, Specify the date range to process."
         End If
      End If
      If (xmlError = "") Then
         ...
         If (reqProcess = 2) Then
            ...
            Set oPB = server.CreateObject("ptsPBUser.CPB")
            If oPB Is Nothing Then
               DoError Err.Number, Err.Source, "Unable to Create Object - ptsPBUser.CPB"
            Else
               With oPB
         ...
         If (reqProcess = 24) Then
            Total = 0
            ExportWallet reqCompanyID, 12, reqToDate, 0, tmpTotal, tmpFile, tmpAmount
         ...
         If (reqProcess = 30) Then
            ...
            Set oPBs = server.CreateObject("ptsPBUser.CPBs")
            If oPBs Is Nothing Then
               DoError Err.Number, Err.Source, "Unable to Create Object - ptsPBUser.CPBs"
            Else
               With oPBs
                  .SysCurrentLanguage = reqSysLanguage
                  .CustomList 3, 0, 0, 0
         ...
         If (reqProcess = 35) Then
            ...
            If (tmpTotal <> 0) Then
               ...
               LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
               MSG = CSTR(tmpTotal) + " PB Submitted Orders (" & FormatCurrency(tmpAmount,2) & ")  "
               reqResult = reqResult + MSG + LINK + "<br>"
               NOTE = NOTE + MSG + URL + "  " + VBLF
            End If
            ...
            If (tmpTotal <> 0) Then
               URL = "http://www.pinnaclep.com/orders/" + CStr(reqCompanyID) + "/" + tmpFile
               LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
               MSG = CSTR(tmpTotal) + " Access Glory Cards (" & FormatCurrency(tmpAmount,2) & ")  "
               reqResult = reqResult + MSG + LINK + "<br>"
               NOTE = NOTE + MSG + URL + "  " + VBLF
            End If
            ...
            If (tmpTotal <> 0) Then
               URL = "http://www.pinnaclep.com/orders/" + CStr(reqCompanyID) + "/" + tmpFile
               LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
               MSG = CSTR(tmpTotal) + " Glory Cards Legal Services (" & FormatCurrency(tmpAmount,2) & ")  "
               reqResult = reqResult + MSG + LINK + "<br>"
               NOTE = NOTE + MSG + URL + "  " + VBLF
            End If
            If (Total <> 0) Then
         ...
         If (reqProcess = 42) Then
            ...
            Else
               With oPB
                  .SysCurrentLanguage = reqSysLanguage
                  Count = CLng(.Custom(201, 0, 0, 0))
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  reqResult = Count & " Payments Updated in the Binary Sales Volume!"
               End With
            End If
            Set oPB = Nothing
         End If
         ...
         If (reqProcess = 44) Then
            ...
            Else
               With oMembers
                  ...
                  xmlMembers = .XML()
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  reqResult = "Top Binary Recruiters!"
               End With
            End If
            Set oMembers = Nothing
         End If
         ...
         If (reqProcess = 46) Then
            ...
            Else
               With oMembers
                  ...
                  xmlMembers = .XML()
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  reqResult = "Top Binary Countries!"
               End With
            End If
            Set oMembers = Nothing
         End If
      ...
      If (reqProcess = 50) Then
         ...
         Else
            With oPB
               .SysCurrentLanguage = reqSysLanguage
               Result = .Custom(302, reqToDate, 0, 0)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               ...
               reqResult =  Good & " Qualified, " & Bad & " Unqualified"
            End With
         End If
         Set oPB = Nothing
      End If
...
xmlTransaction = xmlTransaction +  xmlPBs
...
fileLanguage = "Language" + "\13100[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\13100[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "13100 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "13100 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "13100 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "13100 Load file (oData) failed with error code " + CStr(oData.parseError)
