...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_0463URL", reqReturnURL
   SetCache "m_0463DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_0463")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionLogon):
      ...
      Response.Redirect "m_0103.asp" & "?ReturnURL=" & reqPageURL

   Case CLng(cActionPassword):

      Response.Redirect "m_0106.asp" & "?ReturnURL=" & reqPageURL
End Select
...
fileLanguage = "Language" + "\m_0463[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\m_0463[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "m_0463 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_0463 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_0463 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_0463 Load file (oData) failed with error code " + CStr(oData.parseError)
