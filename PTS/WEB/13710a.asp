...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "13710aURL", reqReturnURL
   SetCache "13710aDATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "13710a")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oProfile
            ...
            reqSampleGraphPath = "Sections/Company/" + CStr(reqCompanyID) + "/Profile" + CStr(reqProfileType) + "/"
            LanguageFile = "Sections/Company/" + CStr(reqCompanyID) + "/Profile" + CStr(reqProfileType)
            
                  pos = InStr( tmpProfileDate, " " )
                  If pos > 0 Then tmpProfileDate = Left(tmpProfileDate, pos-1)

                  reqVQILeft = 38 - (6 + (2 * CLng(.VQClarity_I)))
                  reqVQELeft = 38 - (6 + (2 * CLng(.VQClarity_E)))
                  reqVQSLeft = 38 - (6 + (2 * CLng(.VQClarity_S)))
                  reqSQILeft = 38 - (6 + (2 * CLng(.SQClarity_I)))
                  reqSQELeft = 38 - (6 + (2 * CLng(.SQClarity_E)))
                  reqSQSLeft = 38 - (6 + (2 * CLng(.SQClarity_S)))

                  reqVQITop = (88 - (6 + (2 * CLng(.VQClarity_I)))) - (CLng(.xVQBias_I) * 7)
                  reqVQETop = (88 - (6 + (2 * CLng(.VQClarity_E)))) - (CLng(.xVQBias_E) * 7)
                  reqVQSTop = (88 - (6 + (2 * CLng(.VQClarity_S)))) - (CLng(.xVQBias_S) * 7)
                  reqSQITop = (88 - (6 + (2 * CLng(.SQClarity_I)))) - (CLng(.xSQBias_I) * 7)
                  reqSQETop = (88 - (6 + (2 * CLng(.SQClarity_E)))) - (CLng(.xSQBias_E) * 7)
                  reqSQSTop = (88 - (6 + (2 * CLng(.SQClarity_S)))) - (CLng(.xSQBias_S) * 7)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("13710aURL")
      reqReturnData = GetCache("13710aDATA")
      SetCache "13710aURL", ""
      SetCache "13710aDATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "13710a Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "13710a Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "13710a Load file (oData) failed with error code " + CStr(oData.parseError)
