...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "15031URL", reqReturnURL
   SetCache "15031DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "15031")
   ...
   Case CLng(cActionNew):
      ...
      If (reqCompanyID = 21) Then
         ...
         Else
            With oNexxus
               .SysCurrentLanguage = reqSysLanguage
               Result = .Custom(29, 0, CLng(reqMerchantID), 0)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
         Set oNexxus = Nothing
...
fileLanguage = "Language" + "\15031[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\15031[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "15031 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "15031 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "15031 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "15031 Load file (oData) failed with error code " + CStr(oData.parseError)
