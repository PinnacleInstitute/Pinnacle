...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0419URL", reqReturnURL
   SetCache "0419DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0419")
...
reqError =  Numeric(GetInput("Error", reqPageData))
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      End If
      If (reqError <> 0) Then
         DoError 10112, "", "Oops, The selected feature is not available with your membership. Please review the available upgrade options."
      End If
      ...
      Else
         With oMember
            ...
            If (.Billing = 3) And ((.Status = 3) Or (.Level <> 1)) Then
               reqUpgrade = 1
            End If
         End With
      End If
...
xmlParam = xmlParam + " error=" + Chr(34) + CStr(reqError) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "0419 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0419 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0419 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0419 Load file (oData) failed with error code " + CStr(oData.parseError)
