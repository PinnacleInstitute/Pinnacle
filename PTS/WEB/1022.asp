...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1022URL", reqReturnURL
   SetCache "1022DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1022")
...
reqSTP =  Numeric(GetInput("STP", reqPageData))
...
If (reqSysUserGroup = 1) Or (reqSysCompanyID = 17) Then
   ...
   Else
      With oMember
         CountryID = .Custom(17, CLng(reqMemberID), 1010)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         If (CountryID <> 0) Then
            reqSTP = STPCountry( CountryID )
         End If
      End With
   End If
   Set oMember = Nothing
...
Else
   With oPayments
      reqBookmark = .FindOwner(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, 04, CLng(reqMemberID), CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
xmlParam = xmlParam + " stp=" + Chr(34) + CStr(reqSTP) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "1022 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1022 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1022 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1022 Load file (oData) failed with error code " + CStr(oData.parseError)
