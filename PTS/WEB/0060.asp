...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0060URL", reqReturnURL
   SetCache "0060DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0060")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      End If
      If (reqCompanyID = 0) Then
         reqFromDate = Month(Date())&"/1/"&Year(Date())
         reqToDate = reqSysDate
         reqTerm = 7
      End If
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("0060URL")
      reqReturnData = GetCache("0060DATA")
      SetCache "0060URL", ""
      SetCache "0060DATA", ""
...
fileLanguage = "Language\0060.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0060 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0060 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0060 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0060 Load file (oData) failed with error code " + CStr(oData.parseError)
