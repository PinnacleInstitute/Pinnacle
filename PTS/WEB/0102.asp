...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0102URL", reqReturnURL
   SetCache "0102DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0102")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      Else
         With oAuthUser
            ...
            If (.AuthUserID = 0) Then
               DoError -2147220512, "", "Oops, Could not find the Email address. Please re-enter."
            End If
         End With
      End If
      ...
      If (xmlError = "") Then
         Response.Redirect "0105.asp" & "?AuthUserID=" & tmpAuthUserID & "&MailReturnURL=" & GetCache("0102URL")
      End If
End Select
...
If oLanguage.parseError <> 0 Then
   Response.Write "0102 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0102 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0102 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0102 Load file (oData) failed with error code " + CStr(oData.parseError)
