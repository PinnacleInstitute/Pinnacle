...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8810URL", reqReturnURL
   SetCache "8810DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8810")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionReturn):
      ...
      If (reqNewsLetterID = 0) Then
         Response.Redirect "8804.asp" & "?EmailID=" & reqEmailID & "&ReturnURL=" & GetCache("8810URL")
      End If

      If (reqEmailType = 3) Then
         Response.Redirect "8813.asp" & "?EmailID=" & reqEmailID & "&ReturnURL=" & GetCache("8810URL")
      End If

      If (reqEmailType = 4) Then
         Response.Redirect "8823.asp" & "?EmailID=" & reqEmailID & "&ReturnURL=" & GetCache("8810URL")
      End If
End Select
...
If oLanguage.parseError <> 0 Then
   Response.Write "8810 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8810 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8810 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8810 Load file (oData) failed with error code " + CStr(oData.parseError)
