...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8128URL", reqReturnURL
   SetCache "8128DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8128")
...
Sub LoadLeads()
   ...
   Else
      With oProspects
         .Distributed CLng(reqMemberID), reqFromDate, tmpToDate
         ...
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            ...
            For Each oProspect in oProspects
               reqMembers = reqMembers + 1
               reqCount = reqCount + CLNG(oProspect.Count)
            Next
            reqMembers = FormatNumber(reqMembers,0)
...
fileLanguage = "Language\8128.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "8128 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8128 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8128 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8128 Load file (oData) failed with error code " + CStr(oData.parseError)
