...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8129URL", reqReturnURL
   SetCache "8129DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8129")
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oProspects
            ...
            If (reqRpt = 1) Then
               .MemberDate CLng(reqMemberID), reqVisitDate
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqRpt = 2) Then
               .MemberDate CLng(reqMemberID), reqDate30
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqRpt = 3) Then
               .MemberActive CLng(reqMemberID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            xmlProspects = .XML(13)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            ...
            For Each oProspect in oProspects
               reqCount = reqCount + 1
               reqAmount = reqAmount + CCUR(oProspect.Potential)
            Next
...
fileLanguage = "Language" + "\8129[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\8129[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "8129 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8129 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8129 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8129 Load file (oData) failed with error code " + CStr(oData.parseError)
