...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "9505URL", reqReturnURL
   SetCache "9505DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "9505")
...
Function GetCategory(bvIssueCategoryID)
   On Error Resume Next
   ...
   Else
      With oIssueCategory
         .SysCurrentLanguage = reqSysLanguage
         .Load bvIssueCategoryID, CLng(reqSysUserID)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   ...
   Else
      With oIssue
         ...
         .IssueCategoryID = bvIssueCategoryID
         xmlIssue = .XML(2)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oIssue
            ...
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            .IssueCategoryID = reqCategory
            xmlIssue = .XML(2)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oIssue = Nothing
      If (reqCategory <> 0) Then
         GetCategory(reqCategory)
      End If
      LoadList

   Case CLng(cActionCategory):
      tmpIssueCategoryID = Request.Form.Item("IssueCategoryID")
      GetCategory(tmpIssueCategoryID)
      LoadList
   ...
   Case CLng(cActionAdd):
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("9505URL")
         reqReturnData = GetCache("9505DATA")
         SetCache "9505URL", ""
         SetCache "9505DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "9505 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "9505 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "9505 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "9505 Load file (oData) failed with error code " + CStr(oData.parseError)
