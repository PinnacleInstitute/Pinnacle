...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "9811URL", reqReturnURL
   SetCache "9811DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "9811")
...
reqUpdateParent =  Numeric(GetInput("UpdateParent", reqPageData))
...
Function ListNews(add)
   ...
   Else
      With oNewss
         ...
         .ListShare CLng(reqCompanyID), tmpOption, CDate(reqFromDate), CDate(reqToDate)
...
xmlParam = xmlParam + " updateparent=" + Chr(34) + CStr(reqUpdateParent) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "9811 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "9811 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "9811 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "9811 Load file (oData) failed with error code " + CStr(oData.parseError)
