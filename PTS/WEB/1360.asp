...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1360URL", reqReturnURL
   SetCache "1360DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1360")
...
reqMembers =  Numeric(GetInput("Members", reqPageData))
...
Sub LoadReport()
   ...
   Else
      With oSession
         reqMembers = CLng(.MemberAccess(CLng(reqOrgID), reqFromDate, reqToDate))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
   ...
   Else
      With oSessions
         .ReportOrg CLng(reqOrgID), reqFromDate, reqToDate
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
xmlParam = xmlParam + " members=" + Chr(34) + CStr(reqMembers) + Chr(34)
xmlParam = xmlParam + " />"
...
fileLanguage = "Language\1360.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "1360 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1360 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1360 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1360 Load file (oData) failed with error code " + CStr(oData.parseError)
