...
this.toolbar = [
//Font Related Tools
    ['formatblock', 'fontname', 'fontsize','forecolor','backcolor'],
//	['fontstyle'],
//	['template'],
//	['linebreak'],
...
//Format Text Block
	['OrderedList','UnOrderedList','Outdent','Indent','spell'],
	['linebreak'],
...
//	if ((ord == 13) && editEvent.type == 'keypress') {
	if (ctrlKey && (ord == 13) && editEvent.type == 'keydown') {
...
// don't execute more than 3 times a second (eg: too soon after last execution)
	...
	var editor_obj  = document.all["_" +objname+  "_editor"];// html editor object
	var isWYSIWYG = (editor_obj.tagName.toLowerCase() != 'textarea');
	var editdoc = isWYSIWYG ? editor_obj.contentWindow.document : null;

// If we are losing focus (blur) hide the table border so it is not saved
	if (isWYSIWYG && runDelay == -1 && config.showborders) { nullBorders(editdoc,'hide') }

	editor_updateOutput(objname);
	editor_updateToolbar(objname);

// If we are losing focus (blur), reshow the table border
	if (isWYSIWYG && runDelay == -1 && config.showborders) { nullBorders(editdoc,'show') }
...
// end insert lvn property pallettes
    ...
    ['Cut', 'Ctrl-X', function() {editor_action('_' + objname + '_' + 'Cut');}],
    ['Copy', 'Ctrl-C', function() {editor_action('_' + objname + '_' + 'Copy');}],
    ['Paste', 'Ctrl-V', function() {editor_action('_' + objname + '_' + 'Paste');}],
    ['Undo', 'Ctrl-Z', function() {editor_action('_' + objname + '_' + 'Undo');}],
    ['---', null, null],
    ['Select All', 'Ctrl-A', function() {editor_action('_' + objname + '_' + 'selectall');}],
    ['Clear All', '', function() {editor_action('_' + objname + '_' + 'Refresh');}],
    ['---', null, null],
...
function editor_cMenu_setPosition(parentWin, editorWin, objname) {      // set object position that won't overlap window edge
  ...
  if (mouseY + 5 + cMenuH > pageH) { var top = mouseY - cMenuH + 250; } // too far down
...
// RAW 2/3/04 strip out the leading <P> - IE inserts this when you replace the selected text with anything 
if ( unclean.substring(0,3) == '<P>') {unclean = unclean.substring(3);} //very beginning
if ( unclean.substring(2,5) == '<P>') {unclean = unclean.substring(5);} //leading newline
...
//Some work just for formatting the output HTML file
	unclean = unclean.replace(/<P/gi, '\n<P');
	unclean = unclean.replace(/<P>\n/gi, '');
	//unclean = unclean.replace(/<H/gi, '\n<H');
	//unclean = unclean.replace(/<TBODY>/gi, '');			//Removes opening <TBODY>
	//unclean = unclean.replace(/<\/TBODY>/gi, '');		//Removes closing </TBODY>
	unclean = unclean.replace(/<T/gi, '\n<T');
	unclean = unclean.replace(/<TD>\n/gi, '<TD>');
	unclean = unclean.replace(/<\/TR>/gi, '\n<\/TR>');
	unclean = unclean.replace(/<\/TR>\n/gi, '<\/TR>');
...
//Remove unnecesary code
	unclean = unclean.replace(/<B><\/B>/ig, '');		//Empty <B></B>
	unclean = unclean.replace(/<I><\/I>/ig, '');		//Empty <I></I>
	unclean = unclean.replace(/<U><\/U>/ig, '');		//Empty <U></U>
	unclean = unclean.replace(/<H[^>]*><\/H[^>]*>/gi, "");//Empty <H1></H1>, <H2></H2> etc.
	//unclean = unclean.replace(/<A HREF=[^>]*#top\">/gi, '<A HREF=\"#top\">');
	unclean = unclean.replace(/<BR[^>]*>/gi, "\n<BR>");
//	unclean = unclean.replace(/\&nbsp\;/ig, '');		//No breaking spaces
//	unclean = unclean.replace(/<BR>\n<BR>/gi, '<P>');		//Subtitutes double break with a paragraph 
	unclean = unclean.replace(/dir=ltr /gi, '');		//dir ltr doesn't make sense since it's the default
	unclean = unclean.replace(/> <\//gi, '><\/');		//This is to remove a blank space in empty tags
	unclean = unclean.replace(/<\/LI>/gi, '');			//Removes closing </LI>
	unclean = unclean.replace(/<\/P>/gi, '');			//Removes closing </P>
	unclean = unclean.replace(/<\/DT>/gi, '');			//Removes closing </DT>
	unclean = unclean.replace(/<\/DD>/gi, '');			//Removes closing </DD>
    /*unclean = unclean.replace(/<H1[^>]*><B><U>/gi, '<H5>');
	unclean = unclean.replace(/<\/U><\/B><\/H1[^>]*>/gi, '<\/H5>');
    unclean = unclean.replace(/<H2[^>]*><B><U>/gi, '<H5>');
	unclean = unclean.replace(/<\/U><\/B><\/H2[^>]*>/gi, '<\/H5>');
    unclean = unclean.replace(/<H3[^>]*><B><U>/gi, '<H5>');
	unclean = unclean.replace(/<\/U><\/B><\/H3[^>]*>/gi, '<\/H5>');
    unclean = unclean.replace(/<H4[^>]*><B><U>/gi, '<H5>');
	unclean = unclean.replace(/<\/U><\/B><\/H4[^>]*>/gi, '<\/H5>');
    unclean = unclean.replace(/<H6[^>]*><B><U>/gi, '<H5>');
	unclean = unclean.replace(/<\/U><\/B><\/H6[^>]*>/gi, '<\/H5>');
    unclean = unclean.replace(/<H7[^>]*><B><U>/gi, '<H5>');
	unclean = unclean.replace(/<\/U><\/B><\/H7[^>]*>/gi, '<\/H5>');
    unclean = unclean.replace(/<H1[^>]*><U>/gi, '<H5>');
	unclean = unclean.replace(/<\/U><\/B><\/H1[^>]*>/gi, '<\/H5>');
    unclean = unclean.replace(/<H2[^>]*><U>/gi, '<H5>');
	unclean = unclean.replace(/<\/U><\/B><\/H3[^>]*>/gi, '<\/H5>');
    unclean = unclean.replace(/<H4[^>]*><U>/gi, '<H5>');
	unclean = unclean.replace(/<\/U><\/B><\/H3[^>]*>/gi, '<\/H5>');
    unclean = unclean.replace(/<H4[^>]*><U>/gi, '<H5>');
	unclean = unclean.replace(/<\/U><\/B><\/H4[^>]*>/gi, '<\/H5>');
    unclean = unclean.replace(/<H6[^>]*><U>/gi, '<H5>');
	unclean = unclean.replace(/<\/U><\/B><\/H6[^>]*>/gi, '<\/H5>');
    unclean = unclean.replace(/<H7[^>]*><U>/gi, '<H5>');
	unclean = unclean.replace(/<\/U><\/B><\/H7[^>]*>/gi, '<\/H5>');
    unclean = unclean.replace(/<H1>/gi, '<H5>');
	unclean = unclean.replace(/<\/H1>/gi, '<\/H5>');
    unclean = unclean.replace(/<H2>/gi, '<H5>');
	unclean = unclean.replace(/<\/H2>/gi, '<\/H5>');
    unclean = unclean.replace(/<H3>/gi, '<H5>');
	unclean = unclean.replace(/<\/H3>/gi, '<\/H5>');
    unclean = unclean.replace(/<H4>/gi, '<H5>');
	unclean = unclean.replace(/<\/H4>/gi, '<\/H5>');
    unclean = unclean.replace(/<H6>/gi, '<H5>');
	unclean = unclean.replace(/<\/H6>/gi, '<\/H5>');
    unclean = unclean.replace(/<H7>/gi, '<H5>');
	unclean = unclean.replace(/<\/H7>/gi, '<\/H5>');*/
...
//Otherwise, comment it out and remove it on server-side
	unclean = unclean.replace(/http\:\/\/www\.andsell\.com/gi, ''); 
	unclean = unclean.replace(/http\:\/\/localhost\/ice\//gi, ''); 
	unclean = unclean.replace(/http\:\/\/localhost\/wms\//gi, ''); 
