<%
'*****************************************************************************************************
Function ExportCheck( byVal CompanyID, byVal CommDate, byVal Amount, byRef reqTotal, byRef reqFile, byRef reqAmount)
...
'*****************************************************************************************************
Function ExportWallet( byVal CompanyID, byVal Processor, byVal CommDate, byVal Amount, byRef reqTotal, byRef reqFile, byRef reqAmount)
	On Error Resume Next
	...
	Else
		With oBillings
		.SysCurrentLanguage = reqSysLanguage
		.CommWallet CompanyID, CommDate, Amount
		If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
		...
		For Each oBill in oBillings
		    With oBill
    		    If CLng(.CardType) = Processor Then
                    cnt = cnt + 1
                    If reqTotal = 0 Then
                        Set oFile = oFileSys.CreateTextFile(Path + reqFile, True)
                        If oFile Is Nothing Then
                    ...
                    Case Processor_iPayout:
			            ...
			            Rec = CHR(34) + .CardName + CHR(34) + "," + .Amount + "," + "USD" + "," + .BillingID + "," + CHR(34) + .BillingName + " - " + .CountryName + CHR(34)
                    Case Processor_PayQuicker:
			            'OLD: Email Address, Amount, Currency, Member #, Comment
			            ...
			            End If
		                Rec = .CardName + VBTAB + .Amount  + VBTAB + tmpCountryCode + VBTAB + tmpStateCode + VBTAB + .BillingID + VBTAB + .BillingName + " - " + .CountryName
                    Case Processor_SolidTrust:
                        '[USERNAME] [tab] [AMOUNT] [tab] [COMMENTS] [tab] [UDF1] [tab]
			            tmpNote = .BillingID + " - " + .BillingName + " - " + .CountryName
		                Rec = .CardName + VBTAB + .Amount  + VBTAB + tmpNote + VBTAB + .BillingID
                    Case Processor_Coinbase:
			            'Wallet #, Amount, #, Comment
			            tmpNote = .BillingID + " - " + Replace(.BillingName + " - " + .CountryName, ",", "")
			            Rec = Trim(.CardName) +  "," + .Amount + "," + tmpNote
                    End Select
...
'*****************************************************************************************************
Function ExportACH( byVal CompanyID, byVal CommDate, byVal Amount, byRef reqTotal, byRef reqFile, byRef reqAmount)
	On Error Resume Next
