...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "14102URL", reqReturnURL
   SetCache "14102DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "14102")
...
Sub AddFriend()
   ...
   Else
      With oFriend
         ...
         reqFriendID = CLng(.Add(CLng(reqSysUserID)))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         If (xmlError <> "") Then
            xmlFriend = .XML(2)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("14102URL")
         reqReturnData = GetCache("14102DATA")
         SetCache "14102URL", ""
         SetCache "14102DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "14102 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "14102 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "14102 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "14102 Load file (oData) failed with error code " + CStr(oData.parseError)
