...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "5201URL", reqReturnURL
   SetCache "5201DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "5201")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("5201URL")
      reqReturnData = GetCache("5201DATA")
      SetCache "5201URL", ""
      SetCache "5201DATA", ""
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oSalesOrder
            .SysCurrentLanguage = reqSysLanguage
            .DeleteAbandoned CLng(reqCompanyID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "5201 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "5201 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "5201 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "5201 Load file (oData) failed with error code " + CStr(oData.parseError)
