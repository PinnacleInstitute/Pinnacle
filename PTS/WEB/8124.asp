...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8124URL", reqReturnURL
   SetCache "8124DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8124")
   ...
   Else
      With oProspects
         If (reqMemberID = 0) Then
            .ClosedCompany CLng(reqCompanyID), reqFromDate, reqToDate
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqMemberID <> 0) Then
            .ClosedMember CLng(reqMemberID), reqFromDate, reqToDate
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         xmlProspects = .XML()
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
fileLanguage = "Language\8124.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "8124 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8124 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8124 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8124 Load file (oData) failed with error code " + CStr(oData.parseError)
