...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1415URL", reqReturnURL
   SetCache "1415DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1415")
...
reqRate =  GetInput("Rate", reqPageData)
...
Sub LoadReport()
   ...
   Else
      With oLeadPages
         If (reqMemberID = 0) And (reqAffiliateID = 0) Then
            .ReportCompany CLng(reqLeadCampaignID), reqFromDate, tmpToDate
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqMemberID <> 0) And (reqAffiliateID = 0) Then
            .ReportMember CLng(reqLeadCampaignID), reqMemberID, reqFromDate, tmpToDate
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqAffiliateID <> 0) Then
            .ReportAffiliate CLng(reqLeadCampaignID), reqAffiliateID, reqFromDate, tmpToDate
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         
            tmpCount = -1
            For Each oLeadPage in oLeadPages
               With oLeadPage
                  If tmpCount > 0 Then
                     .Language = CStr(formatPercent(CLng(.Seq)/Clng(tmpCount),0))
                  End If   
                  If tmpCount = 0 Then .Language = "0%"
                   tmpCount = .Seq
               End With
            Next
   ...
   If (xmlError = "") Then
      ...
      Else
         With oProspect
            If (reqMemberID = 0) And (reqAffiliateID = 0) Then
               reqCount = CLng(.ReportCompany(CLng(reqLeadCampaignID), reqFromDate, tmpToDate))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqMemberID <> 0) And (reqAffiliateID = 0) Then
               reqCount = CLng(.ReportMember(CLng(reqLeadCampaignID), reqMemberID, reqFromDate, tmpToDate))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqAffiliateID <> 0) Then
               reqCount = CLng(.ReportAffiliate(CLng(reqLeadCampaignID), reqAffiliateID, reqFromDate, tmpToDate))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            
            If tmpCount > 0 Then
               reqRate = CStr(formatPercent(CLng(reqCount)/Clng(tmpCount),0))
            Else
               reqRate = "0%"   
            End If   
...
Sub LoadCampaign()
   ...
   If (xmlError = "") And (reqMemberID <> 0) Then
   ...
   If (xmlError = "") And (reqAffiliateID <> 0) Then
      ...
      Else
         With oAffiliate
            .Load reqAffiliateID, CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlAffiliate = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionClear):
      ...
      Else
         With oLeadLog
            If (reqMemberID = 0) And (reqAffiliateID = 0) Then
               Count = CLng(.ClearAll(CLng(reqLeadCampaignID), reqFromDate, tmpToDate))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqMemberID <> 0) And (reqAffiliateID = 0) Then
               Count = CLng(.ClearMember(reqMemberID, CLng(reqLeadCampaignID), reqFromDate, tmpToDate))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqAffiliateID <> 0) Then
               Count = CLng(.ClearAffiliate(reqAffiliateID, CLng(reqLeadCampaignID), reqFromDate, tmpToDate))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
         End With
      End If
      ...
      LoadCampaign
                  DoError 0, "Delete", Count & " Statistics Cleared!"
End Select
...
xmlParam = xmlParam + " rate=" + Chr(34) + CleanXML(reqRate) + Chr(34)
xmlParam = xmlParam + " />"
...
xmlTransaction = xmlTransaction +  xmlAffiliate
...
fileLanguage = "Language\1415.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "1415 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1415 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1415 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1415 Load file (oData) failed with error code " + CStr(oData.parseError)
