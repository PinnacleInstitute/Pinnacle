...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "18001URL", reqReturnURL
   SetCache "18001DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "18001")
...
Else
   With oAttendees
      .SysCurrentLanguage = reqSysLanguage
      reqBookmark = .Find(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqSeminarID), CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      xmlAttendees = .XML(15)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
...
If oLanguage.parseError <> 0 Then
   Response.Write "18001 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "18001 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "18001 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "18001 Load file (oData) failed with error code " + CStr(oData.parseError)
