...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "3830URL", reqReturnURL
   SetCache "3830DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "3830")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oCompany
            ...
            xmlMB = .MBCount(CLng(reqCompanyID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlCompany = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      ...
      Else
         With oMembers
               ...
               .ReportMembership CLng(reqCompanyID), "1/1/1900", "12/31/2099"
            xmlMBM = "<PTSMBM "
            total = 0
            For Each oMember in oMembers
               total = total + oMember.Quantity
            Next
            For Each oMember in oMembers
               With oMember
                  tmp = .Quantity + " (" + FormatPercent((CLng(.Quantity)/total), 0) + ")"
                  xmlMBM = xmlMBM + "status" + .Status + "=""" + tmp + """ "
               End With
            Next
            xmlMBM = xmlMBM + "total"  & "=""" & total & """ "
            xmlMBM = xmlMBM + "/>"
            ...
            For Each oMember in oMembers
               With oMember
                  tmp = .Quantity + " (" + FormatPercent((CLng(.Quantity)/total), 0) + ")"
                  xmlMBV = xmlMBV  + "visit" + .Amount + "=""" + tmp + """ "
               End With
            Next
            xmlMBV = xmlMBV + "/>"
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("3830URL")
      reqReturnData = GetCache("3830DATA")
      SetCache "3830URL", ""
      SetCache "3830DATA", ""
...
xmlTransaction = xmlTransaction +  xmlMB
xmlTransaction = xmlTransaction +  xmlMBM
xmlTransaction = xmlTransaction +  xmlMBV
...
fileLanguage = "Language\3830.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "3830 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "3830 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "3830 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "3830 Load file (oData) failed with error code " + CStr(oData.parseError)
