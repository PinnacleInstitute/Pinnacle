...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8108URL", reqReturnURL
   SetCache "8108DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8108")
...
Sub LoadList()
   ...
   If (reqProspect = 0) Then
      ...
      Else
         With oPages
            .SysCurrentLanguage = reqSysLanguage
            xmlPages = .EnumPage(CLng(reqCompanyID), tmpGroupID, tmpGroupID1, tmpGroupID2, tmpGroupID3, 4)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oPages = Nothing
               ...
               If email <> "" And InStr(email, "@") <> 0 Then
                  ...
                  Else
                     With oNote
                        ...
                        If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                        .Notes = "EMAILED Lead Page #" & reqLeadCampaignID & " to new contact"
                        .AuthUserID = 1
                        .NoteDate = Now
                        .OwnerType = 22
                        .OwnerID = ProspectID
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      ...
      If (xmlError = "") And (reqPopup = 0) Then
         reqReturnURL = GetCache("8108URL")
         reqReturnData = GetCache("8108DATA")
         SetCache "8108URL", ""
         SetCache "8108DATA", ""
...
fileLanguage = "Language" + "\8108[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\8108[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "8108 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8108 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8108 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8108 Load file (oData) failed with error code " + CStr(oData.parseError)
