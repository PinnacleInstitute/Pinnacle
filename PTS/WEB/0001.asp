...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0001URL", reqReturnURL
   SetCache "0001DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0001")
...
reqAffiliate =  GetInput("Affiliate", reqPageData)
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      End If
      If (reqCompanyID > 0) Then
         tmpPage = GetCache("COMPANYHOMEPAGE")

         If (tmpPage = "HOME") Then
            Response.Redirect "Home.asp" & "?CompanyID=" & reqCompanyID
         End If
         If (tmpPage <> "") Then
            Response.Redirect "Sections/Company/" & reqCompanyID & "/" & tmpPage
         End If

         Response.Redirect "Ascend.asp" & "?CompanyID=" & reqCompanyID
      End If
      ...
      If (reqAffiliate <> "") Or (reqSysAffiliateID > 0) Then
         ...
         Else
            With oAffiliate
               ...
               If (.AffiliateID > 0) Then
                  ...
                  reqSysAffiliateType = .Rank
                  SetCache "AFFILIATETYPE", reqSysAffiliateType
               End If
               xmlAffiliate = .XML(3)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
...
xmlConfig = xmlConfig + " />"
xmlParam = "<PARAM"
xmlParam = xmlParam + " affiliate=" + Chr(34) + CleanXML(reqAffiliate) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "0001 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0001 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0001 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0001 Load file (oData) failed with error code " + CStr(oData.parseError)
