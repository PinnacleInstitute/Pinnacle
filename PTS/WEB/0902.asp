...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0902URL", reqReturnURL
   SetCache "0902DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0902")
   ...
   Case CLng(cActionAdd):
      ...
      Else
         With oCommission
            ...
            CommissionID = CLng(.Add(CLng(reqSysUserID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (xmlError <> "") Then
               xmlCommission = .XML(2)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("0902URL")
         reqReturnData = GetCache("0902DATA")
         SetCache "0902URL", ""
         SetCache "0902DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "0902 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0902 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0902 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0902 Load file (oData) failed with error code " + CStr(oData.parseError)
