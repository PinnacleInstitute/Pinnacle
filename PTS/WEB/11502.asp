...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "11502URL", reqReturnURL
   SetCache "11502DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "11502")
   ...
   Case CLng(cActionAdd):
      ...
      Else
         With oDripPage
            ...
            .Days = Request.Form.Item("Days")
            DripPageID = CLng(.Add(CLng(reqSysUserID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (xmlError <> "") Then
               xmlDripPage = .XML(2)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("11502URL")
         reqReturnData = GetCache("11502DATA")
         SetCache "11502URL", ""
         SetCache "11502DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "11502 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "11502 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "11502 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "11502 Load file (oData) failed with error code " + CStr(oData.parseError)
