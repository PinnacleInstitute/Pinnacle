...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0203URL", reqReturnURL
   SetCache "0203DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0203")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oEmployee
            .Load CLng(reqEmployeeID), CLng(reqSysUserID)
   ...
   Case CLng(cActionUpdate):
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("0203URL")
         reqReturnData = GetCache("0203DATA")
         SetCache "0203URL", ""
         SetCache "0203DATA", ""
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oEmployee
            .Delete CLng(reqEmployeeID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oEmployee = Nothing
...
If oLanguage.parseError <> 0 Then
   Response.Write "0203 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0203 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0203 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0203 Load file (oData) failed with error code " + CStr(oData.parseError)
