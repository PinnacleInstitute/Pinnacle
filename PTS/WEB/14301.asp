...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "14301URL", reqReturnURL
   SetCache "14301DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "14301")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionResetPriority):
      ...
      Else
         With oAd
            .SysCurrentLanguage = reqSysLanguage
            Result = CLng(.Custom(CLng(reqCompanyID), 1, 0, 0))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oAd = Nothing
...
Else
   With oAds
      ...
      End If
      If (reqMemberID <> 0) Then
         reqBookmark = .FindMember(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqCompanyID), CLng(reqMemberID), CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "14301 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "14301 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "14301 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "14301 Load file (oData) failed with error code " + CStr(oData.parseError)
