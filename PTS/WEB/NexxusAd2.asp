...
<!--#include file="Include\PaymentConsumer.asp"-->
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "NexxusAd2URL", reqReturnURL
   SetCache "NexxusAd2DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "nexxusad2")
...
Sub AddOrder()
   ...
   If (xmlError = "") Then
      ...
      Result = GetPayment( reqCompanyID, reqConsumerID, 0, reqPayType, reqPayDesc, BillingPrice, Purpose, tmpProcessor, tmpAcct, PostProcess, EmailBonuses )
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("NexxusAd2URL")
         reqReturnData = GetCache("NexxusAd2DATA")
         SetCache "NexxusAd2URL", ""
         SetCache "NexxusAd2DATA", ""
...
fileLanguage = "Language" + "\NexxusAd2[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\NexxusAd2[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "NexxusAd2 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "NexxusAd2 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
xslPage = "NexxusAd2.xsl"
Set oStyle = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oStyle.load server.MapPath(xslPage)
If oStyle.parseError <> 0 Then
   Response.Write "NexxusAd2 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "NexxusAd2 Load file (oData) failed with error code " + CStr(oData.parseError)
