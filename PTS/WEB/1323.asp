...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1323URL", reqReturnURL
   SetCache "1323DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1323")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oSessions
            If (reqReport = 1) Then
               .CoursesMember CLng(reqMemberID), reqFromDate, reqToDate
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqReport = 2) Then
               .CoursesBusiness reqMasterID, reqFromDate, reqToDate, CLng(reqCourseID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqReport > 1) Then
...
fileLanguage = "Language\1323.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "1323 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1323 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1323 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1323 Load file (oData) failed with error code " + CStr(oData.parseError)
