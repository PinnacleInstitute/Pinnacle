...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_1321URL", reqReturnURL
   SetCache "m_1321DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_1321")
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oCourse
            ...
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            reqAssessmentID = .ExamID
            tmpExamWeight = .ExamWeight
            xmlCourse = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      ...
      Else
         With oSessionLessons
            .SysCurrentLanguage = reqSysLanguage
            .ListQuiz CLng(reqCourseID), CLng(reqSessionID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      ...
      If (reqAssessmentID <> 0) Then
         ...
         Else
            With oAssessment
               .SysCurrentLanguage = reqSysLanguage
               .Load reqAssessmentID, CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               xmlAssessment = .XML()
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
      ...
      If (reqAssessmentID <> 0) Then
         ...
         Else
            With oMemberAssess
               .SysCurrentLanguage = reqSysLanguage
               reqMemberAssessID = CLng(.CheckAssessment(reqAssessmentID, CLng(reqMemberID), CLng(reqSysUserID)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_1321 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_1321 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_1321 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_1321 Load file (oData) failed with error code " + CStr(oData.parseError)
