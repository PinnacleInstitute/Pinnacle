...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "16211URL", reqReturnURL
   SetCache "16211DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "16211")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Set oStaffs = server.CreateObject("ptsStaffUser.CStaffs")
      If oStaffs Is Nothing Then
         DoError Err.Number, Err.Source, "Unable to Create Object - ptsStaffUser.CStaffs"
      Else
         With oStaffs
            .SysCurrentLanguage = reqSysLanguage
            .List CLng(reqMerchantID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlStaffs = xmlStaffs + .XML(13)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oStaffs = Nothing
   ...
   Case CLng(cActionReturn):
      ...
      reqReturnURL = GetCache("16211URL")
      reqReturnData = GetCache("16211DATA")
      SetCache "16211URL", ""
      SetCache "16211DATA", ""
...
xmlTransaction = xmlTransaction +  xmlStaffs
...
If oLanguage.parseError <> 0 Then
   Response.Write "16211 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "16211 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "16211 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "16211 Load file (oData) failed with error code " + CStr(oData.parseError)
