...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_3703URL", reqReturnURL
   SetCache "m_3703DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_3703")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      Else
         With oAddress
            ...
            .AddressType = Request.Form.Item("AddressType")
            ...
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (oldIsActive = 0) And (.IsActive <> 0) Then
               result = CLng(.Activate(CLng(.OwnerType), CLng(.OwnerID), CLng(.AddressType), CLng(reqAddressID)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
      ...
      If (xmlError = "") And (reqPopup = 0) Then
         reqReturnURL = GetCache("m_3703URL")
         reqReturnData = GetCache("m_3703DATA")
         SetCache "m_3703URL", ""
         SetCache "m_3703DATA", ""
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oAddress
            .SysCurrentLanguage = reqSysLanguage
            .Delete CLng(reqAddressID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (xmlError = "") And (reqPopup <> 0) Then
               reqPopup = 3
            End If
...
fileLanguage = "Language" + "\Address[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\Address[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "m_3703 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_3703 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_3703 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_3703 Load file (oData) failed with error code " + CStr(oData.parseError)
