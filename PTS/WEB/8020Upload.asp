...
' set this page to not timeout for 10 minutes
Server.ScriptTimeout = 600

AttachmentID = CStr(GetCache("ATTACHMENTID"))
ContentPage = CStr(GetCache("CONTENTPAGE"))
SizeLimit = CStr(GetCache("SIZELIMIT"))
Mini = CStr(GetCache("MINI"))
SetCache "MINI", ""
If SizeLimit = "" or SizeLimit = "0" Then SizeLimit = 1024 * 1024 * 3 '3MB
...
If Err.number <> 0 Then
	Response.Redirect "8003.asp?AttachmentID=" & AttachmentID & "&ContentPage=" & ContentPage & "&mini=" & Mini & "&UploadError=" & Err.Number & "&UploadErrorDesc=" & Err.Description 
Else
	Response.Redirect "8003.asp?AttachmentID=" & AttachmentID & "&ContentPage=" & ContentPage & "&mini=" & Mini
End If
