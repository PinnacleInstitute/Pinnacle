...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "9304URL", reqReturnURL
   SetCache "9304DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "9304")
...
reqGrpCompanyID =  Numeric(GetInput("GrpCompanyID", reqPageData))
...
If (reqSysUserGroup = 41) Then
   If (reqGrpCompanyID = 0) Then
      reqGrpCompanyID = reqSysCompanyID
   End If
   ...
   If (reqGroupID = 0) Then
      ...
      If (CSTR(reqSysMemberID) = tmpGroupID) Then
         reqGroupID = tmpGroupID
      End If
   End If
End If
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If (reqMemberID <> 0) Then
         reqParentType = 04
         reqParentID = reqMemberID
      End If
      If (reqMemberID = 0) Then
         reqParentType = 38
         reqParentID = reqCompanyID
      End If

   Case CLng(cActionCancel):

      reqReturnURL = GetCache("9304URL")
      reqReturnData = GetCache("9304DATA")
      SetCache "9304URL", ""
      SetCache "9304DATA", ""
...
xmlParam = xmlParam + " grpcompanyid=" + Chr(34) + CStr(reqGrpCompanyID) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "9304 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "9304 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "9304 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "9304 Load file (oData) failed with error code " + CStr(oData.parseError)
