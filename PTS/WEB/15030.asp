...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "15030URL", reqReturnURL
   SetCache "15030DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "15030")
...
fileLanguage = "Language" + "\15030[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\15030[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "15030 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "15030 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
s=s+   "</ITEM>"
s=s+   "<ITEM label=""mRewards"" width=""100"" menuwidth=""125"" height=""30"" menuheight=""30"">"
...
s=s+         "<LINK name=""JAVA(ShowTab('TabReward',1))""/>"
s=s+      "</ITEM>"
s=s+      "<DIVIDER/>"
s=s+      "<ITEM label=""mBonuses"">"
s=s+         "<IMAGE name=""Finance.gif"" width=""18"" height=""18""/>"
s=s+         "<LINK name=""JAVA(ShowTab('TabReward',2))""/>"
s=s+      "</ITEM>"
s=s+      "<ITEM label=""mStatements"">"
s=s+         "<IMAGE name=""Finance.gif"" width=""18"" height=""18""/>"
s=s+         "<LINK name=""JAVA(ShowTab('TabReward',3))""/>"
s=s+      "</ITEM>"
s=s+   "</ITEM>"
...
If oStyle.parseError <> 0 Then
   Response.Write "15030 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "15030 Load file (oData) failed with error code " + CStr(oData.parseError)
