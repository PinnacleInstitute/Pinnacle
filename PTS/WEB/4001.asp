...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "4001URL", reqReturnURL
   SetCache "4001DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "4001")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("4001URL")
      reqReturnData = GetCache("4001DATA")
      SetCache "4001URL", ""
      SetCache "4001DATA", ""
...
If (reqActionCode <> 0) Or (reqOrgID > 0) Or (reqCompanyID > 0) Then
   ...
   Else
      With oSurveys
         .SysCurrentLanguage = reqSysLanguage
         If (reqOrgID = 0) And (reqCompanyID = 0) Then
...
If oLanguage.parseError <> 0 Then
   Response.Write "4001 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "4001 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "4001 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "4001 Load file (oData) failed with error code " + CStr(oData.parseError)
