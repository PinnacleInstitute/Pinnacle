...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "7507URL", reqReturnURL
   SetCache "7507DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "7507")
...
Sub LoadLists()
   ...
   Else
      With oProjects
         .SysCurrentLanguage = reqSysLanguage
         xmlProjects = .EnumMember(CLng(reqProjectID), CLng(reqMemberID), , , CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
   ...
   Case CLng(cActionUpdate):
      ...
      Else
         With oProject
            ...
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (.ParentID <> 0) Then
               Result = CLng(.UpdateHierarchy(CLng(.ParentID)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (.ParentID = 0) Then
               Result = CLng(.UpdateHierarchy(CLng(.ProjectID)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (xmlError <> "") Then
               xmlProject = .XML(2)
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("7507URL")
         reqReturnData = GetCache("7507DATA")
         SetCache "7507URL", ""
         SetCache "7507DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "7507 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "7507 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "7507 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "7507 Load file (oData) failed with error code " + CStr(oData.parseError)
