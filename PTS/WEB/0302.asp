...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0302URL", reqReturnURL
   SetCache "0302DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0302")
   ...
   Case CLng(cActionAdd):
      ...
      Else
         With oTrainer
            ...
            tmpLogon = .NewLogon
            .SponsorID = reqSysAffiliateID
            
         tmpAgree = Request.Form.Item("Agree")
         ...
         End If
            ...
            If (xmlError = "") Then
               ...
               Else
                  With oAuthUser
                     .SysCurrentLanguage = reqSysLanguage
                     IsAvailable = CLng(.IsLogon(tmpLogon))
                     If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
                  End With
               End If
            ...
            End If
            If (xmlError = "") Then
               TrainerID = CLng(.Add(CLng(reqSysUserID)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
      ...
      If (xmlError = "") Then
         ...
         Else
            With oHTMLFile
               .Filename = "TrainerTemplate.htm"
               .Path = reqSysWebDirectory + "Sections\Trainer\"
               .Language = reqSysLanguage
               .Project = SysProject
               .Load 
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               .Filename = CStr(TrainerID) + ".htm"
               .Save 
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
      ...
      If (xmlError = "") And (tmpAuthUser > 0) Then
         ...
         If (reqSysUserGroup > 23) Then
            Response.Redirect "0105.asp" & "?AuthUserID=" & tmpAuthUser & "&MailReturnURL=" & "0330.asp?TrainerID=" & TrainerID
         End If

         If (reqSysUserGroup <= 23) Then
            Response.Redirect "0105.asp" & "?AuthUserID=" & tmpAuthUser & "&MailReturnURL=" & Replace(GetCache("0302URL"), "%26", "&")
         End If
      End If

      If (xmlError = "") Then
         reqReturnURL = GetCache("0302URL")
         reqReturnData = GetCache("0302DATA")
         SetCache "0302URL", ""
         SetCache "0302DATA", ""
...
fileLanguage = "Language" + "\Trainer[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\Trainer[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0302 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0302 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0302 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0302 Load file (oData) failed with error code " + CStr(oData.parseError)
