...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "12200URL", reqReturnURL
   SetCache "12200DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "12200")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      If (reqSysUserGroup = 21) Then
         If InStr( ",2,3,5,6,11,12,13,21,23,24,25,26,27,29,33,34,35,36,37,38,39,40,47,48,49,50,", CSTR(reqProcess)+"," ) = 0 Then reqProcess = 0
      End If
   ...
   Case CLng(cActionUpdate):
      If (reqProcess = 12) Or (reqProcess = 39) Or (reqProcess = 47) Then
         ...
         End If
      End If
      If (reqProcess = 2) Or (reqProcess = 11) Or (reqProcess = 21) Or (reqProcess = 24) Or (reqProcess = 25) Or (reqProcess = 28) Or (reqProcess = 48) Or (reqProcess = 49) Or (reqProcess = 50) Then
         If (reqToDate = "") Then
            DoError 10115, "", "Oops, Specify the date range to process."
         End If
      End If
      If (xmlError = "") Then
         ...
         If (reqProcess = 2) Then
            ...
            Set oSims = server.CreateObject("ptsSimsUser.CSims")
            If oSims Is Nothing Then
               DoError Err.Number, Err.Source, "Unable to Create Object - ptsSimsUser.CSims"
            Else
               With oSims
         ...
         If (reqProcess = 24) Then
            ...
            If (tmpTotal <> 0) Then
               URL = "http://www.mytrackerpro.com/payout/" + CStr(reqCompanyID) + "/" + tmpFile
         ...
         If (reqProcess = 30) Then
            ...
            Set oSimss = server.CreateObject("ptsSimsUser.CSimss")
            If oSimss Is Nothing Then
               DoError Err.Number, Err.Source, "Unable to Create Object - ptsSimsUser.CSimss"
            Else
               With oSimss
                  .SysCurrentLanguage = reqSysLanguage
                  .CustomList 3, 0, 0, 0
         ...
         If (reqProcess = 35) Then
            Total = 0
            ExportOrder reqCompanyID, 101, 1, tmpFile, tmpTotal, tmpAmount
            Total = Total + tmpTotal
            If (tmpTotal <> 0) Then
               URL = "http://www.mytrackerpro.com/orders/" + CStr(reqCompanyID) + "/" + tmpFile
               LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
               MSG = CSTR(tmpTotal) + " GFT Submitted Orders (" & FormatCurrency(tmpAmount,2) & ")  "
               reqResult = reqResult + MSG + LINK + "<br>"
               NOTE = NOTE + MSG + URL + "  " + VBLF
            End If
            ExportOrder reqCompanyID, 101, 2, tmpFile, tmpTotal, tmpAmount
            Total = Total + tmpTotal
            If (tmpTotal <> 0) Then
               URL = "http://www.mytrackerpro.com/orders/" + CStr(reqCompanyID) + "/" + tmpFile
               LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
               MSG = CSTR(tmpTotal) + " SIMS Submitted Orders (" & FormatCurrency(tmpAmount,2) & ")  "
               reqResult = reqResult + MSG + LINK + "<br>"
               NOTE = NOTE + MSG + URL + "  " + VBLF
            End If
            If (Total <> 0) Then
         ...
         If (reqProcess = 36) Then
            ...
            If (tmpTotal <> 0) Then
               URL = "http://www.mytrackerpro.com/orders/" + CStr(reqCompanyID) + "/" + tmpFile
               LINK = "<a href=""" + URL + """ target=""_blank"">" + URL + "</a>"
               MSG = CSTR(tmpTotal) + " SIMS Inprocess Orders (" & FormatCurrency(tmpAmount,2) & ")  "
               reqResult = reqResult + MSG + LINK + "<br>"
               NOTE = NOTE + MSG + URL + "  " + VBLF
            End If
            If (Total <> 0) Then
...
xmlTransaction = xmlTransaction +  xmlSimss
...
fileLanguage = "Language" + "\12200[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\12200[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "12200 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "12200 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "12200 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "12200 Load file (oData) failed with error code " + CStr(oData.parseError)
