...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "10903URL", reqReturnURL
   SetCache "10903DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "10903")
...
reqGovidID =  Numeric(GetInput("GovidID", reqPageData))
...
Sub LoadCountrys()
   ...
   Else
      With oCountrys
         ...
         xmlCountrys = .EnumCompany(CLng(reqSysCompanyID), , , CLng(reqSysUserID))
...
Sub UpdateGovid()
   On Error Resume Next
   tmpType = Request.Form.Item("GType")
   reqState = Request.Form.Item("State")
   reqCountryID = Request.Form.Item("CountryID")
   tmpCountry = ""

   If (tmpType <> 5) And (tmpType <> 10) Then
      ...
      Else
         With oCountry
            .SysCurrentLanguage = reqSysLanguage
            .Load CLng(reqCountryID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            tmpCountry = .Code
         End With
      End If
   ...
   Else
      With oGovid
         ...
         .FetchMember CLng(reqMemberID)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         frmGNumber = Request.Form.Item("GNumber")
         tmpGNumber = .GNumber
         reqGovidID = .GovidID
         If (reqGovidID = 0) Then
         ...
         .GType = Request.Form.Item("GType")
         .GNumber = Request.Form.Item("GNumber")
         ...
         If (reqGovidID = 0) Then
            GovidID = CLng(.Add(CLng(reqSysUserID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqGovidID <> 0) Then
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("10903URL")
         reqReturnData = GetCache("10903DATA")
         SetCache "10903URL", ""
         SetCache "10903DATA", ""
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oGovid
            .SysCurrentLanguage = reqSysLanguage
            .Delete CLng(reqGovidID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oGovid = Nothing
      ...
      If (xmlError <> "") Then
         ...
         Else
            With oGovid
               .SysCurrentLanguage = reqSysLanguage
               .Load CLng(reqGovidID), CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               xmlGovid = .XML(2)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
...
xmlConfig = xmlConfig + " />"
xmlParam = "<PARAM"
xmlParam = xmlParam + " govidid=" + Chr(34) + CStr(reqGovidID) + Chr(34)
...
fileLanguage = "Language" + "\Govid[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\Govid[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "10903 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "10903 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "10903 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "10903 Load file (oData) failed with error code " + CStr(oData.parseError)
