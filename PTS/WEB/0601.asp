...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0601URL", reqReturnURL
   SetCache "0601DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0601")
...
If (reqSysUserGroup = 41) And (InStr(reqSysUserOptions,"X") = 0) Then
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Set oAffiliates = server.CreateObject("ptsAffiliateUser.CAffiliates")
      If oAffiliates Is Nothing Then
         DoError Err.Number, Err.Source, "Unable to Create Object - ptsAffiliateUser.CAffiliates"
      Else
         With oAffiliates
            .SysCurrentLanguage = reqSysLanguage
            .FindTypeID = reqFindTypeID
            xmlAffiliates = .XML(14)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oAffiliates = Nothing

   Case CLng(cActionCancel):

      reqReturnURL = GetCache("0601URL")
      reqReturnData = GetCache("0601DATA")
      SetCache "0601URL", ""
      SetCache "0601DATA", ""
...
xmlTransaction = "<TXN>"
xmlTransaction = xmlTransaction +  xmlAffiliates
...
If oLanguage.parseError <> 0 Then
   Response.Write "0601 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0601 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0601 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0601 Load file (oData) failed with error code " + CStr(oData.parseError)
