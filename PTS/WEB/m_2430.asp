...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_2430URL", reqReturnURL
   SetCache "m_2430DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_2430")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oAttachments
            .SysCurrentLanguage = reqSysLanguage
            .ListActiveAttachments CLng(reqLessonID), 23, tmpSecurityLevel, CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      ...
      Else
         With oLesson
            ...
            If (reqSysUserGroup = 41) Or (reqSysUserGroup = 99) Then
               ...
               If (curQuiz = 3) Then
                  Response.Redirect "m_2112.asp" & "?LessonID=" & reqLessonID & "&SessionID=" & reqSessionID & "&CourseID=" & reqCourseID & "&Lessons=" & reqLessons & "&AutoQuiz=" & 1
               End If
            End If
   ...
   Case CLng(cActionTakeQuiz):
      ...
      Response.Redirect "m_2112.asp" & "?LessonID=" & reqLessonID & "&SessionID=" & reqSessionID & "&CourseID=" & reqCourseID & "&Lessons=" & reqLessons

   Case CLng(cActionReviewQuiz):
      UpdateStatus

      Response.Redirect "m_2112.asp" & "?LessonID=" & reqLessonID & "&SessionID=" & reqSessionID & "&CourseID=" & reqCourseID & "&Review=" & 1 & "&Lessons=" & reqLessons
   ...
   Case CLng(cActionNextLesson):
      ...
      If (reqNextLessonID > 0) Then
         Response.Redirect "m_2430.asp" & "?LessonID=" & reqNextLessonID & "&SessionID=" & reqSessionID & "&CourseID=" & reqCourseID & "&Lessons=" & reqLessons & "&MemberID=" & reqMemberID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup
      End If
   ...
   Case CLng(cActionReturnClass):
      ...
      Response.Redirect "m_1330.asp" & "?SessionID=" & reqSessionID & "&CourseID=" & reqCourseID & "&MemberID=" & reqMemberID & "&Lessons=" & reqLessons & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup
End Select
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_2430 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_2430 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_2430 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_2430 Load file (oData) failed with error code " + CStr(oData.parseError)
