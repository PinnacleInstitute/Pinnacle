...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "13201URL", reqReturnURL
   SetCache "13201DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "13201")
...
reqIsDonate =  Numeric(GetInput("IsDonate", reqPageData))
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      ...
      reqIsDonate = Request.Form.Item("IsDonate")
      If (reqIsAgree = 0) Then
         DoError 10103, "", "Oops, You must agree to the terms and conditions."
      End If
      If (reqGroupID <> 0) And (reqIsDonate = 0) Then
         DoError -1, "", "Oops, You must agree to the donation percentage."
      End If
      If (xmlError = "") Then
         ValidateContact
      End If
...
xmlParam = xmlParam + " isdonate=" + Chr(34) + CStr(reqIsDonate) + Chr(34)
...
fileLanguage = "Language" + "\13201[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\13201[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "13201 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "13201 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "13201 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "13201 Load file (oData) failed with error code " + CStr(oData.parseError)
