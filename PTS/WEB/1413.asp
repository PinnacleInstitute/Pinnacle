...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1413URL", reqReturnURL
   SetCache "1413DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1413")
...
reqPage =  GetInput("Page", reqPageData)
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If (tmpPage <> "") And (reqMemberID <> 0) Then
         ...
         Else
            ...
            pos = InStr( tmpPage, ".")
            If pos > 0 Then
               reqPage = .Logon + "." + tmpPage
            Else
               pos = InStrRev( reqSysServerName, ".")
               If pos > 0 Then
                  pos = InStrRev( reqSysServerName, ".", pos-1)
                  If pos > 0 Then
                     tmp = Mid( reqSysServerName, pos )
                     reqPage = .Logon + tmp + reqSysServerPath + tmpPage
                  End If
               End If
            End If
      ...
      Else
         With oLeadPages
            .SysCurrentLanguage = reqSysLanguage
            .Languages CLng(reqLeadCampaignID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            
            For Each oLeadPage in oLeadPages
               With oLeadPage
                  If tmpPageType = 2 Then  'Presentation Site
                     tmp = "http://" + reqSysServerName + reqSysServerPath + "pp.asp?p=" & reqLeadCampaignID
                  Else
                     tmp = "http://" + reqSysServerName + reqSysServerPath + "lp.asp?p=" & reqLeadCampaignID
                  End If   
                  If .Language <> "en" Then tmp = tmp + "&l=" + .Language
                  If reqMemberID > 0 Then tmp = tmp + "&m=" & reqMemberID
                  If reqAffiliateID > 0 Then tmp = tmp + "&a=" & reqAffiliateID
                  .Inputs = tmp
               End With
            Next
      ...
      Else
         With oHTMLFile
            If (reqMemberID <> 0) Then
               .Filename = "HelpLead.htm"
            End If
            If (reqMemberID = 0) Then
               .Filename = "HelpLeadAdmin.htm"
            End If
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("1413URL")
      reqReturnData = GetCache("1413DATA")
      SetCache "1413URL", ""
      SetCache "1413DATA", ""
   ...
   Case CLng(cActionLogon):
      ...
      Response.Redirect "0103.asp" & "?ReturnURL=" & reqPageURL
End Select
...
xmlTransaction = xmlTransaction +  xmlHTMLFilePromo
...
If oLanguage.parseError <> 0 Then
   Response.Write "1413 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1413 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1413 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1413 Load file (oData) failed with error code " + CStr(oData.parseError)
