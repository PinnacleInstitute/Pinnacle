...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "9701URL", reqReturnURL
   SetCache "9701DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "9701")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("9701URL")
      reqReturnData = GetCache("9701DATA")
      SetCache "9701URL", ""
      SetCache "9701DATA", ""
...
If (reqActionCode <> 0) Then
   ...
   Else
      With oMsgs
         .SysCurrentLanguage = reqSysLanguage
         reqBookmark = .Find(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqOwnerType), CLng(reqOwnerID), CDate(reqFromDate), tmpToDate, CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         xmlMsgs = .XML(15)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
...
xmlTransaction = "<TXN>"
xmlTransaction = xmlTransaction +  xmlMsgs
...
If oLanguage.parseError <> 0 Then
   Response.Write "9701 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "9701 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "9701 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "9701 Load file (oData) failed with error code " + CStr(oData.parseError)
