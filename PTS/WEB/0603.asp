...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0603URL", reqReturnURL
   SetCache "0603DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0603")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      If (xmlError = "") And (reqPopup = 0) Then
         reqReturnURL = GetCache("0603URL")
         reqReturnData = GetCache("0603DATA")
         SetCache "0603URL", ""
         SetCache "0603DATA", ""
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oAffiliate
            ...
            .Load CLng(reqAffiliateID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            .Delete CLng(reqAffiliateID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oAffiliate = Nothing
...
If oLanguage.parseError <> 0 Then
   Response.Write "0603 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0603 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
s = "<TAB name=""AffiliateTab"">"
s=s+   "<ITEM label=""Status"" width=""145"">"
s=s+      "<LINK name=""JAVA(ShowTab('TabStatus',1))""/>"
s=s+   "</ITEM>"
s=s+   "<ITEM label=""Contact"" width=""145"">"
s=s+      "<LINK name=""JAVA(ShowTab('TabContact',1))""/>"
s=s+   "</ITEM>"
s=s+   "<ITEM label=""Terms"" width=""145"">"
s=s+      "<LINK name=""JAVA(ShowTab('TabTerms',1))""/>"
s=s+   "</ITEM>"
      If (reqSysUserGroup <> 61) Then
s=s+   "<ITEM label=""Notes"" width=""145"">"
...
xmlData = xmlData +  xmlAffiliateTab
...
If oStyle.parseError <> 0 Then
   Response.Write "0603 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0603 Load file (oData) failed with error code " + CStr(oData.parseError)
