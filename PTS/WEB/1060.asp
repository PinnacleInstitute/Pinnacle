...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1060URL", reqReturnURL
   SetCache "1060DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1060")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oPayments
            If (reqReport = 1) Then
               .ReportStatusAmount CDate(reqFromDate), CDate(reqToDate), reqOption
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqReport = 2) Then
               .ReportStatusCount CDate(reqFromDate), CDate(reqToDate)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqReport = 3) Then
               .ReportPayTypeAmount 0, CDate(reqFromDate), CDate(reqToDate)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqReport = 4) Then
               .ReportPayTypeCount 0, CDate(reqFromDate), CDate(reqToDate)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
...
fileLanguage = "Language\1060.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "1060 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1060 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1060 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1060 Load file (oData) failed with error code " + CStr(oData.parseError)
