...
pos = InStr(LCASE(reqSysServerPath), "0105")
...
If oStyle.parseError <> 0 Then
   Response.Write "0105 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If (tmpMemberID > 0) And (reqCompanyID > 0) Then
...
fileLanguage = "Language" + "\0105[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\0105[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0105 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0105 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0105 Load file (oData) failed with error code " + CStr(oData.parseError)
