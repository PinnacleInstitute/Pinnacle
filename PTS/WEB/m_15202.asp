...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_15202URL", reqReturnURL
   SetCache "m_15202DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_15202")
...
Sub NewReward()
   ...
   Else
      With oAwards
         .SysCurrentLanguage = reqSysLanguage
         xmlAwards = xmlAwards + .EnumMerchant(CLng(reqMerchantID), 1, , , CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
   Set oAwards = Nothing
...
Sub AddReward()
   ...
   Else
      With oReward
         ...
         Rate = CCUR(AwardAmount) / 100
         tmpAmount = Round(reqPurchase * Rate,2)
         tmpNote = CSTR(reqPurchase) + " * " + CSTR(Rate) + "% "
         .Note = tmpNote + .Note
         ...
         If (xmlError <> "") Then
            ...
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (xmlError = "") And (IsAwards <> 0) Then
            .RewardType = 2
            RewardID = CLng(.Add(CLng(reqSysUserID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("m_15202URL")
      reqReturnData = GetCache("m_15202DATA")
      SetCache "m_15202URL", ""
      SetCache "m_15202DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_15202 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_15202 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_15202 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_15202 Load file (oData) failed with error code " + CStr(oData.parseError)
