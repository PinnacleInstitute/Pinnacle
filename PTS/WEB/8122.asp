...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "8122URL", reqReturnURL
   SetCache "8122DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "8122")
...
reqGroup =  Numeric(GetInput("Group", reqPageData))
reqSalesCampaignID =  Numeric(GetInput("SalesCampaignID", reqPageData))
reqCount =  Numeric(GetInput("Count", reqPageData))
reqPotential =  Numeric(GetInput("Potential", reqPageData))
reqCurrent =  Numeric(GetInput("Current", reqPageData))
...
Sub LoadReport()
   ...
   If (reqMemberID <> 0) Then
      ...
      Else
         With oMember
            ...
            If (InStr(reqSysUserOptions,"]") <> 0) Or (reqSysUserGroup <= 23) Or (reqSysUserGroup = 51) Or (reqSysUserGroup = 52) Then
   ...
   Else
      With oSalesCampaigns
         .List CLng(reqCompanyID)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         
               cnt = 0
               For Each oSalesCampaign in oSalesCampaigns
                  With oSalesCampaign
                     tmp = .SalesCampaignID
                     cnt = cnt + 1
                  End With 
               Next
               If cnt = 1 Then reqSalesCampaignID = tmp

         tmpGroupID = GetCache("GROUPID")
         xmlSalesCampaigns = .EnumCompany(CLng(reqCompanyID), tmpGroupID, CLng(reqSalesCampaignID), , CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
   Set oSalesCampaigns = Nothing

   If (reqSalesCampaignID <> 0) Then
      ...
      Else
         With oProspects
            If (reqMemberID = 0) Then
               .BoardCompany CLng(reqCompanyID), CLng(reqSalesCampaignID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqMemberID <> 0) Then
               If (reqGroup = 0) Then
                  .BoardMember CLng(reqMemberID), CLng(reqSalesCampaignID)
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               End If
               If (reqGroup <> 0) Then
                  .BoardGroup reqGroupID, CLng(reqSalesCampaignID)
                  If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               End If
            End If
            xmlProspects = .XML(13)
...
xmlParam = xmlParam + " group=" + Chr(34) + CStr(reqGroup) + Chr(34)
...
xmlParam = xmlParam + " potential=" + Chr(34) + CStr(reqPotential) + Chr(34)
xmlParam = xmlParam + " current=" + Chr(34) + CStr(reqCurrent) + Chr(34)
xmlParam = xmlParam + " />"
...
fileLanguage = "Language\8122.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "8122 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8122 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "8122 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "8122 Load file (oData) failed with error code " + CStr(oData.parseError)
