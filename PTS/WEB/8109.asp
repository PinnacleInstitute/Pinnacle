...
pos = InStr(LCASE(reqSysServerPath), "8109")
...
reqLink =  GetInput("Link", reqPageData)
...
If oStyle.parseError <> 0 Then
   Response.Write "8109 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oLeadPage
            .SysCurrentLanguage = reqSysLanguage
            .FetchEmail tmpLeadCampaignID, reqSysLanguage
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      ...
      Else
         With oMember
            ...
            If (tmpLeadCampaignID <> 0) Then
               Signature = .GetSignature(tmpMemberID, 4, 0, reqSysLanguage)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               If (Signature <> "") Then
                  tmpSignature = Signature
               End If
      ...
      reqLink = "http://" + reqSysServerName + reqSysServerPath + "lp.asp?p=" & tmpLeadCampaignID & "&m=" & tmpMemberID & "&r=" & reqProspectID 
      xmlHTMLFile = Replace( xmlHTMLFile, "{link}", reqLink )

      tmpSubject = tmpProspectName
...
fileLanguage = "Language" + "\8109[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\8109[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "8109 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "8109 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
xmlParam = xmlParam + " link=" + Chr(34) + CleanXML(reqLink) + Chr(34)
xmlParam = xmlParam + " />"
...
If oData.parseError <> 0 Then
   Response.Write "8109 Load file (oData) failed with error code " + CStr(oData.parseError)
