...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0303URL", reqReturnURL
   SetCache "0303DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0303")
...
Sub UpdateTrainer()
   ...
   Else
      With oTrainer
         ...
         .Load CLng(reqTrainerID), CLng(reqSysUserID)
   ...
   Else
      With oHTMLFile
         .Filename = CStr(reqTrainerID) + ".htm"
...
If (reqSysUserGroup > 31) Then
   ...
   reqReturnURL = GetCache("0303URL")
   reqReturnData = GetCache("0303DATA")
   SetCache "0303URL", ""
   SetCache "0303DATA", ""
...
If (reqUploadImageFile <> "") Then
   reqTrainerID = GetCache("TRAINERID")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oTrainer
            .SysCurrentLanguage = reqSysLanguage
            .Delete CLng(reqTrainerID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oTrainer = Nothing
   ...
   Case CLng(cActionUploadImage):
      ...
      If (xmlError = "") Then
         Response.Redirect "0320.asp" & "?TrainerID=" & reqTrainerID
      End If
End Select
...
If oLanguage.parseError <> 0 Then
   Response.Write "0303 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0303 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0303 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0303 Load file (oData) failed with error code " + CStr(oData.parseError)
