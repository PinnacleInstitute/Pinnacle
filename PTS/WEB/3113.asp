...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "3113URL", reqReturnURL
   SetCache "3113DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "3113")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      ...
      If (xmlError = "") Then
         ...
         If (tmpAssessmentType = 1) Then
            Response.Redirect "3212.asp" & "?AssessmentID=" & reqAssessmentID & "&MemberAssessID=" & reqMemberAssessID & "&New=" & 1 & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup & "&ReturnURL=" & GetCache("3113URL")
         End If

         If (tmpAssessmentType >= 3) Then
            Response.Redirect "3414.asp" & "?AssessmentID=" & reqAssessmentID & "&MemberAssessID=" & reqMemberAssessID & "&MemberID=" & reqMemberID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup & "&ReturnURL=" & GetCache("3113URL")
         End If
      End If
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("3113URL")
      reqReturnData = GetCache("3113DATA")
      SetCache "3113URL", ""
      SetCache "3113DATA", ""
   ...
   Case CLng(cActionReturn):
      ...
      Response.Redirect "3411.asp" & "?MemberID=" & reqMemberID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
End Select
...
If oLanguage.parseError <> 0 Then
   Response.Write "3113 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "3113 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "3113 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "3113 Load file (oData) failed with error code " + CStr(oData.parseError)
