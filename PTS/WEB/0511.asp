...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0511URL", reqReturnURL
   SetCache "0511DATA", reqReturnData
End If
...
pos = InStr(reqSysServerPath, "0511")
...
CheckSecurity reqSysUserID, reqSysUserGroup, 1, 22
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oMails
            .SysCurrentLanguage = reqSysLanguage
            .ListSalesCampaign CLng(reqSalesCampaignID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlMails = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("0511URL")
      reqReturnData = GetCache("0511DATA")
      SetCache "0511URL", ""
      SetCache "0511DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "0511 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0511 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0511 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0511 Load file (oData) failed with error code " + CStr(oData.parseError)
