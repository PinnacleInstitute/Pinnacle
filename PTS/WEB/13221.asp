...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "13221URL", reqReturnURL
   SetCache "13221DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "13221")
...
reqIsGroup =  Numeric(GetInput("IsGroup", reqPageData))
...
Sub GetImage()
   On Error Resume Next
               ...
               FileName = "Company\" + CStr(reqCompanyID) + "\Logo" + CSTR(reqGroupID)
   ...
   Else
      With oMember
         ...
         If (reqIsGroup <> 0) Then
            ...
            GetImage
         End If
         If (reqIsGroup = 0) And (oldIsGroup <> 0) Then
            .IsCompany = 0
            .CompanyName = .NameLast + ", " + .NameFirst
            .GroupID = .Referral
         End If
         SetCache "GROUPID", .GroupID
         SetCache "LOGO", ""
         If (xmlError = "") Then
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUploadImage):
      ...
      If (xmlError = "") Then
         ...
         Response.Redirect "13222.asp" & "?ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
      End If
End Select
...
xmlParam = xmlParam + " isgroup=" + Chr(34) + CStr(reqIsGroup) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "13221 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "13221 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "13221 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "13221 Load file (oData) failed with error code " + CStr(oData.parseError)
