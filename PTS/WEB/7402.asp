...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "7402URL", reqReturnURL
   SetCache "7402DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "7402")
...
Sub ListMembers()
   ...
   Else
      With oProjectMembers
         .SysCurrentLanguage = reqSysLanguage
         xmlProjectMembers = xmlProjectMembers + .EnumMember(CLng(reqProjectID), CLng(reqMemberID), , CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
   ...
   Case CLng(cActionAdd):
      ...
      Else
         With oTask
            ...
            TaskID = CLng(.Add(CLng(reqSysUserID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (xmlError <> "") Then
               xmlTask = .XML(2)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("7402URL")
         reqReturnData = GetCache("7402DATA")
         SetCache "7402URL", ""
         SetCache "7402DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "7402 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "7402 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "7402 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "7402 Load file (oData) failed with error code " + CStr(oData.parseError)
