...
Const cActionYes = 1
Const cActionNo = 2
Const cActionMaybe = 4
Const cActionShopping = 5
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "2506URL", reqReturnURL
   SetCache "2506DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "2506")
...
reqAddress =  GetInput("Address", reqPageData)
reqIsShop =  Numeric(GetInput("IsShop", reqPageData))
...
Sub LoadInvitation()
   ...
   If (reqG <> 0) Then
      Set oGuest = server.CreateObject("ptsGuestUser.CGuest")
      If oGuest Is Nothing Then
         DoError Err.Number, Err.Source, "Unable to Create Object - ptsGuestUser.CGuest"
      Else
         With oGuest
   ...
   Else
      With oParty
         ...
         reqMemberID = CLng(.FetchMemberID(reqP))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
   Set oParty = Nothing
   ...
   Else
      With oHTMLFile
         .Filename = "Invitation" & tmpTheme & ".htm"
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionYes):
      UpdateGuest(3)

   Case CLng(cActionNo):
      UpdateGuest(4)

   Case CLng(cActionMaybe):
      UpdateGuest(5)

   Case CLng(cActionShopping):

      Response.Redirect "5250.asp" & "?C=" & reqSysCompanyID & "&SP=" & reqMemberID & "&PA=" & reqP & "&N=" & 1 & "&ReturnURL=" & reqPageURL

   Case CLng(cActionReturn):

      reqReturnURL = GetCache("2506URL")
      reqReturnData = GetCache("2506DATA")
      SetCache "2506URL", ""
      SetCache "2506DATA", ""
...
xmlParam = xmlParam + " address=" + Chr(34) + CleanXML(reqAddress) + Chr(34)
xmlParam = xmlParam + " isshop=" + Chr(34) + CStr(reqIsShop) + Chr(34)
xmlParam = xmlParam + " memberid=" + Chr(34) + CStr(reqMemberID) + Chr(34)
xmlParam = xmlParam + " />"

'-----get the transaction XML
xmlTransaction = "<TXN>"
xmlTransaction = xmlTransaction +  xmlGuest
...
fileLanguage = "Language" + "\Party[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\Party[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "2506 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "2506 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "2506 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "2506 Load file (oData) failed with error code " + CStr(oData.parseError)
