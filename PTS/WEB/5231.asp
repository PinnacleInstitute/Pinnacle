...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "5231URL", reqReturnURL
   SetCache "5231DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "5231")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oSalesOrders
            .SysCurrentLanguage = reqSysLanguage
            .ListProspect CLng(reqProspectID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlSalesOrders = xmlSalesOrders + .XML(13)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("5231URL")
      reqReturnData = GetCache("5231DATA")
      SetCache "5231URL", ""
      SetCache "5231DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "5231 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "5231 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "5231 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "5231 Load file (oData) failed with error code " + CStr(oData.parseError)
