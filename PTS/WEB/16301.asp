...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "16301URL", reqReturnURL
   SetCache "16301DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "16301")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If oStatements Is Nothing Then
         DoError Err.Number, Err.Source, "Unable to Create Object - ptsStatementUser.CStatements"
      Else
         With oStatements
            .SysCurrentLanguage = reqSysLanguage
            .FindTypeID = reqFindTypeID
            xmlStatements = .XML(14)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oStatements = Nothing

      If (reqMerchantID <> 0) And (reqTitle = "") Then
...
Else
   With oStatements
      ...
      If (reqMerchantID = 0) Then
         ...
         End If
         If (reqStatus <> 0) Then
            reqBookmark = .FindStatus(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqStatus), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
      End If
      If (reqMerchantID <> 0) Then
         ...
         End If
         If (reqStatus <> 0) Then
            reqBookmark = .FindMerchantStatus(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMerchantID), CLng(reqStatus), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
      End If
      xmlStatements = .XML(15)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
End If
Set oStatements = Nothing
...
xmlTransaction = "<TXN>"
xmlTransaction = xmlTransaction +  xmlStatements
...
If oLanguage.parseError <> 0 Then
   Response.Write "16301 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "16301 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "16301 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "16301 Load file (oData) failed with error code " + CStr(oData.parseError)
