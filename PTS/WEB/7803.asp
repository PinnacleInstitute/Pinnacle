...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "7803URL", reqReturnURL
   SetCache "7803DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "7803")
...
reqSalesStepID =  Numeric(GetInput("SalesStepID", reqPageData))
...
Sub LoadSalesStep()
   ...
   Else
      With oSalesStep
         ...
         .Load CLng(reqSalesStepID), CLng(reqSysUserID)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         reqSalesCampaignID = .SalesCampaignID
         reqEmailID = .EmailID
         xmlSalesStep = .XML(2)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
...
Sub LoadAttachments()
   ...
   Else
      With oAttachments
         .SysCurrentLanguage = reqSysLanguage
         .ListAttachments CLng(reqSalesStepID), 78, CLng(reqSysUserID)
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   ...
   Else
      With oEmails
         .SysCurrentLanguage = reqSysLanguage
         xmlEmails = .EnumProspect(reqCompanyID, CLng(reqEmailID), , CLng(reqSysUserID))
         If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      End With
   End If
   Set oEmails = Nothing
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionUpdate):
      ...
      Else
         With oSalesStep
            ...
            .SalesStepName = Request.Form.Item("SalesStepName")
            .Description = Request.Form.Item("Description")
            .Seq = Request.Form.Item("Seq")
            .Length = Request.Form.Item("Length")
            .DateNo = Request.Form.Item("DateNo")
            .IsBoard = Request.Form.Item("IsBoard")
            .BoardRate = Request.Form.Item("BoardRate")
            .AutoStep = Request.Form.Item("AutoStep")
            .EmailID = Request.Form.Item("EmailID")
            .Email = Request.Form.Item("Email")
            .Delay = Request.Form.Item("Delay")
            .NextStep = Request.Form.Item("NextStep")
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("7803URL")
         reqReturnData = GetCache("7803DATA")
         SetCache "7803URL", ""
         SetCache "7803DATA", ""
   ...
   Case CLng(cActionDelete):
      ...
      Else
         With oSalesStep
            .SysCurrentLanguage = reqSysLanguage
            .Delete CLng(reqSalesStepID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oSalesStep = Nothing
...
xmlConfig = xmlConfig + " />"
xmlParam = "<PARAM"
xmlParam = xmlParam + " salesstepid=" + Chr(34) + CStr(reqSalesStepID) + Chr(34)
...
fileLanguage = "Language" + "\SalesStep[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\SalesStep[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "7803 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "7803 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "7803 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "7803 Load file (oData) failed with error code " + CStr(oData.parseError)
