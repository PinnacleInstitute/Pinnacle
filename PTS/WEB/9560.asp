...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "9560URL", reqReturnURL
   SetCache "9560DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "9560")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oIssues
            If (reqReport = 1) Then
               .ReportStatus CLng(reqCompanyID), reqFromDate, reqToDate
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (reqReport = 2) Then
               .ReportStatusAssigned CLng(reqCompanyID), reqFromDate, reqToDate, reqAssigned
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
...
fileLanguage = "Language\9560.xml"
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "9560 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "9560 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "9560 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "9560 Load file (oData) failed with error code " + CStr(oData.parseError)
