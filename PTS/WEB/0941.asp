...
Dim oCommType, xmlCommType
Dim oCommissions, xmlCommissions
...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0941URL", reqReturnURL
   SetCache "0941DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0941")
...
reqCommType =  Numeric(GetInput("CommType", reqPageData))
...
Else
   With oPayout
      ...
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      tmpMemberID = .OwnerID
      If (reqSysUserGroup = 41) And (tmpMemberID <> reqSysMemberID) Then
         AbortUser()
      End If
      xmlPayout = .XML(2)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
Set oCommType = server.CreateObject("ptsCommTypeUser.CCommType")
If oCommType Is Nothing Then
   DoError Err.Number, Err.Source, "Unable to Create Object - ptsCommTypeUser.CCommType"
Else
   With oCommType
      .SysCurrentLanguage = reqSysLanguage
      .FetchCommTypeNo tmpCompanyID, CLng(reqCommType)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      xmlCommType = .XML(2)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
End If
Set oCommType = Nothing
...
Else
   With oCommissions
      .SysCurrentLanguage = reqSysLanguage
      reqBookmark = .FindPayout(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqPayoutID), CLng(reqCommType), CLng(reqSysUserID))
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
      xmlCommissions = .XML(15)
      If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
   End With
End If
Set oCommissions = Nothing
...
xmlParam = xmlParam + " commtype=" + Chr(34) + CStr(reqCommType) + Chr(34)
xmlParam = xmlParam + " />"
...
xmlTransaction = xmlTransaction +  xmlCommType
...
fileLanguage = "Language" + "\Commission[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\Commission[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "0941 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0941 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0941 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0941 Load file (oData) failed with error code " + CStr(oData.parseError)
