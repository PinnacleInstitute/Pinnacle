...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "5005URL", reqReturnURL
   SetCache "5005DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "5005")
...
Sub LoadInputOptions()
   ...
   If (reqProspectTypeID <> 0) Then
      ...
      Else
         With oProspectType
            .SysCurrentLanguage = reqSysLanguage
            .Load reqProspectTypeID, CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            reqInputOptions = .InputOptions
            xmlProspectType = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
If oLanguage.parseError <> 0 Then
   Response.Write "5005 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "5005 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "5005 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "5005 Load file (oData) failed with error code " + CStr(oData.parseError)
