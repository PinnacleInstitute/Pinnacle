...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "0107URL", reqReturnURL
   SetCache "0107DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "0107")
...
Sub Load2FA()
   ...
   Else
      With oAuthUser
         ...
         reqUser = .NameFirst + " " + .NameLast + " (" + .Logon + ")"
         If (.UserKey <> "") Then
            reqMode = 1
         End If
         xmlAuthUser = .XML()
   ...
   If (reqMode = 0) Then
      ...
      If (reqSysUserGroup = 41) Or (reqSysUserGroup = 51) Or (reqSysUserGroup = 52) Then
      ...
      If (xmlError = "") Then
         ...
         Else
            With oAuthUser
               ...
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               .UserKey = reqUserKey
               .UpdateUserKey CLng(reqSysUserID)
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
         Set oAuthUser = Nothing
...
If oLanguage.parseError <> 0 Then
   Response.Write "0107 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "0107 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "0107 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "0107 Load file (oData) failed with error code " + CStr(oData.parseError)
