...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "7505URL", reqReturnURL
   SetCache "7505DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "7505")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oProjectMembers
            ...
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlProjectMembers = xmlProjectMembers + .XML(13)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
   ...
   Case CLng(cActionCopy):
      ...
      End If
      Set oProjectMembers = Nothing
      tmpProjectName = Request.Form.Item("ProjectName")
      ...
      Else
         With oProject
            .SysCurrentLanguage = reqSysLanguage
            NewProjectID = CLng(.Copy(CLng(reqProjectID), CLng(reqCompanyID), CLng(reqMemberID), tmpProjectName, tmpExclude, tmpAssignID, CLng(reqSysUserID)))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            Result = CLng(.UpdateHierarchy(NewProjectID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
      Set oProject = Nothing
      ...
      If (xmlError = "") Then
         Response.Redirect "7503.asp" & "?ProjectID=" & NewProjectID & "&ReturnURL=" & GetCache("7505URL")
      End If
End Select
...
If oLanguage.parseError <> 0 Then
   Response.Write "7505 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "7505 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "7505 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "7505 Load file (oData) failed with error code " + CStr(oData.parseError)
