...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "5214URL", reqReturnURL
   SetCache "5214DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "5214")
...
reqTax =  Numeric(GetInput("Tax", reqPageData))
...
Sub LoadReport()
   ...
   Else
      With oSalesOrders
         ...
         .ReportCompany CLng(reqCompanyID), reqFromDate, tmpToDate
...
xmlParam = xmlParam + " tax=" + Chr(34) + CStr(reqTax) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "5214 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "5214 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "5214 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "5214 Load file (oData) failed with error code " + CStr(oData.parseError)
