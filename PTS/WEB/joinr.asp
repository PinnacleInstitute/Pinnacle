...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "joinrURL", reqReturnURL
   SetCache "joinrDATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "joinr")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oAddress
            .SysCurrentLanguage = reqSysLanguage
            .FetchOwner 4, CLng(reqMemberID), 2
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            xmlAddress = .XML()
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
...
fileLanguage = "Language" + "\joinr[" + reqSysLanguage + "].xml"
If reqSysLanguage <> "en" Then
   If Not FileExists( reqSysWebDirectory + fileLanguage ) Then fileLanguage = "Language\joinr[en].xml"
End If
Set oLanguage = server.CreateObject("MSXML2.FreeThreadedDOMDocument")
oLanguage.load server.MapPath(fileLanguage)
If oLanguage.parseError <> 0 Then
   Response.Write "joinr Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "joinr Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "joinr Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "joinr Load file (oData) failed with error code " + CStr(oData.parseError)
