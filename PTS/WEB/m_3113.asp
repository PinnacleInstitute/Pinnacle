...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_3113URL", reqReturnURL
   SetCache "m_3113DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_3113")
...
reqErrno =  Numeric(GetInput("Errno", reqPageData))
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oAssessment
            ...
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (.Takes > 0) Then
               Result = CLng(.TestTakes(CLng(reqAssessmentID), CLng(reqMemberID), CLng(.Takes)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               If (Result <> 0) Then
                  reqMode = 1
                  reqErrno = 1
            ...
            End If
            If (.Delay > 0) Then
               Result = CLng(.TestDelay(CLng(reqAssessmentID), CLng(reqMemberID), CLng(.Delay)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               If (Result <> 0) Then
                  reqMode = 1
                  reqErrno = 2
   ...
   Case CLng(cActionAdd):
      ...
      If (xmlError = "") Then
         ...
         Else
            With oMemberAssess
               ...
               .StartDate = Now
               .CommStatus = 1
               reqMemberAssessID = CLng(.Add(CLng(reqSysUserID)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End With
         End If
      ...
      If (xmlError = "") Then
         ...
         If (tmpAssessmentType = 1) Then
            Response.Redirect "m_3212.asp" & "?AssessmentID=" & reqAssessmentID & "&MemberAssessID=" & reqMemberAssessID & "&New=" & 1 & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup & "&ReturnURL=" & GetCache("m_3113URL")
         End If

         If (tmpAssessmentType >= 3) Then
            Response.Redirect "m_3414.asp" & "?AssessmentID=" & reqAssessmentID & "&MemberAssessID=" & reqMemberAssessID & "&MemberID=" & reqMemberID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup & "&ReturnURL=" & GetCache("m_3113URL")
         End If
      End If
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("m_3113URL")
      reqReturnData = GetCache("m_3113DATA")
      SetCache "m_3113URL", ""
      SetCache "m_3113DATA", ""
   ...
   Case CLng(cActionReturn):
      ...
      Response.Redirect "m_3411.asp" & "?MemberID=" & reqMemberID & "&ContentPage=" & reqContentPage & "&Popup=" & reqPopup & "&ReturnURL=" & reqPageURL & "&ReturnData=" & reqPageData
End Select
...
xmlParam = xmlParam + " errno=" + Chr(34) + CStr(reqErrno) + Chr(34)
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_3113 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_3113 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_3113 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_3113 Load file (oData) failed with error code " + CStr(oData.parseError)
