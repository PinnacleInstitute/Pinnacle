...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_1116URL", reqReturnURL
   SetCache "m_1116DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_1116")
...
reqFavoritesName =  GetInput("FavoritesName", reqPageData)
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oCourses
            .SysCurrentLanguage = reqSysLanguage
            .ListFavorite CLng(reqOrgID), CLng(reqSysUserID)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
...
xmlParam = xmlParam + " favoritesname=" + Chr(34) + CleanXML(reqFavoritesName) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_1116 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_1116 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_1116 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_1116 Load file (oData) failed with error code " + CStr(oData.parseError)
