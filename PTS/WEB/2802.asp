...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "2802URL", reqReturnURL
   SetCache "2802DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "2802")
...
reqHierarchy =  GetInput("Hierarchy", reqPageData)
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionAdd):
      ...
      If (reqParentID <> 0) Then
         ...
         Else
            With oOrg
               ...
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
               reqLevel = .Level + 1
               reqHierarchy = .Hierarchy
               tmpPrivateID = .PrivateID
            End With
         End If
         Set oOrg = Nothing
      End If
      ...
      Else
         ...
         End If
            ...
            If (xmlError = "") Then
               reqOrgID = CLng(.Add(CLng(reqSysUserID)))
               If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            End If
            If (xmlError <> "") Then
               xmlOrg = .XML(2)
            ...
            If (xmlError = "") Then
               ...
               If (reqParentID <> 0) Then
                  .Level = reqLevel
                  .Hierarchy = reqHierarchy & reqOrgID & "/"
               End If
               If (.IsPublic = 0) Then
                  .PrivateID = .OrgID
               End If
               If (.IsPublic <> 0) Then
                  .PrivateID = tmpPrivateID
               End If
      ...
      If (xmlError = "") Then
         If (tmpAuthUser > 0) Then
            ...
            If (reqParentID > 0) Then
               Response.Redirect "0105.asp" & "?AuthUserID=" & tmpAuthUser & "&MailReturnURL=" & "2830.asp"
            End If

            If (reqParentID = 0) Then
               Response.Redirect "0105.asp" & "?AuthUserID=" & tmpAuthUser & "&MailReturnURL=" & "2809.asp?CompanyID=" & reqOrgID
            End If
         End If
         If (tmpAuthUser = 0) Then
            ...
            If (reqParentID = 0) Then
               Response.Redirect "2809.asp" & "?CompanyID=" & reqOrgID
            End If
         End If
      End If
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("2802URL")
      reqReturnData = GetCache("2802DATA")
      SetCache "2802URL", ""
      SetCache "2802DATA", ""
...
xmlParam = xmlParam + " hierarchy=" + Chr(34) + CleanXML(reqHierarchy) + Chr(34)
xmlParam = xmlParam + " />"
...
If oLanguage.parseError <> 0 Then
   Response.Write "2802 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "2802 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "2802 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "2802 Load file (oData) failed with error code " + CStr(oData.parseError)
