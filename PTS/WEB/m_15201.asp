...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_15201URL", reqReturnURL
   SetCache "m_15201DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_15201")
...
reqRedeem =  Numeric(GetInput("Redeem", reqPageData))
...
m_CheckSecurity reqSysUserID, reqSysUserGroup, 0, 0, "m_15201.asp"
...
Else
   With oRewards
      ...
      If (reqMerchantID <> 0) Then
         If (reqRedeem = 1) Then
            reqBookmark = .FindMerchant(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMerchantID), CLng(reqRewardType), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqRedeem = 2) Then
            reqBookmark = .FindMerchantRedeem(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqMerchantID), CLng(reqRewardType), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
      End If
      If (reqConsumerID <> 0) Then
         If (reqRedeem = 1) Then
            reqBookmark = .FindConsumer(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqConsumerID), CLng(reqRewardType), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
         If (reqRedeem = 2) Then
            reqBookmark = .FindConsumerRedeem(reqFindTypeID, reqBookmark, reqSearchText, reqDirection, CLng(reqConsumerID), CLng(reqRewardType), CLng(reqSysUserID))
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End If
      End If
      xmlRewards = .XML(15)
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_15201 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_15201 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_15201 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_15201 Load file (oData) failed with error code " + CStr(oData.parseError)
