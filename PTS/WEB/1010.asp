...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "1010URL", reqReturnURL
   SetCache "1010DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "1010")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      Else
         With oPayment
            ...
            tmpPurpose = .Purpose
            tmpAmount = .Amount
            tmpBTC = GetUSDBTC(tmpAmount)
            tmpAddress = ReceiveBitCoin( "",  reqPaymentID, tmpAmount ) 
            xmlPayment = .XML(2)
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
         End With
      End If
   ...
   Case CLng(cActionCancel):
      ...
      reqReturnURL = GetCache("1010URL")
      reqReturnData = GetCache("1010DATA")
      SetCache "1010URL", ""
      SetCache "1010DATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "1010 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "1010 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "1010 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "1010 Load file (oData) failed with error code " + CStr(oData.parseError)
