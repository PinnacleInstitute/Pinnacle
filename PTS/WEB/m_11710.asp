...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "m_11710URL", reqReturnURL
   SetCache "m_11710DATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "m_11710")
...
Select Case CLng(reqActionCode)
   ...
   Case CLng(cActionNew):
      ...
      If (reqGroupID = 0) And (reqSysMemberID <> 0) Then
         Set oMember = server.CreateObject("ptsMemberUser.CMember")
         If oMember Is Nothing Then
            DoError Err.Number, Err.Source, "Unable to Create Object - ptsMemberUser.CMember"
         Else
...
If oLanguage.parseError <> 0 Then
   Response.Write "m_11710 Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "m_11710 Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "m_11710 Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "m_11710 Load file (oData) failed with error code " + CStr(oData.parseError)
