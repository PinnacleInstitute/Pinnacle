...
If (Len(Request.Item("ReturnURL")) > 0) Then
   ...
   SetCache "2802aURL", reqReturnURL
   SetCache "2802aDATA", reqReturnData
End If
...
pos = InStr(LCASE(reqSysServerPath), "2802a")
   ...
   Case CLng(cActionAdd):
      ...
      Else
         With oOrg
            ...
            .OrgName = .NameFirst + " " + .NameLast
            .OrgID = 1
            tmpLogon = .NewLogon
            .Validate 
            If (Err.Number <> 0) Then DoError Err.Number, Err.Source, Err.Description End If
            If (xmlError = "") And (tmpLogon <> "") Then
      ...
      If (xmlError = "") Then
         reqReturnURL = GetCache("2802aURL")
         reqReturnData = GetCache("2802aDATA")
         SetCache "2802aURL", ""
         SetCache "2802aDATA", ""
...
If oLanguage.parseError <> 0 Then
   Response.Write "2802a Load file (oLanguage) failed with error code " + CStr(oLanguage.parseError)
   Response.End
End If
...
If oCommon.parseError <> 0 Then
   Response.Write "2802a Load file (oCommon) failed with error code " + CStr(oCommon.parseError)
   Response.End
End If
...
If oStyle.parseError <> 0 Then
   Response.Write "2802a Load file (oStyle) failed with error code " + CStr(oStyle.parseError)
   Response.End
End If
...
If oData.parseError <> 0 Then
   Response.Write "2802a Load file (oData) failed with error code " + CStr(oData.parseError)
