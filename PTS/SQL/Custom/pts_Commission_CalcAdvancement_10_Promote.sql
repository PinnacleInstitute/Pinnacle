EXEC [dbo].pts_CheckProc 'pts_Commission_CalcAdvancement_10_Promote'
GO

--SELECT Title2, MinTitle, TitleDate, Title FROM Member WHERE MemberID = 7238
--DECLARE @Count int EXEC pts_Commission_CalcAdvancement_10_Promote 7238, 3, 0, 0, 0, @Count OUTPUT PRINT @Count
--select title, title2, maxmembers from member where memberid=572

CREATE PROCEDURE [dbo].pts_Commission_CalcAdvancement_10_Promote
	@MemberID int, 
	@PayTitle int, 
	@MinTitle int, 
	@TitleDate datetime, 
...
--	-- Check if member meets the next recruiting requirement, if so, check next titles 
	EXEC pts_Commission_CalcAdvancement_10_Test @MemberID, @NewTitle, @EnrollDate, @Qualify OUTPUT
...
--	-- If this member was promoted, Check if his Referrer can be promoted
	...
	BEGIN
		...
		EXEC pts_Commission_CalcAdvancement_10 @ReferralID, @EnrollDate, @cnt OUTPUT
	END	
END
