EXEC [dbo].pts_CheckProc 'pts_GCR_Sales2'
GO

--declare @Result varchar(1000) EXEC pts_GCR_Sales2 @Result output print @Result

-- *******************************************************************
-- Recalculate all group sales volumes for matrix
-- *******************************************************************
CREATE PROCEDURE [dbo].pts_GCR_Sales2
...
--print CAST(@cnt AS VARCHAR(10)) + ': ' + CAST(@MemberID AS VARCHAR(10)) + ' - ' + CAST(GETDATE() AS VARCHAR(20))
	EXEC pts_GCR_SetTotals2 @MemberID, 1, 1, @Result output
	FETCH NEXT FROM Member_cursor INTO @MemberID
END
CLOSE Member_cursor
