EXEC [dbo].pts_CheckProc 'pts_ZaZZed_PaymentBinaryDeduct'
GO

--EXEC pts_ZaZZed_PaymentBinaryDeduct 8015, 13946, 3
--select * from SalesOrder order by salesorderid desc

CREATE PROCEDURE [dbo].pts_ZaZZed_PaymentBinaryDeduct
   @MemberID int ,
   @SalesOrderID int ,
   @SaleType int 
AS
