EXEC [dbo].pts_CheckProc 'pts_Pinnacle_UpdateBadPayments'
GO

--DECLARE @Count varchar(1000) EXEC pts_Pinnacle_Payments 0, @Count OUTPUT print @Count

CREATE PROCEDURE [dbo].pts_Pinnacle_UpdateBadPayments
   @CompanyID int ,
...
WHERE me.CompanyID = @CompanyID AND me.Status >= 1 AND me.Status <= 4 AND pa.Status = 4
