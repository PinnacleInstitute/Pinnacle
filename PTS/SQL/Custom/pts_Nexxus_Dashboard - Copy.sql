...
DECLARE @Reward1 int, @Reward2 int, @Reward3 int, @Reward4 int, @Reward5 money, @cnt int
...
SELECT @Reward3 = COUNT(*) FROM Merchant WHERE MemberID = @MemberID
SELECT @Reward4 = COUNT(*) FROM Merchant WHERE MemberID = @MemberID AND Options NOT LIKE '%*%'
SELECT @Reward5 = ISNULL(SUM(Amount),0) FROM Commission WHERE OwnerType = 4 AND OwnerID = @MemberID AND CommType = 27

SET @Result = @Result + CAST(@Reward1 AS VARCHAR(10)) + ';' + CAST(@Reward2 AS VARCHAR(10)) + ';' + CAST(@Reward3 AS VARCHAR(10)) + ';' + CAST(@Reward4 AS VARCHAR(10)) + ';' + CAST(@Reward5 AS VARCHAR(15))
