EXEC [dbo].pts_CheckProc 'pts_PB_Stats'
GO

--DECLARE @Result varchar(1000) EXEC pts_PB_Stats 0, @Result OUTPUT print @Result

CREATE PROCEDURE [dbo].pts_PB_Stats
   @Days int ,
...
DECLARE @JK1 int, @JK7 int, @JK99 int
DECLARE @PK1 int, @PK7 int, @PK99 int
DECLARE @SK1 int, @SK7 int, @SK99 int
...
-- Junior Kits
SELECT @JK1 = ISNULL(COUNT(MemberID),0) FROM Member 
WHERE CompanyID = @CompanyID AND [Level] = 1 AND Status >= 1 AND Status <= 5 AND EnrollDate >= @StartDate AND EnrollDate < @EndDate AND Options2 like '%101%'

-- Junior Kits
SELECT @PK1 = ISNULL(COUNT(MemberID),0) FROM Member 
WHERE CompanyID = @CompanyID AND [Level] = 1 AND Status >= 1 AND Status <= 5 AND EnrollDate >= @StartDate AND EnrollDate < @EndDate AND Options2 like '%102%'

-- Junior Kits
SELECT @SK1 = ISNULL(COUNT(MemberID),0) FROM Member 
WHERE CompanyID = @CompanyID AND [Level] = 1 AND Status >= 1 AND Status <= 5 AND EnrollDate >= @StartDate AND EnrollDate < @EndDate AND Options2 like '%103%'
...
SELECT @JK7 = ISNULL(COUNT(MemberID),0) FROM Member 
WHERE CompanyID = @CompanyID AND [Level] = 1 AND Status >= 1 AND Status <= 5 AND EnrollDate >= @StartDate AND EnrollDate < @EndDate AND Options2 like '%101%'

-- Junior Kits
SELECT @PK7 = ISNULL(COUNT(MemberID),0) FROM Member 
WHERE CompanyID = @CompanyID AND [Level] = 1 AND Status >= 1 AND Status <= 5 AND EnrollDate >= @StartDate AND EnrollDate < @EndDate AND Options2 like '%102%'

-- Junior Kits
SELECT @SK7 = ISNULL(COUNT(MemberID),0) FROM Member 
WHERE CompanyID = @CompanyID AND [Level] = 1 AND Status >= 1 AND Status <= 5 AND EnrollDate >= @StartDate AND EnrollDate < @EndDate AND Options2 like '%103%'
...
-- ****************************************
SELECT @JK99 = ISNULL(COUNT(MemberID),0) FROM Member 
WHERE CompanyID = @CompanyID AND [Level] = 1 AND Status >= 1 AND Status <= 5 AND EnrollDate < @EndDate AND Options2 like '%101%'

-- Junior Kits
SELECT @PK99 = ISNULL(COUNT(MemberID),0) FROM Member 
WHERE CompanyID = @CompanyID AND [Level] = 1 AND Status >= 1 AND Status <= 5 AND EnrollDate < @EndDate AND Options2 like '%102%'

-- Junior Kits
SELECT @SK99 = ISNULL(COUNT(MemberID),0) FROM Member 
WHERE CompanyID = @CompanyID AND [Level] = 1 AND Status >= 1 AND Status <= 5 AND EnrollDate < @EndDate AND Options2 like '%103%'

-- Total Kits
SET @T99 = @JK99 + @PK99 + @SK99
...
SET @Result = '<PTSSTATS ' + 
'jk1="'  + CAST(@JK1 AS VARCHAR(10)) + '" ' +
'jk7="'  + CAST(@JK7 AS VARCHAR(10))  + '" ' +
'jk99="' + CAST(@JK99 AS VARCHAR(10)) + '" ' +
'pk1="'  + CAST(@PK1 AS VARCHAR(10)) + '" ' +
'pk7="'  + CAST(@PK7 AS VARCHAR(10))  + '" ' +
'pk99="' + CAST(@PK99 AS VARCHAR(10)) + '" ' +
'sk1="'  + CAST(@SK1 AS VARCHAR(10)) + '" ' +
'sk7="'  + CAST(@SK7 AS VARCHAR(10))  + '" ' +
'sk99="' + CAST(@SK99 AS VARCHAR(10)) + '" ' +
