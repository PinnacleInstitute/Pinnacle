EXEC [dbo].pts_CheckProc 'pts_SIMS_Payments'
GO

--DECLARE @Count varchar(1000) EXEC pts_SIMS_Payments 0, @Count OUTPUT print @Count

CREATE PROCEDURE [dbo].pts_SIMS_Payments
   @BillDate datetime,
...
DECLARE @Count int, @Free int, @NoteID int
DECLARE @MemberID int, @Price money, @InitPrice money, @Token int, @TokenType int, @TokenOwner int
DECLARE @EnrollDate datetime, @BillingID int, @PaidDate datetime, @Level int
...
DECLARE @Now datetime, @PaymentID int, @CreditAmt money, @Status int, @ProductID int 
...
SELECT MemberID, Price, InitPrice, EnrollDate, BillingID, PaidDate, Level
...
OPEN Member_cursor
FETCH NEXT FROM Member_cursor INTO @MemberID, @Price, @InitPrice, @EnrollDate, @BillingID, @PaidDate, @Level
...
--	Check for Referrals for free membership
	SET @cnt = 0
--	Check for $9.95 team leader membership
	If @Level = 2
	BEGIN
		SELECT @cnt = COUNT(MemberID) FROM Member WHERE ReferralID = @MemberID AND Status >= 1 AND Status <= 4 AND Level=2
	END	
--	Check for $4.95 basic membership
	If @Level = 3
	BEGIN 
		SELECT @cnt = COUNT(MemberID) FROM Member WHERE ReferralID = @MemberID AND Status >= 1 AND Status <= 4 AND (Level=2 OR Level=3)
	END	
	...
	IF @Free > 0
	BEGIN
		SET @Notes = 'Free Subscription - ' + CAST(@cnt AS VARCHAR(10)) + ' referrals'
--		NoteID,OwnerType,OwnerID,AuthUserID,NoteDate,Notes,IsLocked,IsFrozen,IsReminder,UserID
		EXEC pts_Note_Add @NoteID, 4, @MemberID, 1, @Now, @Notes, 0, 0, 0, 1 
	END
	ELSE
	BEGIN
		SET @tmpPurpose = 'myTrackerPro Subscription'
...
--				Amount,Total,Credit,Retail,Commission,Description,Purpose,Status,Reference,Notes,CommStatus,CommDate,TokenType, TokenOwner,Token,UserID
		EXEC pts_Payment_Add @PaymentID OUTPUT, @CompanyID, 4, @MemberID, 0, 0, 0, @PaidDate, 0, @PayType, 
					 @Price, @Total, @Credit, 0, 0, @tmpDescription, @tmpPurpose, @Status, '', @Notes, 1, 0, @TokenType, @TokenOwner, @Token, 1
		SET @Count = @Count + 1
	END
