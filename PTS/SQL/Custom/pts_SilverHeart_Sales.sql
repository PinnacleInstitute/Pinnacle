EXEC [dbo].pts_CheckProc 'pts_SilverHeart_Sales'
GO

--declare @Result varchar(1000) EXEC pts_SilverHeart_Sales @Result output print @Result
-- *******************************************************************
-- Recalculate all group sales volumes and promotions
-- *******************************************************************
CREATE PROCEDURE [dbo].pts_SilverHeart_Sales
...
BEGIN
	EXEC pts_SilverHeart_SetTotals @MemberID, 1, 1, @MaxTitle, @Result output
	FETCH NEXT FROM Member_cursor INTO @MemberID, @MaxTitle
END
CLOSE Member_cursor
