EXEC [dbo].pts_CheckProc 'pts_SilverHeart_Qualified'
GO
--589
--542
--declare @Result varchar(1000) EXEC pts_SilverHeart_Qualified 2, '12/31/14', @Result output print @Result

CREATE PROCEDURE [dbo].pts_SilverHeart_Qualified
   @Status int ,
   @czDate datetime ,
...
BEGIN
	EXEC pts_SilverHeart_QualifiedMember @MemberID, @Status, @czDate, @Result2
	FETCH NEXT FROM Member_cursor INTO @MemberID
END
CLOSE Member_cursor
