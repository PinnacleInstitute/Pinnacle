EXEC [dbo].pts_CheckProc 'pts_ZaZZed_SalesB'
GO

--declare @Result varchar(1000) EXEC pts_ZaZZed_SalesB @Result output print @Result

-- *******************************************************************
-- Recalculate all group sales volumes and promotions
-- *******************************************************************
CREATE PROCEDURE [dbo].pts_ZaZZed_SalesB
...
WHERE CompanyID = 9 AND Sponsor3ID > 0
...
FROM Member AS me WHERE me.CompanyID = 9 AND Sponsor3ID > 0 AND Status > 0
...
SELECT Sponsor3ID
FROM Member WHERE CompanyID = 9 AND Sponsor3ID > 0 AND BV4 = 0
--AND Status >= 1 AND Status <= 4 
...
BEGIN
	EXEC pts_ZaZZed_SetTotalsB @MemberID, 0, @Result output
	FETCH NEXT FROM Member_cursor INTO @MemberID
END
CLOSE Member_cursor
