EXEC [dbo].pts_CheckProc 'pts_Commission_CalcAdvancement_21_Promote'
GO

--	SELECT Title2, MinTitle, TitleDate, Title FROM Member WHERE MemberID = 10733
--	EXEC pts_Commission_CalcAdvancement_21_Promote @10733, 2, 0, 0, 5, 0, @Count OUTPUT

--SELECT Title2, MinTitle, TitleDate, Title FROM Member WHERE MemberID = 7238
--DECLARE @Count int EXEC pts_Commission_CalcAdvancement_21_Promote 10733, 2, 0, 0, 5, 0, @Count OUTPUT PRINT @Count
--select title, title2, maxmembers from member where memberid=572

CREATE PROCEDURE [dbo].pts_Commission_CalcAdvancement_21_Promote
	@MemberID int, 
	@PayTitle int, 
	@MinTitle int, 
	@TitleDate datetime, 
...
DECLARE @MaxTitle int, @NewTitle int
DECLARE @Qualify int, @cnt int, @ID int, @IsEarned bit, @Result VARCHAR(20)
...
--	-- Check if member meets the next recruiting requirement, if so, check next titles 
	EXEC pts_Commission_CalcAdvancement_21_Test @MemberID, @NewTitle, @EnrollDate, @Qualify OUTPUT, @Result OUTPUT
...
--	-- Check for order upgrade
	EXEC pts_Nexxus_UpgradeOrder @MemberID, @Result
...
--	-- If this member was promoted, Check if his Referrer can be promoted
	...
	BEGIN
		...
		EXEC pts_Commission_CalcAdvancement_21 @ReferralID, @EnrollDate, @cnt OUTPUT
	END	
END
