EXEC [dbo].pts_CheckProc 'pts_GFTG_Payments'
GO

--DECLARE @Count varchar(1000) EXEC pts_GFTG_Payments 0, @Count OUTPUT print @Count

CREATE PROCEDURE [dbo].pts_GFTG_Payments
   @BillDate datetime,
...
SELECT MemberID, Price, EnrollDate, BillingID, PaidDate, Options2
...
OPEN Member_cursor
FETCH NEXT FROM Member_cursor INTO @MemberID, @Price, @EnrollDate, @BillingID, @PaidDate, @Options2
WHILE @@FETCH_STATUS = 0
BEGIN
	SET @tmpPurpose = ''

--	Only process Autoships
	SET @tmpCode = '104' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '105' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '106' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '107' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '108' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '109' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '110' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '111' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '224' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '225' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '226' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '227' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '228' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '229' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '221' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '334' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '335' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '336' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '337' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '338' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '339' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode
	SET @tmpCode = '331' IF CHARINDEX(@tmpCode, @Options2) > 0 SET @tmpPurpose = @tmpCode

--	Set # of months for autoship payment
	SET @months = 1
	IF CHARINDEX(@tmpPurpose, '224,225,226,227,228,229,221') > 0  SET @months = 2
	IF CHARINDEX(@tmpPurpose, '334,335,336,337,338,339,331') > 0  SET @months = 3
...
--		Do Payment post processing
		EXEC pts_GFTG_PaymentPost @PaymentID, @tmpCount OUTPUT
