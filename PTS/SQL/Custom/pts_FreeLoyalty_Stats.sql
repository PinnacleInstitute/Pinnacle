EXEC [dbo].pts_CheckProc 'pts_FreeLoyalty_Stats'
GO

--DECLARE @Result varchar(1000) EXEC pts_FreeLoyalty_Stats 0, @Result OUTPUT print @Result

CREATE PROCEDURE [dbo].pts_FreeLoyalty_Stats
   @Days int ,
...
DECLARE @M11 int, @M18 int, @M199 int
...
SET	@M11 = 0 SET	@M18 = 0 SET	@M199 = 0
...
WHERE CompanyID = @CompanyID AND Status = 1 AND Title = 7 AND EnrollDate >= @StartDate AND EnrollDate < @EndDate

-- Total Members
SET @T1 = @M11 + @M21 + @M31 + @M41 + @M51 + @M61 + @M71
...
-- Affiliates
SELECT @M18 = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 1 AND Title = 1 AND EnrollDate >= @StartDate AND EnrollDate < @EndDate
...
-- Total Members
SET @T7 = @M18 + @M27 + @M37 + @M47 + @M57 + @M67 + @M77
...
WHERE CompanyID = @CompanyID AND Status = 1 AND Title = 7 AND EnrollDate < @EndDate

-- Total Members
SET @T99 = @M199 + @M299 + @M399 + @M499 + @M599 + @M699 + @M799
...
'm71="'  + CAST(@M71 AS VARCHAR(10)) + '" ' +
'm77="'  + CAST(@M77 AS VARCHAR(10))  + '" ' +
'm799="' + CAST(@M799 AS VARCHAR(10)) + '" ' +
