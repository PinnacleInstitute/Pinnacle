EXEC [dbo].pts_CheckProc 'pts_Note_ListActivityOwner'
GO

--EXEC pts_Note_ListActivityOwner 4, 0, '3/1/05', '4/29/05'

CREATE PROCEDURE [dbo].pts_Note_ListActivityOwner
   @OwnerType int ,
   @AuthUserID int ,
   @FromDate datetime ,
   @ToDate datetime
...
BEGIN
	...
	AND nt.NoteDate >= @FromDate AND nt.NoteDate <= @ToDate AND nt.IsReminder = 0
