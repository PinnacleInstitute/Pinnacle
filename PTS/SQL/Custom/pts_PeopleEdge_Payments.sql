EXEC [dbo].pts_CheckProc 'pts_PeopleEdge_Payments'
GO

--DECLARE @Count varchar(1000) EXEC pts_PeopleEdge_Payments 0, @Count OUTPUT print @Count

CREATE PROCEDURE [dbo].pts_PeopleEdge_Payments
   @BillDate datetime,
...
DECLARE @Count int, @tmpCount int, @Free int, @NoteID int
...
DECLARE @cnt int, @Notes nvarchar(500), @tmpNotes nvarchar(500), @tmpDescription varchar(200), @tmpPurpose varchar(100), @Options2 varchar(100)
...
SELECT MemberID, Price, InitPrice, EnrollDate, BillingID, PaidDate, Level, Options2
...
OPEN Member_cursor
FETCH NEXT FROM Member_cursor INTO @MemberID, @Price, @InitPrice, @EnrollDate, @BillingID, @PaidDate, @Level, @Options2
...
--	Do Payment post processing
	EXEC pts_PeopleEdge_PaymentPost @PaymentID, @tmpCount OUTPUT
