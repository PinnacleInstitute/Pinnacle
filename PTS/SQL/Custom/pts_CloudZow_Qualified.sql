EXEC [dbo].pts_CheckProc 'pts_CloudZow_Qualified'
GO

--declare @Result varchar(1000) EXEC pts_CloudZow_Qualified 1, '6/3/12', @Result output print @Result

CREATE PROCEDURE [dbo].pts_CloudZow_Qualified
   @Status int ,
   @czDate datetime ,
...
--	--------------------------------------------------------------------------------------------------------
	...
	WHERE CompanyID = @CompanyID AND Status >= 1 AND Status <= 4 AND Level = 1 AND ( Qualify = 1 OR  Qualify = 2 )
	
	UPDATE Member SET Qualify = 0 
	WHERE CompanyID = @CompanyID AND Status >= 1 AND Status <= 4 AND Level = 1 AND Qualify = 3 AND QualifyDate > 0 AND QualifyDate < @czDate
