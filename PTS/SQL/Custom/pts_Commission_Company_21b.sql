EXEC [dbo].pts_CheckProc 'pts_Commission_Company_21b'
GO
--DECLARE @Count int EXEC pts_Commission_Company_21b 7995, 0, '', @Count OUTPUT PRINT @Count
--select * from Commission order by CommissionID desc

CREATE PROCEDURE [dbo].pts_Commission_Company_21b
   @MemberID int ,
   @PaymentID int ,
   @Ref varchar(100),
   @Count int OUTPUT 
...
DECLARE @CompanyID int, @ID int, @Now datetime, @ReferralID int, @Sponsor3ID int, @Level int, @PayTitle int
...
--	-- Get the Sponsor3's info
	SET @Sponsor3ID = -1
	SELECT @Sponsor3ID = Sponsor3ID, @Qualify = Qualify, @PayTitle = Title2 FROM Member WHERE MemberID = @MemberID
...
--				-- Calculate Cycle Bonus based on Title
				SET @Rate = 0
				IF @PayTitle >= 3 AND @PayTitle <= 6 SET @Rate = .05
				IF @PayTitle >= 7 AND @PayTitle <= 8 SET @Rate = .06
				IF @PayTitle >= 9 AND @PayTitle <= 10 SET @Rate = .07
				IF @PayTitle >= 11 AND @PayTitle <= 11 SET @Rate = .08
				IF @PayTitle >= 12 AND @PayTitle <= 13 SET @Rate = .09
				IF @PayTitle = 14 SET @Rate = .10
...
--				-- Pay a cycle bonus for each $100 on the lesser leg sales volume
			...
			BEGIN
				SET @CommType = 12
				SET @Desc = @Ref + ' (' + CAST( @Level AS VARCHAR(5) ) + '-' + CAST( @rate*100 AS VARCHAR(10) ) + ')'
...
--				-- Calculate Matching Bonuses
				EXEC pts_Commission_Company_21c @PaymentID, @Now, @MemberID, 0, @Bonus, @Ref, @cnt OUTPUT
				SET @Count = @Count + @cnt
