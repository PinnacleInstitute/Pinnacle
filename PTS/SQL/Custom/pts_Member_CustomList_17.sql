EXEC [dbo].pts_CheckProc 'pts_Member_CustomList_17'
GO

--EXEC pts_Member_CustomList_17 108, 0

CREATE PROCEDURE [dbo].pts_Member_CustomList_17
   @Status int ,
   @Level int
AS
...
-- Get All Member with Coin Wallet Transfers
IF @Status = 108
BEGIN
	SET @ListDate = DATEADD(d,(@Level*-1) + -1,dbo.wtfn_DateOnly(GETDATE()))

	Select me.MemberID 'MemberID', me.Status 'Status', me.Level 'Level', 
	CAST(me.MemberID AS VARCHAR(10)) + ' ' + CAST(mi.Coins AS VARCHAR(30)) AS 'Signature' 
	FROM Mining AS mi
	JOIN Member AS me ON mi.MemberID = me.MemberID
	WHERE mi.MiningDate = @ListDate AND mi.Status = 0
	
--  UPDATE Mining SET Status = 1 WHERE MiningDate = @ListDate	
END
...
BEGIN
	...
	BEGIN
		...
		AND (Options2 LIKE '%202%' OR Options2 LIKE '%203%' OR Options2 LIKE '%204%' OR Options2 LIKE '%205%' OR Options2 LIKE '%206%' OR Options2 LIKE '%207%' OR Options2 LIKE '%208%' ) 

		INSERT INTO @Exception
		Select me.MemberID, 2, me.Title, '#' + CAST(me.MemberID AS VARCHAR(10)) + ' - ' + me.NameFirst + ' ' + me.NameLast, 'Missing Autoship Code' 
		FROM Member AS me WHERE me.CompanyID = @CompanyID AND me.Status = 1 AND me.Billing IN (3,5) AND me.Price > 0
		AND (Options2 NOT LIKE '%202%' AND Options2 NOT LIKE '%203%' AND Options2 NOT LIKE '%204%' AND Options2 NOT LIKE '%205%'
		AND  Options2 NOT LIKE '%206%' AND Options2 NOT LIKE '%207%' AND Options2 NOT LIKE '%208%' )
	END	
