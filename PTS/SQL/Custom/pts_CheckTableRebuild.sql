IF EXISTS (SELECT * FROM sysobjects WHERE id = OBJECT_ID('[dbo].[pts_CheckTableRebuild]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[pts_CheckTableRebuild]
GO

CREATE PROCEDURE [dbo].pts_CheckTableRebuild (
...
WHILE			(@@fetch_status = 0)
			BEGIN
				IF LEFT( @mConstraintName, 7 ) != '_WA_Sys'
				BEGIN
					SET @mSQL = 'DROP INDEX ' + @TableName + '.' + @mConstraintName 
					EXEC (@mSQL)
print @mConstraintName
				END
				FETCH NEXT FROM cursorIDX INTO @mConstraintName
			END
