EXEC [dbo].pts_CheckProc 'pts_CIS_CustomList'
GO

-- CIS pts_CIS_CustomList 3, 0, 0, 0

CREATE PROCEDURE [dbo].pts_CIS_CustomList
   @Status int ,
   @czDate datetime ,
   @Quantity int ,
   @Amount int
...
--UserName,FirstName,LastName,CompanyName,Address1,Address2,City,State,ZipCode,Country2xFormat,PhoneNumber,CellPhoneNumber,EmailAddress,SSN,CompanyTaxID,GovernmentID,MilitaryID,PassportNumber,DriversLicense,DateOfBirth,WebsitePassword,DefaultCurrency,PreferredLanguage
	SELECT me.MemberID AS 'CISID', '"' + au.Logon + '","' + me.NameFirst + '","' + me.NameLast + '",,,,,,,,,,"' + me.Email + '",,,,,,,,"' + au.Password + '",,' AS 'Result'
	FROM Member AS me
