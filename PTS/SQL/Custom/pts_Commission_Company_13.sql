EXEC [dbo].pts_CheckProc 'pts_Commission_Company_13'
GO

--DECLARE @Count int EXEC pts_Commission_Company_13 1, '4/1/12', '6/17/12', @Count OUTPUT PRINT @Count

CREATE PROCEDURE [dbo].pts_Commission_Company_13
   @CommType int ,
   @FromDate datetime ,
   @ToDate datetime ,
   @Count int OUTPUT
...
--	AND me.MemberID = 4282
	...
	BEGIN
		SET @tmpCount = 0
		EXEC pts_Commission_Company_13a @PaymentID, @tmpCount OUTPUT
