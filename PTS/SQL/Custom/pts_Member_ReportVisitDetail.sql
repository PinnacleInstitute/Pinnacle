EXEC [dbo].pts_CheckProc 'pts_Member_ReportVisitDetail'
GO

CREATE PROCEDURE [dbo].pts_Member_ReportVisitDetail
   @CompanyID int,
   @Quantity int
AS
...
BEGIN
	...
	WHERE VisitDate = 0
	AND (@CompanyID = 0 OR CompanyID = @CompanyID) AND ( Status > 0 AND Status < 4 )
END
IF @Quantity = 1
BEGIN
	SELECT TOP 1000 MemberID, LTRIM(RTRIM(NameLast)) +  ', '  + LTRIM(RTRIM(NameFirst)) 'MemberName', CompanyName, VisitDate FROM Member
