EXEC [dbo].pts_CheckProc 'pts_BarterAd_Search1'
 GO

--DECLARE @MaxRows tinyint EXEC pts_BarterAd_Search1 'plano', '', @MaxRows OUTPUT, 1, 0, 0, 0, 0, 0, 'Price:5,500;Payment:A,1' PRINT @MaxRows
--select * from barterad
--select * from bartercategory

CREATE PROCEDURE [dbo].pts_BarterAd_Search1 ( 
...
-- Process Relevent Ad Sort
IF @Status = 1
BEGIN
	IF @Bookmark = '' SET @Bookmark = '9999'

	SELECT TOP 21
		dbo.wtfn_FormatNumber(K.[RANK], 4) + dbo.wtfn_FormatNumber(ba.BarterAdID, 10) 'BookMark',
		ba.BarterAdID, 
		ba.BarterArea1ID, 
		ba.BarterArea2ID, 
		ba.BarterCategoryID, 
	...
	INNER JOIN CONTAINSTABLE(BarterAdFT,*, @SearchText, 1000 ) AS K ON ba.BarterAdID = K.[KEY]
	...
	WHERE dbo.wtfn_FormatNumber(K.[RANK], 4) + dbo.wtfn_FormatNumber(ba.BarterAdID, 10) <= @Bookmark
	AND ba.Status = 2
