EXEC [dbo].pts_CheckProc 'pts_Folder_EnumFolderList'
GO
--EXEC pts_Folder_EnumFolderList 9,0,7164,22,1 

CREATE PROCEDURE [dbo].pts_Folder_EnumFolderList
   @CompanyID int ,
   @GroupID int ,
   @MemberID int ,
   @Entity int ,
...
WHERE (f1.CompanyID=0 OR f1.CompanyID=@CompanyID) AND (f1.Entity=@Entity) AND f1.Virtual=0 AND f1.ParentID=0
...
WHERE (f2.CompanyID=0 OR f2.CompanyID=@CompanyID) AND (f2.Entity=@Entity) AND f2.Virtual=0 AND f1.ParentID=0
...
WHERE (f3.CompanyID=0 OR f3.CompanyID=@CompanyID) AND (f3.Entity=@Entity) AND f3.Virtual=0 AND f1.ParentID=0
...
WHERE (f4.CompanyID=0 OR f4.CompanyID=@CompanyID) AND (f4.Entity=@Entity) AND f4.Virtual=0 AND f1.ParentID=0
