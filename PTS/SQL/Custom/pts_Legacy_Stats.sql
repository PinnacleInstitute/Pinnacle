EXEC [dbo].pts_CheckProc 'pts_Legacy_Stats'
GO

--DECLARE @Result varchar(1000) EXEC pts_Legacy_Stats 0, @Result OUTPUT print @Result

CREATE PROCEDURE [dbo].pts_Legacy_Stats
   @Days int ,
...
DECLARE @Z1 int, @Z7 int, @Z99 int
DECLARE @01 int, @07 int, @099 int, @0Z int
DECLARE @11 int, @17 int, @199 int, @1Z int
DECLARE @21 int, @27 int, @299 int, @2Z int
DECLARE @31 int, @37 int, @399 int, @3Z int
DECLARE @41 int, @47 int, @499 int, @4Z int
DECLARE @51 int, @57 int, @599 int, @5Z int
DECLARE @61 int, @67 int, @699 int, @6Z int
DECLARE @T1 int, @T7 int, @T99 int, @TZ int
...
SET	@Z1 = 0 SET	@Z7 = 0 SET	@Z99 = 0 
SET	@01 = 0 SET	@07 = 0 SET	@099 = 0 SET @0Z = 0 
SET	@11 = 0 SET	@17 = 0 SET	@199 = 0 SET @1Z = 0 
SET	@21 = 0 SET	@27 = 0 SET	@299 = 0 SET @2Z = 0 
SET	@31 = 0 SET	@37 = 0 SET	@399 = 0 SET @3Z = 0 
SET	@41 = 0 SET	@47 = 0 SET	@499 = 0 SET @4Z = 0 
SET	@51 = 0 SET	@57 = 0 SET	@599 = 0 SET @5Z = 0 
SET	@61 = 0 SET	@67 = 0 SET	@699 = 0 SET @6Z = 0 
SET	@T1 = 0 SET	@T7 = 0 SET	@T99 = 0 SET @TZ = 0 
...
SET @StartDate = @Now
-- ZaZZed
SELECT @Z1 = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 1 AND Options2 = '113' AND EndDate >= @StartDate AND EndDate < @EndDate

-- Reseller
SELECT @01 = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 1 AND Title = 0 AND EnrollDate >= @StartDate AND EnrollDate < @EndDate
...
-- DD+
SELECT @61 = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 1 AND Title >= 6 AND EnrollDate >= @StartDate AND EnrollDate < @EndDate

-- Total Members
SET @T1 = @01 + @11 + @21 + @31 + @41 + @51 + @61

-- ZaZZed
SELECT @Z1 = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 2 AND Level = 3 AND Title >= 6 AND EnrollDate >= @StartDate AND EnrollDate < @EndDate
...
-- ZaZZed
SELECT @Z7 = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 1 AND Options2 = '113' AND EndDate >= @StartDate AND EndDate < @EndDate
...
-- DD+
SELECT @67 = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 1 AND Title >= 6 AND EnrollDate >= @StartDate AND EnrollDate < @EndDate

-- Total Members
SET @T7 = @07 + @17 + @27 + @37 + @47 + @57 + @67

-- ****************************************
-- ZaZZed
SELECT @Z99 = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 1 AND Options2 = '113' AND EndDate < @EndDate

-- Reseller
SELECT @099 = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 1 AND Title = 0 AND EnrollDate < @EndDate
...
-- Emerald+
SELECT @699 = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 1 AND Title >= 6 AND EnrollDate < @EndDate

-- Total Members
SET @T99 = @099 + @199 + @299 + @399 + @499 + @599 + @699


-- Inactive ZaZZed Members
-- ****************************************
-- Reseller
SELECT @0Z = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 2 AND Level = 3 AND Title = 0

-- Wholesale Buyers
SELECT @1Z = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 2 AND Level = 3 AND Title = 1

-- Affiliates
SELECT @2Z = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 2 AND Level = 3 AND Title = 2

-- Silver
SELECT @3Z = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 2 AND Level = 3 AND Title = 3

-- Gold
SELECT @4Z = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 2 AND Level = 3 AND Title = 4

-- Platinum
SELECT @5Z = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 2 AND Level = 3 AND Title = 5

-- Emerald+
SELECT @6Z = ISNULL(COUNT(MemberID),0) FROM Member
WHERE CompanyID = @CompanyID AND Status = 2 AND Level = 3 AND Title = 6

-- Total Members
SET @TZ = @0Z + @1Z + @2Z + @3Z + @4Z + @5Z + @6Z
