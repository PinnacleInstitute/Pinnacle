EXEC [dbo].pts_CheckProc 'pts_Member_AddCustom_1'
GO

CREATE PROCEDURE [dbo].pts_Member_AddCustom_1
   @MemberID int
AS

DECLARE @Status int, @Title int, @Price money, @BillingID int, @ID int, @PaidDate datetime
DECLARE @PayType int, @tmpDescription varchar(200), @BillingInfo varchar(200) 

SELECT @Status = Status, @Title = Title, @Price = InitPrice, @BillingID = BillingID FROM Member WHERE MemberID = @MemberID
