EXEC [dbo].pts_CheckProc 'pts_Commission_Company_15'
GO

--DECLARE @Count int EXEC pts_Commission_Company_15 1, '4/1/12', '6/17/12', @Count OUTPUT PRINT @Count

CREATE PROCEDURE [dbo].pts_Commission_Company_15
   @CommType int ,
   @FromDate datetime ,
   @ToDate datetime ,
   @Count int OUTPUT
...
--	AND me.MemberID = 4282
	...
	BEGIN
		SET @tmpCount = 0
		EXEC pts_Commission_Company_15a @PaymentID, @tmpCount OUTPUT
