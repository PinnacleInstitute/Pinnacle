EXEC [dbo].pts_CheckProc 'pts_Silver_Qualified'
GO

--declare @Result varchar(1000) EXEC pts_Silver_Qualified 2, '6/3/12', @Result output print @Result

CREATE PROCEDURE [dbo].pts_Silver_Qualified
   @Status int ,
   @czDate datetime ,
...
--	--------------------------------------------------------------------------------------------------------
	...
	WHERE CompanyID = @CompanyID AND Status >= 1 AND Status <= 4 AND Qualify = 0 AND Title > 0 AND Price > 0
