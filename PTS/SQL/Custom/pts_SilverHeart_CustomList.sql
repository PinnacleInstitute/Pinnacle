EXEC [dbo].pts_CheckProc 'pts_SilverHeart_CustomList'
GO

-- EXEC pts_SilverHeart_CustomList 13, '1/1/14', 199, 0

CREATE PROCEDURE [dbo].pts_SilverHeart_CustomList
   @Status int ,
   @czDate datetime ,
   @Quantity int ,
   @Amount int
...
--UserName,FirstName,LastName,CompanyName,Address1,Address2,City,State,ZipCode,Country2xFormat,PhoneNumber,CellPhoneNumber,EmailAddress,SSN,CompanyTaxID,GovernmentID,MilitaryID,PassportNumber,DriversLicense,DateOfBirth,WebsitePassword,DefaultCurrency,PreferredLanguage
	SELECT me.MemberID AS 'SilverHeartID', '"' + au.Logon + '","' + me.NameFirst + '","' + me.NameLast + '",,,,,,,,,,"' + me.Email + '",,,,,,,,"' + au.Password + '",,' AS 'Result'
	FROM Member AS me
...
BEGIN
	...
	SELECT  so.SalesOrderID AS 'SilverHeartID', CAST(so.SalesOrderID AS VARCHAR(20)) + ',' + dbo.wtfn_DateOnlyStr(so.OrderDate) + ',' + CAST(so.Status AS VARCHAR(20)) + ',' + CAST(so.Total AS VARCHAR(20)) + ',' + CAST(so.Amount AS VARCHAR(20)) + ',' + CAST(so.Shipping AS VARCHAR(20)) + ',' + CAST(me.MemberID AS VARCHAR(20)) + ',' + ISNULL(gr.NameLast,'') + ',' + ISNULL(co.CountryName,ISNULL(co2.CountryName,'')) AS 'Result'
	...
	AND so.SalesOrderID NOT IN (SELECT SilverHeartID FROM @ExportData)
	ORDER BY so.OrderDate, so.SalesOrderID
	
	SELECT SilverHeartID, Result FROM @ExportData
END
...
BEGIN
	...
	INSERT INTO @ExportData
	SELECT si.SalesItemID AS 'SilverHeartID', CAST(si.SalesItemID AS VARCHAR(20)) + ',' + CAST(so.SalesOrderID AS VARCHAR(20)) + ',' + dbo.wtfn_DateOnlyStr(so.OrderDate) + ',' + CAST(so.Status AS VARCHAR(20)) + ',' + CAST(si.Quantity AS VARCHAR(20)) + ',' + CAST(si.Price AS VARCHAR(20)) + ',' + pr.ProductName + ',' + pt.ProductTypeName + ',' + CAST(me.MemberID AS VARCHAR(20)) + ',' + ISNULL(gr.NameLast,'') + ',' + ISNULL(co.CountryName,ISNULL(co2.CountryName,'')) AS 'Result'
	...
	AND si.SalesItemID NOT IN (SELECT SilverHeartID FROM @ExportData)
	ORDER BY so.OrderDate, so.SalesOrderID
...
BEGIN
	...
	INSERT INTO @ExportData
	SELECT pa.PaymentID AS 'SilverHeartID', CAST(pa.PaymentID AS VARCHAR(20)) + ',' + CAST(so.SalesOrderID AS VARCHAR(20)) + ',' + dbo.wtfn_DateOnlyStr(pa.PaidDate) + ',' + CAST(pa.Total AS VARCHAR(20)) + ',' + CAST(pa.Status AS VARCHAR(20)) + ',' + pa.Purpose + ',' + CAST(pa.PayType AS VARCHAR(20)) + ',' + CAST(pa.CommStatus AS VARCHAR(20)) + ',' + CAST(me.MemberID AS VARCHAR(20)) + ',' + ISNULL(gr.NameLast,'') + ',' + ISNULL(co.CountryName,ISNULL(co2.CountryName,'')) AS 'Result'
	FROM Payment AS pa
	...
	AND dbo.wtfn_DateOnly(so.OrderDate) >= @FromDate AND dbo.wtfn_DateOnly(OrderDate) <= @ToDate
	AND pa.PaymentID NOT IN (SELECT SilverHeartID FROM @ExportData)
	ORDER BY so.OrderDate, so.SalesOrderID
...
BEGIN
	...
	INSERT INTO @ExportData
	SELECT cm.CommissionID AS 'SilverHeartID', CAST(cm.CommissionID AS VARCHAR(20)) + ',' + dbo.wtfn_DateOnlyStr(cm.CommDate) + ',' + CAST(cm.Total AS VARCHAR(20)) + ',' + CAST(cm.Status AS VARCHAR(20)) + ',' + CAST(cm.CommType AS VARCHAR(20)) + ',' + CAST(cm.RefID AS VARCHAR(20)) + ',' + CAST(me.MemberID AS VARCHAR(20)) + ',' + ISNULL(gr.NameLast,'') + ',' + ISNULL(co.CountryName,'') AS 'Result'
	...
	AND cm.CommissionID NOT IN (SELECT SilverHeartID FROM @ExportData)
