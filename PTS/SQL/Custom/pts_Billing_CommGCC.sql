EXEC [dbo].pts_CheckProc 'pts_Billing_CommGCC'
GO

CREATE PROCEDURE [dbo].pts_Billing_CommGCC
   @CompanyID int ,
   @CommDate datetime ,
   @Amount money
AS
...
WHERE bi.CommType = 4 AND me.Status >= 1 AND me.Status <= 3 AND me.Qualify >= 2
