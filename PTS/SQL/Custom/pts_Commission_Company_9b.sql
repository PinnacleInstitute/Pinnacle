EXEC [dbo].pts_CheckProc 'pts_Commission_Company_9b'
GO
--DECLARE @Count int EXEC pts_Commission_Company_9b 7995, 0, '', @Count OUTPUT PRINT @Count
--select * from Commission order by CommissionID desc

CREATE PROCEDURE [dbo].pts_Commission_Company_9b
   @MemberID int ,
   @PaymentID int ,
   @Ref varchar(100),
   @Count int OUTPUT 
...
--	--Did we find the member
	IF @Sponsor3ID >= 0
	BEGIN
--		-- Get their number of referred Affiliates in the binary plan (must be Gold or higher)
		SELECT @cnt = COUNT(*) FROM Member WHERE ReferralID = @MemberID AND Status >= 1 AND Status <= 4 AND Title >= 4 AND Options2 like 'B%'
...
--			-- If we have sales volume in both legs
			IF @QVLeft > 0 AND @QVRight > 0
			BEGIN
--				-- Use the lesser leg sales volume
				IF @QVLeft > @QVRight SET @Amount = @QVRight ELSE SET @Amount = @QVLeft
--				-- Pay a 15% bonus on the lesser leg sales volume
				SET @Bonus = ROUND(@Amount * .15, 2)
			END
...
--						-- Are they title qualified for the level
 						IF @Level2 = 1 AND @Title < 7 SET @Qualify = 0
						IF @Level2 = 2 AND @Title < 8 SET @Qualify = 0
						IF @Level2 = 3 AND @Title < 9 SET @Qualify = 0
						IF @Level2 = 4 AND @Title < 10 SET @Qualify = 0

--						SET @cnt = 1
-- 						IF @Level2 = 1 AND @Title < 7 SET @cnt = 0
--						IF @Level2 = 2 AND @Title < 8 SET @cnt = 0
--						IF @Level2 = 3 AND @Title < 9 SET @cnt = 0
--						IF @Level2 = 4 AND @Title < 10 SET @cnt = 0
--					
--						IF @cnt = 0 AND @Qualify > 1
--						BEGIN
--							-- Get their number of referred active Affiliates in the binary plan
--							SELECT @cnt = COUNT(*) FROM Member WHERE ReferralID = @MemberID AND Status >= 1 AND Status <= 4 AND Options2 like 'B%'
--
--							-- Are they qualified for the level
--							IF @Level2 = 1 AND @cnt < 3 SET @Qualify = 0
--							IF @Level2 = 2 AND @cnt < 6 SET @Qualify = 0
--							IF @Level2 = 3 AND @cnt < 9 SET @Qualify = 0
--							IF @Level2 = 4 AND @cnt < 12 SET @Qualify = 0
--						END 
