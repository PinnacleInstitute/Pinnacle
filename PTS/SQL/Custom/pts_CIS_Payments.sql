EXEC [dbo].pts_CheckProc 'pts_CIS_Payments'
GO

--DECLARE @Count varchar(1000) EXEC pts_CIS_Payments 0, @Count OUTPUT print @Count

CREATE PROCEDURE [dbo].pts_CIS_Payments
   @BillDate datetime,
...
--	Affiliate 
	IF CHARINDEX('101', @Options2) = 0 SET @tmpPurpose = '101'

--	Silver
	IF CHARINDEX('102', @Options2) = 0 SET @tmpPurpose = '102'
...
--		Do Payment post processing
		EXEC pts_CIS_PaymentPost @PaymentID, @tmpCount OUTPUT

--		Advance paid date to next year
		SET @PaidDate = DATEADD(month,12,@PaidDate)
