EXEC [dbo].pts_CheckProc 'pts_Consumer_ListEmail'
GO
--EXEC pts_Consumer_ListEmail 1, 1, 1, 30, '75075,75093', 224
--select * from Consumer
--select * from Block

CREATE PROCEDURE [dbo].pts_Consumer_ListEmail
   @MerchantID int ,
   @TargetArea int ,
   @TargetType int ,
   @TargetDays int ,
...
BEGIN
	...
	IF @TargetType <= 1
	BEGIN 
		SELECT ConsumerID, Email2, NameLast, NameFirst FROM Consumer (NOLOCK)
