EXEC [dbo].pts_CheckProc 'pts_Commission_Company_11'
GO

--DECLARE @Count int EXEC pts_Commission_Company_11 1, '8/12/11', '8/19/13', @Count OUTPUT PRINT @Count

CREATE PROCEDURE [dbo].pts_Commission_Company_11
   @CommType int ,
   @FromDate datetime ,
   @ToDate datetime ,
   @Count int OUTPUT
...
--	AND me.MemberID = 4282
	...
	BEGIN
		SET @tmpCount = 0
		EXEC pts_Commission_Company_11a @PaymentID, @tmpCount OUTPUT
