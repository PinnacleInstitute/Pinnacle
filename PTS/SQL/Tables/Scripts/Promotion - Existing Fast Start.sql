-- *************************************************************
-- Calculate Fast Start Bonuses for existing Affiliates 5/17/12
-- *************************************************************
DECLARE @CompanyID int, @MemberID int, @Today datetime
...
--		Get the number of new active Affiliates enrolled by this member in 7 days
		...
		WHERE ReferralID = @MemberID AND Status >= 1 AND Status <= 4 AND Level = 1 AND Title >= 2 AND EnrollDate < '5/24/12'
		AND ( ( EnrollDate > '5/16/12' AND EnrollDate < '5/24/12') OR (BV >= 40 and QV4 < 40))
...
PRINT CAST(@MemberID AS VARCHAR(10)) + ' ' + @Desc + '(' + CAST(@tmpCount AS VARCHAR(10)) + ')'
			SET @Desc = 'Existing Affiliate: 5/16/12 - 5/24/12'
...
--		Get the number of new active Affiliates enrolled by this member in 7 days
		SET @tmpCount = 0
		SELECT @tmpCount = COUNT(*) FROM Member 
		WHERE ReferralID = @MemberID AND Status >= 1 AND Status <= 4 AND Level = 1 AND Title >= 3 AND EnrollDate < '5/31/12'
		AND ( ( EnrollDate > '5/16/12' AND EnrollDate < '5/31/12') OR (BV >= 40 and QV4 < 40))
		...
		BEGIN
			...
			SET @Bonus = 20
PRINT CAST(@MemberID AS VARCHAR(10)) + ' ' + @Desc + '(' + CAST(@tmpCount AS VARCHAR(10)) + ')'
			SET @Desc = 'Existing Affiliate: 5/16/12 - 5/31/12'
--			CommissionID, CompanyID, OwnerType, OwnerID, PayoutID, RefID, CommDate, Status, CommType, Amount, Total, Charge, Description, Notes, View, UserID
			EXEC pts_Commission_Add @ID, @CompanyID, 4, @MemberID, 0, 0, @Today, 1, @CommissionType, @Bonus, @Bonus, 0, @Desc, '', 1, 1
		END
		FETCH NEXT FROM Member_cursor INTO @MemberID, @Desc
