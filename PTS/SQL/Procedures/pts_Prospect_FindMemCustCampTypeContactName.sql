EXEC [dbo].pts_CheckProc 'pts_Prospect_FindMemCustCampTypeContactName'
 GO

CREATE PROCEDURE [dbo].pts_Prospect_FindMemCustCampTypeContactName ( 
...
SELECT TOP 21
            ISNULL(LTRIM(RTRIM(pr.NameLast)) +  ', '  + LTRIM(RTRIM(pr.NameFirst)), '') + dbo.wtfn_FormatNumber(pr.ProspectID, 10) 'BookMark' ,
...
WHERE ISNULL(LTRIM(RTRIM(pr.NameLast)) +  ', '  + LTRIM(RTRIM(pr.NameFirst)), '') LIKE @SearchText + '%'
AND ISNULL(LTRIM(RTRIM(pr.NameLast)) +  ', '  + LTRIM(RTRIM(pr.NameFirst)), '') + dbo.wtfn_FormatNumber(pr.ProspectID, 10) >= @BookMark
