EXEC [dbo].pts_CheckProc 'pts_Course_FindTrainerCourseID'
 GO

CREATE PROCEDURE [dbo].pts_Course_FindTrainerCourseID ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), cs.CourseID), '') + dbo.wtfn_FormatNumber(cs.CourseID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), cs.CourseID), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), cs.CourseID), '') + dbo.wtfn_FormatNumber(cs.CourseID, 10) >= @BookMark
AND         (cs.TrainerID = @TrainerID)
ORDER BY 'Bookmark'
