EXEC [dbo].pts_CheckProc 'pts_Prospect_FindComCustCampTypeProspectID'
 GO

CREATE PROCEDURE [dbo].pts_Prospect_FindComCustCampTypeProspectID ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), pr.ProspectID), '') + dbo.wtfn_FormatNumber(pr.ProspectID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), pr.ProspectID), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), pr.ProspectID), '') + dbo.wtfn_FormatNumber(pr.ProspectID, 10) >= @BookMark
