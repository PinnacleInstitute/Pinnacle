EXEC [dbo].pts_CheckProc 'pts_Issue_FindAssignedStatusAssignedTo'
 GO

CREATE PROCEDURE [dbo].pts_Issue_FindAssignedStatusAssignedTo ( 
...
SELECT TOP 21
            ISNULL(zis.AssignedTo, '') + dbo.wtfn_FormatNumber(zis.IssueID, 10) 'BookMark' ,
            zis.IssueID 'IssueID' ,
...
WHERE ISNULL(zis.AssignedTo, '') LIKE @SearchText + '%'
AND ISNULL(zis.AssignedTo, '') + dbo.wtfn_FormatNumber(zis.IssueID, 10) >= @BookMark
