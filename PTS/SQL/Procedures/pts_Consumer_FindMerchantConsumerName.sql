EXEC [dbo].pts_CheckProc 'pts_Consumer_FindMerchantConsumerName'
 GO

CREATE PROCEDURE [dbo].pts_Consumer_FindMerchantConsumerName ( 
...
SELECT TOP 21
            ISNULL(LTRIM(RTRIM(csm.NameLast)) +  ', '  + LTRIM(RTRIM(csm.NameFirst)), '') + dbo.wtfn_FormatNumber(csm.ConsumerID, 10) 'BookMark' ,
...
WHERE ISNULL(LTRIM(RTRIM(csm.NameLast)) +  ', '  + LTRIM(RTRIM(csm.NameFirst)), '') LIKE @SearchText + '%'
AND ISNULL(LTRIM(RTRIM(csm.NameLast)) +  ', '  + LTRIM(RTRIM(csm.NameFirst)), '') + dbo.wtfn_FormatNumber(csm.ConsumerID, 10) >= @BookMark
AND         (csm.MerchantID = @MerchantID)
AND         (csm.ReferID = 0)
ORDER BY 'Bookmark'
