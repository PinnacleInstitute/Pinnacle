EXEC [dbo].pts_CheckProc 'pts_LeadCampaign_FindLeadCampaignID'
 GO

CREATE PROCEDURE [dbo].pts_LeadCampaign_FindLeadCampaignID ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), lc.LeadCampaignID), '') + dbo.wtfn_FormatNumber(lc.LeadCampaignID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), lc.LeadCampaignID), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), lc.LeadCampaignID), '') + dbo.wtfn_FormatNumber(lc.LeadCampaignID, 10) >= @BookMark
