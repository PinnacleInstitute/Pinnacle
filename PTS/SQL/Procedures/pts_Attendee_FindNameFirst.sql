EXEC [dbo].pts_CheckProc 'pts_Attendee_FindNameFirst'
 GO

CREATE PROCEDURE [dbo].pts_Attendee_FindNameFirst ( 
...
SELECT TOP 21
            ISNULL(atd.NameFirst, '') + dbo.wtfn_FormatNumber(atd.AttendeeID, 10) 'BookMark' ,
...
WHERE ISNULL(atd.NameFirst, '') LIKE @SearchText + '%'
AND ISNULL(atd.NameFirst, '') + dbo.wtfn_FormatNumber(atd.AttendeeID, 10) >= @BookMark
