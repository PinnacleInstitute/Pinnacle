EXEC [dbo].pts_CheckProc 'pts_Market_FindMarketName'
 GO

CREATE PROCEDURE [dbo].pts_Market_FindMarketName ( 
...
SELECT TOP 21
            ISNULL(mar.MarketName, '') + dbo.wtfn_FormatNumber(mar.MarketID, 10) 'BookMark' ,
...
WHERE ISNULL(mar.MarketName, '') LIKE '%' + @SearchText + '%'
AND ISNULL(mar.MarketName, '') + dbo.wtfn_FormatNumber(mar.MarketID, 10) >= @BookMark
