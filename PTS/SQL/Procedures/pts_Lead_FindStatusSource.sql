EXEC [dbo].pts_CheckProc 'pts_Lead_FindStatusSource'
 GO

CREATE PROCEDURE [dbo].pts_Lead_FindStatusSource ( 
...
SELECT TOP 21
            ISNULL(ld.Source, '') + dbo.wtfn_FormatNumber(ld.LeadID, 10) 'BookMark' ,
            ld.LeadID 'LeadID' ,
...
WHERE ISNULL(ld.Source, '') LIKE @SearchText + '%'
AND ISNULL(ld.Source, '') + dbo.wtfn_FormatNumber(ld.LeadID, 10) >= @BookMark
