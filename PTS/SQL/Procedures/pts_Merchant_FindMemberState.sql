EXEC [dbo].pts_CheckProc 'pts_Merchant_FindMemberState'
 GO

CREATE PROCEDURE [dbo].pts_Merchant_FindMemberState ( 
...
SELECT TOP 21
            ISNULL(mer.State, '') + dbo.wtfn_FormatNumber(mer.MerchantID, 10) 'BookMark' ,
...
WHERE ISNULL(mer.State, '') LIKE @SearchText + '%'
AND ISNULL(mer.State, '') + dbo.wtfn_FormatNumber(mer.MerchantID, 10) >= @BookMark
