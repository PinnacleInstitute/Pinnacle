EXEC [dbo].pts_CheckProc 'pts_SalesOrder_FindStatusSalesOrderID'
 GO

CREATE PROCEDURE [dbo].pts_SalesOrder_FindStatusSalesOrderID ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), so.SalesOrderID), '') + dbo.wtfn_FormatNumber(so.SalesOrderID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), so.SalesOrderID), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), so.SalesOrderID), '') + dbo.wtfn_FormatNumber(so.SalesOrderID, 10) >= @BookMark
