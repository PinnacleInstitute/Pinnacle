EXEC [dbo].pts_CheckProc 'pts_LeadCampaign_FindStatus'
 GO

CREATE PROCEDURE [dbo].pts_LeadCampaign_FindStatus ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), lc.Status), '') + dbo.wtfn_FormatNumber(lc.LeadCampaignID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), lc.Status), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), lc.Status), '') + dbo.wtfn_FormatNumber(lc.LeadCampaignID, 10) >= @BookMark
