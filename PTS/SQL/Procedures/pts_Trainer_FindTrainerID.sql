EXEC [dbo].pts_CheckProc 'pts_Trainer_FindTrainerID'
 GO

CREATE PROCEDURE [dbo].pts_Trainer_FindTrainerID ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), tr.TrainerID), '') + dbo.wtfn_FormatNumber(tr.TrainerID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), tr.TrainerID), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), tr.TrainerID), '') + dbo.wtfn_FormatNumber(tr.TrainerID, 10) >= @BookMark
ORDER BY 'Bookmark'
