EXEC [dbo].pts_CheckProc 'pts_Goal_FindPrimaryStatusDescription'
 GO

CREATE PROCEDURE [dbo].pts_Goal_FindPrimaryStatusDescription ( 
...
SELECT TOP 21
            ISNULL(go.Description, '') + dbo.wtfn_FormatNumber(go.GoalID, 10) 'BookMark' ,
            go.GoalID 'GoalID' ,
...
WHERE ISNULL(go.Description, '') LIKE '%' + @SearchText + '%'
AND ISNULL(go.Description, '') + dbo.wtfn_FormatNumber(go.GoalID, 10) >= @BookMark
