EXEC [dbo].pts_CheckProc 'pts_SalesOrder_FindStatusPinnAmount'
 GO

CREATE PROCEDURE [dbo].pts_SalesOrder_FindStatusPinnAmount ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(15), so.PinnAmount), '') + dbo.wtfn_FormatNumber(so.SalesOrderID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(15), so.PinnAmount), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(15), so.PinnAmount), '') + dbo.wtfn_FormatNumber(so.SalesOrderID, 10) >= @BookMark
