EXEC [dbo].pts_CheckProc 'pts_Attendee_FindMeetingID'
 GO

CREATE PROCEDURE [dbo].pts_Attendee_FindMeetingID ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), atd.MeetingID), '') + dbo.wtfn_FormatNumber(atd.AttendeeID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), atd.MeetingID), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), atd.MeetingID), '') + dbo.wtfn_FormatNumber(atd.AttendeeID, 10) >= @BookMark
