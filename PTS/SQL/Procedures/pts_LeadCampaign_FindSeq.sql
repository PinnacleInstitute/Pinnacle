EXEC [dbo].pts_CheckProc 'pts_LeadCampaign_FindSeq'
 GO

CREATE PROCEDURE [dbo].pts_LeadCampaign_FindSeq ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), lc.Seq), '') + dbo.wtfn_FormatNumber(lc.LeadCampaignID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), lc.Seq), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), lc.Seq), '') + dbo.wtfn_FormatNumber(lc.LeadCampaignID, 10) >= @BookMark
