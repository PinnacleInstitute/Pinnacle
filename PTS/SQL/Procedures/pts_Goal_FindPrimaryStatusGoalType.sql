EXEC [dbo].pts_CheckProc 'pts_Goal_FindPrimaryStatusGoalType'
 GO

CREATE PROCEDURE [dbo].pts_Goal_FindPrimaryStatusGoalType ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), go.GoalType), '') + dbo.wtfn_FormatNumber(go.GoalID, 10) 'BookMark' ,
            go.GoalID 'GoalID' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), go.GoalType), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), go.GoalType), '') + dbo.wtfn_FormatNumber(go.GoalID, 10) >= @BookMark
