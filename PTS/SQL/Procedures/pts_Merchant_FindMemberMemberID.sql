EXEC [dbo].pts_CheckProc 'pts_Merchant_FindMemberMemberID'
 GO

CREATE PROCEDURE [dbo].pts_Merchant_FindMemberMemberID ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), mer.MemberID), '') + dbo.wtfn_FormatNumber(mer.MerchantID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), mer.MemberID), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), mer.MemberID), '') + dbo.wtfn_FormatNumber(mer.MerchantID, 10) >= @BookMark
