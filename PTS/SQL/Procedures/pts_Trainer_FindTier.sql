EXEC [dbo].pts_CheckProc 'pts_Trainer_FindTier'
 GO

CREATE PROCEDURE [dbo].pts_Trainer_FindTier ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), tr.Tier), '') + dbo.wtfn_FormatNumber(tr.TrainerID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), tr.Tier), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), tr.Tier), '') + dbo.wtfn_FormatNumber(tr.TrainerID, 10) >= @BookMark
ORDER BY 'Bookmark'
