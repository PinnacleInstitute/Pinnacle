EXEC [dbo].pts_CheckProc 'pts_Prospect_FindMemCustCampTypeProspectName'
 GO

CREATE PROCEDURE [dbo].pts_Prospect_FindMemCustCampTypeProspectName ( 
...
SELECT TOP 21
            ISNULL(pr.ProspectName, '') + dbo.wtfn_FormatNumber(pr.ProspectID, 10) 'BookMark' ,
...
WHERE ISNULL(pr.ProspectName, '') LIKE '%' + @SearchText + '%'
AND ISNULL(pr.ProspectName, '') + dbo.wtfn_FormatNumber(pr.ProspectID, 10) >= @BookMark
