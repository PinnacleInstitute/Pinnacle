EXEC [dbo].pts_CheckProc 'pts_Merchant_FindMemberZip'
 GO

CREATE PROCEDURE [dbo].pts_Merchant_FindMemberZip ( 
...
SELECT TOP 21
            ISNULL(mer.Zip, '') + dbo.wtfn_FormatNumber(mer.MerchantID, 10) 'BookMark' ,
...
WHERE ISNULL(mer.Zip, '') LIKE @SearchText + '%'
AND ISNULL(mer.Zip, '') + dbo.wtfn_FormatNumber(mer.MerchantID, 10) >= @BookMark
