EXEC [dbo].pts_CheckProc 'pts_Statement_FindMerchantStatusStatementDate'
 GO

CREATE PROCEDURE [dbo].pts_Statement_FindMerchantStatusStatementDate ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), stm.StatementDate, 112), '') + dbo.wtfn_FormatNumber(stm.StatementID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), stm.StatementDate, 112), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), stm.StatementDate, 112), '') + dbo.wtfn_FormatNumber(stm.StatementID, 10) <= @BookMark
