EXEC [dbo].pts_CheckProc 'pts_SalesOrder_FindStatusCommAmount'
 GO

CREATE PROCEDURE [dbo].pts_SalesOrder_FindStatusCommAmount ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(15), so.CommAmount), '') + dbo.wtfn_FormatNumber(so.SalesOrderID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(15), so.CommAmount), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(15), so.CommAmount), '') + dbo.wtfn_FormatNumber(so.SalesOrderID, 10) >= @BookMark
