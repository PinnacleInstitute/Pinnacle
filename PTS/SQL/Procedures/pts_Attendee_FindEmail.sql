EXEC [dbo].pts_CheckProc 'pts_Attendee_FindEmail'
 GO

CREATE PROCEDURE [dbo].pts_Attendee_FindEmail ( 
...
SELECT TOP 21
            ISNULL(atd.Email, '') + dbo.wtfn_FormatNumber(atd.AttendeeID, 10) 'BookMark' ,
...
WHERE ISNULL(atd.Email, '') LIKE @SearchText + '%'
AND ISNULL(atd.Email, '') + dbo.wtfn_FormatNumber(atd.AttendeeID, 10) >= @BookMark
