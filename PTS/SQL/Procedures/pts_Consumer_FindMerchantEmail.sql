EXEC [dbo].pts_CheckProc 'pts_Consumer_FindMerchantEmail'
 GO

CREATE PROCEDURE [dbo].pts_Consumer_FindMerchantEmail ( 
...
SELECT TOP 21
            ISNULL(csm.Email, '') + dbo.wtfn_FormatNumber(csm.ConsumerID, 10) 'BookMark' ,
...
WHERE ISNULL(csm.Email, '') LIKE @SearchText + '%'
AND ISNULL(csm.Email, '') + dbo.wtfn_FormatNumber(csm.ConsumerID, 10) >= @BookMark
