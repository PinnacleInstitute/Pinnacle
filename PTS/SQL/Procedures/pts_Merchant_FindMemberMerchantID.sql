EXEC [dbo].pts_CheckProc 'pts_Merchant_FindMemberMerchantID'
 GO

CREATE PROCEDURE [dbo].pts_Merchant_FindMemberMerchantID ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), mer.MerchantID), '') + dbo.wtfn_FormatNumber(mer.MerchantID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), mer.MerchantID), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), mer.MerchantID), '') + dbo.wtfn_FormatNumber(mer.MerchantID, 10) >= @BookMark
AND         (mer.MemberID = @MemberID)
ORDER BY 'Bookmark'
