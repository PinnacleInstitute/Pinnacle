EXEC [dbo].pts_CheckProc 'pts_Prospect_FindMemCustCampTypeLeadCampaignID'
 GO

CREATE PROCEDURE [dbo].pts_Prospect_FindMemCustCampTypeLeadCampaignID ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), pr.LeadCampaignID), '') + dbo.wtfn_FormatNumber(pr.ProspectID, 10) 'BookMark' ,
...
AND ISNULL(CONVERT(nvarchar(10), pr.LeadCampaignID), '') + dbo.wtfn_FormatNumber(pr.ProspectID, 10) >= @BookMark
