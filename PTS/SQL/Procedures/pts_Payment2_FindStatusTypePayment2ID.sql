EXEC [dbo].pts_CheckProc 'pts_Payment2_FindStatusTypePayment2ID'
 GO

CREATE PROCEDURE [dbo].pts_Payment2_FindStatusTypePayment2ID ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), pa2.Payment2ID), '') + dbo.wtfn_FormatNumber(pa2.Payment2ID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), pa2.Payment2ID), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), pa2.Payment2ID), '') + dbo.wtfn_FormatNumber(pa2.Payment2ID, 10) >= @BookMark
AND         (pa2.MerchantID = @MerchantID)
AND         (pa2.Status = @Status)
AND         (pa2.PayType >= @PayType)
AND         (pa2.PayType <= @PayType2)
ORDER BY 'Bookmark'
