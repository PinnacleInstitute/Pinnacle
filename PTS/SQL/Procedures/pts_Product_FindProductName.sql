EXEC [dbo].pts_CheckProc 'pts_Product_FindProductName'
 GO

CREATE PROCEDURE [dbo].pts_Product_FindProductName ( 
...
SELECT TOP 21
            ISNULL(pd.ProductName, '') + dbo.wtfn_FormatNumber(pd.ProductID, 10) 'BookMark' ,
...
AND ISNULL(pd.ProductName, '') + dbo.wtfn_FormatNumber(pd.ProductID, 10) >= @BookMark
