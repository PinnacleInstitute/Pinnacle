EXEC [dbo].pts_CheckProc 'pts_Product_FindCode'
 GO

CREATE PROCEDURE [dbo].pts_Product_FindCode ( 
...
SELECT TOP 21
            ISNULL(pd.Code, '') + dbo.wtfn_FormatNumber(pd.ProductID, 10) 'BookMark' ,
...
WHERE ISNULL(pd.Code, '') LIKE @SearchText + '%'
AND ISNULL(pd.Code, '') + dbo.wtfn_FormatNumber(pd.ProductID, 10) >= @BookMark
