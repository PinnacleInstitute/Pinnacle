EXEC [dbo].pts_CheckProc 'pts_Expense_FindTypePurpose'
 GO

CREATE PROCEDURE [dbo].pts_Expense_FindTypePurpose ( 
...
SELECT TOP 21
            ISNULL(ex.Purpose, '') + dbo.wtfn_FormatNumber(ex.ExpenseID, 10) 'BookMark' ,
...
WHERE ISNULL(ex.Purpose, '') LIKE '%' + @SearchText + '%'
AND ISNULL(ex.Purpose, '') + dbo.wtfn_FormatNumber(ex.ExpenseID, 10) >= @BookMark
