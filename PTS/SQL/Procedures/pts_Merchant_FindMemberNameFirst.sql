EXEC [dbo].pts_CheckProc 'pts_Merchant_FindMemberNameFirst'
 GO

CREATE PROCEDURE [dbo].pts_Merchant_FindMemberNameFirst ( 
...
SELECT TOP 21
            ISNULL(mer.NameFirst, '') + dbo.wtfn_FormatNumber(mer.MerchantID, 10) 'BookMark' ,
...
WHERE ISNULL(mer.NameFirst, '') LIKE @SearchText + '%'
AND ISNULL(mer.NameFirst, '') + dbo.wtfn_FormatNumber(mer.MerchantID, 10) >= @BookMark
