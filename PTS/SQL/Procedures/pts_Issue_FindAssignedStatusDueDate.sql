EXEC [dbo].pts_CheckProc 'pts_Issue_FindAssignedStatusDueDate'
 GO

CREATE PROCEDURE [dbo].pts_Issue_FindAssignedStatusDueDate ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), zis.DueDate, 112), '') + dbo.wtfn_FormatNumber(zis.IssueID, 10) 'BookMark' ,
            zis.IssueID 'IssueID' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), zis.DueDate, 112), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), zis.DueDate, 112), '') + dbo.wtfn_FormatNumber(zis.IssueID, 10) <= @BookMark
