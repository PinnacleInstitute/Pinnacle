EXEC [dbo].pts_CheckProc 'pts_BonusItem_FindCommType'
 GO

CREATE PROCEDURE [dbo].pts_BonusItem_FindCommType ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), bi.CommType), '') + dbo.wtfn_FormatNumber(bi.BonusItemID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), bi.CommType), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), bi.CommType), '') + dbo.wtfn_FormatNumber(bi.BonusItemID, 10) >= @BookMark
