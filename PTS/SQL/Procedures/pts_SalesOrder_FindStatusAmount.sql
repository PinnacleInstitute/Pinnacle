EXEC [dbo].pts_CheckProc 'pts_SalesOrder_FindStatusAmount'
 GO

CREATE PROCEDURE [dbo].pts_SalesOrder_FindStatusAmount ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(15), so.Amount), '') + dbo.wtfn_FormatNumber(so.SalesOrderID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(15), so.Amount), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(15), so.Amount), '') + dbo.wtfn_FormatNumber(so.SalesOrderID, 10) >= @BookMark
