EXEC [dbo].pts_CheckProc 'pts_Merchant_FindMemberEmail'
 GO

CREATE PROCEDURE [dbo].pts_Merchant_FindMemberEmail ( 
...
SELECT TOP 21
            ISNULL(mer.Email, '') + dbo.wtfn_FormatNumber(mer.MerchantID, 10) 'BookMark' ,
...
WHERE ISNULL(mer.Email, '') LIKE @SearchText + '%'
AND ISNULL(mer.Email, '') + dbo.wtfn_FormatNumber(mer.MerchantID, 10) >= @BookMark
