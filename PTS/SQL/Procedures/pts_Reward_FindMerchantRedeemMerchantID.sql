EXEC [dbo].pts_CheckProc 'pts_Reward_FindMerchantRedeemMerchantID'
 GO

CREATE PROCEDURE [dbo].pts_Reward_FindMerchantRedeemMerchantID ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), rew.MerchantID), '') + dbo.wtfn_FormatNumber(rew.RewardID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), rew.MerchantID), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), rew.MerchantID), '') + dbo.wtfn_FormatNumber(rew.RewardID, 10) >= @BookMark
