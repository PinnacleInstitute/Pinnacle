EXEC [dbo].pts_CheckProc 'pts_Affiliate_FindStatus'
 GO

CREATE PROCEDURE [dbo].pts_Affiliate_FindStatus ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), af.Status), '') + dbo.wtfn_FormatNumber(af.AffiliateID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), af.Status), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), af.Status), '') + dbo.wtfn_FormatNumber(af.AffiliateID, 10) >= @BookMark
ORDER BY 'Bookmark'
