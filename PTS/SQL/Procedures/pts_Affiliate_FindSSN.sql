EXEC [dbo].pts_CheckProc 'pts_Affiliate_FindSSN'
 GO

CREATE PROCEDURE [dbo].pts_Affiliate_FindSSN ( 
   @SearchText nvarchar (18),
   @Bookmark nvarchar (28),
...
SELECT TOP 21
            ISNULL(af.SSN, '') + dbo.wtfn_FormatNumber(af.AffiliateID, 10) 'BookMark' ,
...
WHERE ISNULL(af.SSN, '') LIKE @SearchText + '%'
AND ISNULL(af.SSN, '') + dbo.wtfn_FormatNumber(af.AffiliateID, 10) >= @BookMark
ORDER BY 'Bookmark'
