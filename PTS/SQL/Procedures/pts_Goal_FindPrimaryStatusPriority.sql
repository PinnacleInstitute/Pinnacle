EXEC [dbo].pts_CheckProc 'pts_Goal_FindPrimaryStatusPriority'
 GO

CREATE PROCEDURE [dbo].pts_Goal_FindPrimaryStatusPriority ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), go.Priority), '') + dbo.wtfn_FormatNumber(go.GoalID, 10) 'BookMark' ,
            go.GoalID 'GoalID' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), go.Priority), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), go.Priority), '') + dbo.wtfn_FormatNumber(go.GoalID, 10) >= @BookMark
