EXEC [dbo].pts_CheckProc 'pts_SalesOrder_FindStatusProspectName'
 GO

CREATE PROCEDURE [dbo].pts_SalesOrder_FindStatusProspectName ( 
...
SELECT TOP 21
            ISNULL(pr.ProspectName, '') + dbo.wtfn_FormatNumber(so.SalesOrderID, 10) 'BookMark' ,
...
AND ISNULL(pr.ProspectName, '') + dbo.wtfn_FormatNumber(so.SalesOrderID, 10) >= @BookMark
