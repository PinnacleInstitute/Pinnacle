EXEC [dbo].pts_CheckProc 'pts_Merchant_FindMemberNameLast'
 GO

CREATE PROCEDURE [dbo].pts_Merchant_FindMemberNameLast ( 
...
SELECT TOP 21
            ISNULL(mer.NameLast, '') + dbo.wtfn_FormatNumber(mer.MerchantID, 10) 'BookMark' ,
...
WHERE ISNULL(mer.NameLast, '') LIKE @SearchText + '%'
AND ISNULL(mer.NameLast, '') + dbo.wtfn_FormatNumber(mer.MerchantID, 10) >= @BookMark
