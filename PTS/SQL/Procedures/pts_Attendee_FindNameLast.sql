EXEC [dbo].pts_CheckProc 'pts_Attendee_FindNameLast'
 GO

CREATE PROCEDURE [dbo].pts_Attendee_FindNameLast ( 
...
SELECT TOP 21
            ISNULL(atd.NameLast, '') + dbo.wtfn_FormatNumber(atd.AttendeeID, 10) 'BookMark' ,
...
WHERE ISNULL(atd.NameLast, '') LIKE @SearchText + '%'
AND ISNULL(atd.NameLast, '') + dbo.wtfn_FormatNumber(atd.AttendeeID, 10) >= @BookMark
