EXEC [dbo].pts_CheckProc 'pts_Merchant_FindMemberMerchantName'
 GO

CREATE PROCEDURE [dbo].pts_Merchant_FindMemberMerchantName ( 
...
SELECT TOP 21
            ISNULL(mer.MerchantName, '') + dbo.wtfn_FormatNumber(mer.MerchantID, 10) 'BookMark' ,
...
WHERE ISNULL(mer.MerchantName, '') LIKE @SearchText + '%'
AND ISNULL(mer.MerchantName, '') + dbo.wtfn_FormatNumber(mer.MerchantID, 10) >= @BookMark
