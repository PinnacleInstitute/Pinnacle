EXEC [dbo].pts_CheckProc 'pts_Issue_FindAssignedStatusDoneDate'
 GO

CREATE PROCEDURE [dbo].pts_Issue_FindAssignedStatusDoneDate ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), zis.DoneDate, 112), '') + dbo.wtfn_FormatNumber(zis.IssueID, 10) 'BookMark' ,
            zis.IssueID 'IssueID' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), zis.DoneDate, 112), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), zis.DoneDate, 112), '') + dbo.wtfn_FormatNumber(zis.IssueID, 10) <= @BookMark
