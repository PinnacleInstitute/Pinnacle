EXEC [dbo].pts_CheckProc 'pts_SalesOrder_FindStatusAffiliateID'
 GO

CREATE PROCEDURE [dbo].pts_SalesOrder_FindStatusAffiliateID ( 
...
SELECT TOP 21
            ISNULL(CONVERT(nvarchar(10), so.AffiliateID), '') + dbo.wtfn_FormatNumber(so.SalesOrderID, 10) 'BookMark' ,
...
WHERE ISNULL(CONVERT(nvarchar(10), so.AffiliateID), '') LIKE @SearchText + '%'
AND ISNULL(CONVERT(nvarchar(10), so.AffiliateID), '') + dbo.wtfn_FormatNumber(so.SalesOrderID, 10) >= @BookMark
